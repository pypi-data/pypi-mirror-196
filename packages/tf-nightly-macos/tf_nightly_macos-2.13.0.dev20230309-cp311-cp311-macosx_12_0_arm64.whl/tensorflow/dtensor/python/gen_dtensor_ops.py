...
@_dispatch.add_fallback_dispatch_list
@_dispatch.add_type_based_api_dispatcher
@tf_export('relayout_grad')
def relayout_grad(input, forward_input, name=None):
  r"""TODO: add doc.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "RelayoutGrad", name, input, forward_input)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
      pass
    try:
      _result = _dispatcher_for_relayout_grad(
          (input, forward_input, name,), None)
      if _result is not NotImplemented:
        return _result
      return relayout_grad_eager_fallback(
          input, forward_input, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      _result = _dispatch.dispatch(
            relayout_grad, (), dict(input=input, forward_input=forward_input,
                                    name=name)
          )
      if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
        return _result
  ...
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "RelayoutGrad", input=input, forward_input=forward_input, name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          relayout_grad, (), dict(input=input, forward_input=forward_input,
                                  name=name)
  ...
  if _execute.must_record_gradient():
    ...
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "RelayoutGrad", _inputs_flat, _attrs, _result)
  _result, = _result
  return _result

RelayoutGrad = tf_export("raw_ops.RelayoutGrad")(_ops.to_raw_op(relayout_grad))
_dispatcher_for_relayout_grad = relayout_grad._tf_type_based_dispatcher.Dispatch


def relayout_grad_eager_fallback(input, forward_input, name, ctx):
  _attr_T, _inputs_T = _execute.args_to_matching_eager([input, forward_input], ctx, [])
  (input, forward_input) = _inputs_T
  _inputs_flat = [input, forward_input]
  _attrs = ("T", _attr_T)
  _result = _execute.execute(b"RelayoutGrad", 1, inputs=_inputs_flat,
                             attrs=_attrs, ctx=ctx, name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
        "RelayoutGrad", _inputs_flat, _attrs, _result)
