...
class dataset_loader:
    ...
    def get_data(self, data_path='../data/data.csv'):
        ...
        # 将数据里的数字类型转换为float 保留两位小数
        self.data = self.data.applymap(lambda x: round(float(x), 2) if isinstance(x, (int, float)) else x)
        return self.data
    ...
    def observe(self, mlflow):
        ...
        """
        # 数据报告
        mlflow.log_text("\n".join(self.__data_report()),"data_report" )
        # 新增数据集

    def save(self, path='../data/', name='data.csv'):
        """
        保存数据集
        :param name:
        :param path:
...
class dataset_feature:
    ...
    def save(self, path='../data/', name='dataset.csv'):
        """
        保存数据集
        :param name:
        :param path:
        ...
        if not os.path.exists(path):
            os.mkdir(path)
        self.result.to_csv(path+name, index=False, encoding='utf-8')
    ...
    def execute(self):
        ...
        for symbol in symbol_list:
            ...
            for func in self.features_list:
                func(symbol_data)
            self.result = pd.concat([self.result, symbol_data])
        # 重置索引
        self.result = self.result.reset_index(drop=True)
        # 将日期转换为字符串
        self.result['trade_date'] = self.result['trade_date'].apply(lambda x: str(x))
        # 将数据里的数字类型转换为float 保留两位小数
        self.result = self.result.applymap(lambda x: round(float(x), 2) if isinstance(x, (int, float)) else x)
        return self.result
