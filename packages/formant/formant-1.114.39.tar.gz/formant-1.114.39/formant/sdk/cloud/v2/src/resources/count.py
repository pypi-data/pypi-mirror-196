...
class Count(Resources):
    ...
    def history_async(self, count_history_query: CountHistoryQuery):
        'Gets all the objects(of given type) during the timestamp'
        client = self._get_client()
        response = count_controller_history.asyncio_detailed(client=client, json_body=count_history_query)
        return response
