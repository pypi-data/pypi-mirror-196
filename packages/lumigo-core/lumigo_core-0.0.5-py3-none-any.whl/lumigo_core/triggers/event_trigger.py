...
from lumigo_core.triggers.trigger_parsing import (
    EVENT_TRIGGER_PARSERS,
    INNER_MESSAGES_MAGIC_PATTERN,
)
from lumigo_core.triggers.trigger_parsing.event_trigger_base import TriggerType
...
def recursive_parse_trigger(
    ...
    triggers = []
    if level >= CoreConfiguration.chained_services_max_depth:
    ...
    for parser in EVENT_TRIGGER_PARSERS:
        if parser.should_handle(message):
            ...
            if len(inner_messages) >= CoreConfiguration.chained_services_max_width:
                get_logger().info("Chained services parsing has stopped due to width")
                inner_messages = inner_messages[
                    : CoreConfiguration.chained_services_max_width
                ]
                ...
                if INNER_MESSAGES_MAGIC_PATTERN.search(sub_message):
                    ...
                    triggers.extend(
                        recursive_parse_trigger(
                            json.loads(sub_message),
                            parent_id=current_trigger_id,
                            level=level + 1,
                        )
                    )
