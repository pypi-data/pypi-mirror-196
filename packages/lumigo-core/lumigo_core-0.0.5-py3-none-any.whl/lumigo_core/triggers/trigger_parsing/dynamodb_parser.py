...
class DynamoDBEventTriggerParser(EventTriggerParser):
    ...
    def handle(event: Dict[Any, Any], target_id: Optional[str]) -> TriggerType:
        creation_time = DynamoDBEventTriggerParser._get_ddb_approx_creation_time_ms(
            event
        )
        message_ids = []
        ...
        for record in event["Records"]:
            ...
            if event_name in ("MODIFY", "REMOVE") and record.get("dynamodb", {}).get(
                "Keys"
            ):
    ...
    def _get_ddb_approx_creation_time_ms(event: Dict[Any, Any]) -> int:
        return (
            int(
                event["Records"][0]
                .get("dynamodb", {})
                .get("ApproximateCreationDateTime", 0)
            )
            * 1000
        )
