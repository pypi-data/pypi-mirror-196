...
class ParamsEnv(Env, ABC):
    def __init__(self):
        ...
        self.action_space = Box(low=0, high=2, shape=(12,))
        # 参数范围数组 Box允许在整个空间内有连续值
        self.observation_space = Box(low=-20, high=20, shape=(8,))
        # 参数初始值设定
    ...
    def get_state(self, observation):
        state = np.array(observation).reshape(1, 8)
    ...
    def reset(self):
        ...
        self.last_max = 0
        self.done = False
        observation = self.state  # 重置参数为初始值
    ...
    def step(self, action):
        ...
        vb = (action[5]) + a * 0.01
        p = [k1, k2, k3, k4, fa, vb]
        err = kinetic_modeling.CompartmentModelling.compartment_model_residual(self.patient, self.params, **self.kws)
        print('err= ', err)
        new_err = kinetic_modeling.CompartmentModelling.compartment_model_residual(self.patient, p, **self.kws)
        err_err = new_err - err
        print('err_err= ', err_err)
