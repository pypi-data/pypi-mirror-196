    ...
    def check_link(cls, link: str, client: object = None,
        ...
        if link.startswith('@') or not re.compile(r'rubika\.ir\/join[gc]\/\w+').search(link) and re.search(r'rubika\.ir/\w{4,25}', link):
            ...
            if 'joing' in link:
                ...
                if not isinstance(result, dict):
                    return json.loads(result).get('data')['group']['group_guid']
                else:
                    return result.get('data')['group']['group_guid']
                ...
                if not isinstance(result, dict):
                    return json.loads(result).get('data')['channel']['channel_guid']
                else:
                    return result.get('data')['channel']['channel_guid']
