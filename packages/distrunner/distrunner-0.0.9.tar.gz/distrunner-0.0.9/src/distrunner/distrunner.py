...
class DistRunner(AbstractContextManager):
    ...
    def bootstrap_repo(self) -> ModuleType:
        ...
        self._install_requirements(
            client=self._dask_client, requirements=["GitPython"]
        )
        ...
        requirements_list = self._gather_requirements(
            ...
            git_exec_path=self._git_exec_path,
        )
    ...
    def _gather_requirements(
        ...
        if not repo_path.exists():
            cmd = git.cmd.Git()
            with cmd.custom_environment(GIT_EXEC_PATH=git_exec_path):
