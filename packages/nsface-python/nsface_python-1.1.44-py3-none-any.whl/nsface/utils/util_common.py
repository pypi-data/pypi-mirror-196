...
def draw_vispose(img,faces,param_lst,roi_box_lst,dense=False,std_size=120, \
                 color_box=(255,255,255),line_box=4,color_land=(255,255,255),point_land=5,color_vis=(40, 255, 0),line_vis=2):

    dimg = img.copy()
    # draw bbox
    dimg = draw_result2(dimg,faces,color=color_box,size=line_box)

    # draw land
    dimg = draw_result_land2(dimg,faces,point_size=point_land,color=color_land)

    # draw vis-pose
    for param,roi_box in zip(param_lst,roi_box_lst):
        P,pose = calc_pose(param)
        ver = recon_ver(param,roi_box, dense, std_size)
        dimg = plot_pose_box2(dimg, P, ver,color=color_vis,line_width=line_vis)
