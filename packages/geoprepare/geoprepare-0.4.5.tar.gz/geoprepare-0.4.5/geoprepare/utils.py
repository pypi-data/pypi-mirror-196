...
def fill_till_valid(grp, fill_zero=False, interpolate=False):
    """
    ...
    if fill_zero:
        ...
        grp.loc[:idx] = grp.loc[:idx].astype(float).interpolate(method='linear',
...
def extended_dataframe(df, eo_vars):
    ...
    df.index = pd.to_datetime(df['year'].astype(int), format='%Y') + pd.to_timedelta(df['doy'] - 1, unit='d')
