...
def run(path_config_file='geoextract.txt'):
    ...
    for country, scale, crop, growing_season in pbar:  # e.g. rwanda, cr, admin1
        ...
        if not df_cal.empty:
            ...
            gm.df_ccs.to_csv(r'D:\input\metadata\a5.csv')

            # 7. Post process data
            gm.post_process()
            gm.df_ccs.to_csv(r'D:\input\metadata\a6.csv')
