...
ENGINE_SETTINGS "index_granularity=32, min_index_granularity_bytes=2048"
ENGINE_TTL "toDate(date) + INTERVAL 3 DAY"
        """
        ds = parse(s)
        self.assertIn('engine', ds.nodes[0])
        self.assertEqual(ds.nodes[0]['engine']['type'], 'SummingMergeTree')
        self.assertEqual(ds.nodes[0]['engine']['args'], [('partition_key', 'toYYYYMM(date)'), ('sorting_key', '(country, date)'), ('settings', 'index_granularity=32, min_index_granularity_bytes=2048'), ('ttl', 'toDate(date) + INTERVAL 3 DAY')])
...
DATASOURCE "test"
        ...
        with self.assertRaises(ValidationException) as ctx:
            parse(s)
        self.assertEqual("Validation error, found DATASOURCE\n in line 6: string index out of range", str(ctx.exception))
...
ENGINE "SummingMergeTree${doesnotexist}"
    ...
    def test_parse_datasource_import(self):
        content = """
DESCRIPTION >
    Test
