...
def deploy_flow(project: str, bucket: str, kedro_package: str, kedro_pipeline: str = None, tags: List[str] = None, env: str = None, dev: bool = False) -> None:
    ...
    kubernetes_job = KubernetesJob.load(f"{kedro_package}-kubernetes-job")

    parameters = {"pipeline_name": kedro_pipeline, "env": env}

    deployment = Deployment.build_from_flow(
        flow=_create_pipeline.with_options(name=f"{kedro_package}.{kedro_pipeline}", version=version),
        name="kubernetes-job",
...
def deploy_prefect_flow(project: str, bucket: str, kedro_package: str, kedro_pipeline: str, tags: List[str], env: str, dev: bool) -> None:
    deploy_flow(project, bucket, kedro_package, kedro_pipeline, tags, env, dev)
