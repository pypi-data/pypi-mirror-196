...
class EventManager:
    ...
    def getDataframeFromEvents(self, events):
        ...
        return pd.DataFrame(eventDict, columns=["name", "start", "duration", "end", "amplitude", "manual", "group", "data"])
        ...
        for annotationIndex, event in enumerate(annotationNames):
            ...
            if start:
                ...
                if not isEventMarker:
                    currentEventIndexes[eventName][eventValueName] = [lastDefaultGroupIndex[eventName], None]
                    ...
                    currentEventIndexes[eventName][eventValueName] = [getSampleIndex(annotationIndex), lastEnding]
