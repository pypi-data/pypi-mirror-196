    ...
    def test_vrf(self):
        ...
        for interface_o in interface_qs:
            ...
            if interface_vrf_o == ipaddress_vrf_o:
                ...
                self.log_warning(
                    interface_o,
                    f"VRF differs between interface ({interface_vrf_o}) and interface's IP address ({ipaddress_vrf_o})",
                )
        ...
        for interface_o in interface_qs:
            for ip_address_o in interface_o.ip_addresses.all():
                ...
                if prefix_qs:
                    ...
                    self.log_failure(
                        interface_o,
                        f"Prefix missing for IP address {str(ip_address_o.address)} with VRF {vrf_o}",
                    )
    ...
    def test_ipam(self):
        ...
        for arptableentry_o in ArpTableEntry.objects.all():
            ...
            if ipaddresses_qs:
                ...
                self.log_failure(
                    arptableentry_o,
                    f"IP address {str(arptableentry_o.ip_address)} not found in IPAM for VRF {interface_vrf_o}",
                )
