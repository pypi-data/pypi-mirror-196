...
def ingest(log):
    ...
    for item in log.parsed_output:
        ...
        mac_address = (
            ...
            if item.get("hw_address") and not utils.incomplete_mac(item.get("hw_address")[0])
            else None
        )
        mode = utils.normalize_interface_mode(item.get("port_link_type"))
        int_type = utils.normalize_interface_type(item.get("intf"))
        enabled = utils.normalize_interface_status(item.get("line_status")) if item.get("line_status") else None
        mtu = utils.normalize_interface_mtu(item.get("mtu")) if item.get("mtu") else None
