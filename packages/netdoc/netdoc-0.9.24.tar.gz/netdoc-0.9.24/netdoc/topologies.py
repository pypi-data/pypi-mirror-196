...
def get_l2_drawio_topology(queryset, diagram):
    ...
    for node in nodes:
        ...
        node_style = node.get("role") if node.get("role") in DRAWIO_ROLE_MAP else "unknown"
        drawio_o.add_node(
            id=node_label,
            url="Page-1",
        ...
        for address_o in interface_o.ip_addresses.all():
            ...
            if network_id not in networks:
                networks[network_id] = {
                    ...
                    "title": Template(NETWORK_TEMPLATE).render(network=network_o, vrf=address_o.vrf),
                    "shape": "box",
                    "borderWidth": 2,
                    "color": {
                        "border": "#1b9bd0",
