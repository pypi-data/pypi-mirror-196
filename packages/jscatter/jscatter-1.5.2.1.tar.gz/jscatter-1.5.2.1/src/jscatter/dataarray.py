...
# this speeds up using a pool of workers. Should be safe as only one fit at a time is runing.
bayes_dL = None
...
class dataListBase(list):
    ...
    def _read_objekt(self, objekt=None,
        ...
        if isinstance(objekt, dataList):
            ...
            parsed = parallel.doForList(_parse, filelist, loopover='filename', index=index, usecols=usecols,
                    ...
                    XYeYeX=XYeYeX, delimiter=delimiter, takeline=takeline,
                    lines2parameter=lines2parameter, encoding=encoding, block=self._block,
                    output=False, ncpu=1)  # ncpu=1 avoids multiprossesing
    ...
    def interpolate(self, func=None, invfunc=None, deg=1, col='Y', **kwargs):
        ...
        **kwargs : keyword arguments with float or array-like values.
        ...
        if func is not None:
            YY = np.array([interp1d(ele.X, func(ele[col]), kind=deg, fill_value='extrapolate')(X) for ele in self])
        else:
            YY = np.array([interp1d(ele.X, ele[col], kind=deg, fill_value='extrapolate')(X) for ele in self])
    ...
    def _errorfunction(self, *args, **kwargs):
        ...
        if self._fitmethod in ['lm', 'trf', 'dogbox']:
            ...
            # here error is the residual, but we return ln_likelyhood + ln_prior
    ...
    def getChi2Trace(self):
        ...
        chi2trace.lastfreepar = {k.strip(): v for k, v in zip(self._freepar.keys(), self._chi2trace[-1][2:])}
        return chi2trace
    ...
    def fit(self, model, freepar={}, fixpar={}, mapNames={}, method='lm', xslice=slice(None), condition=None,
        ...
        method : 'lm', 'trf', 'dogbox', ‘Nelder-Mead’, 'bayes', 'differential_evolution', ‘BFGS’,
            ...
            - **'differential_evolution'** is a stochastic population based method that is useful for global
              ...
              - The optional *workers* argument allows parallel processing in a pool of workers.
                ``workers=-1`` uses all cpus, otherwise the number of cpus (default 1).
        ...
        Notes
        -----
        * For :math:`\bm{\chi^2}` **minimization** the unbiased estimate
        ...
        * For :math:`\bm{\chi^2}` **minimization** the resulting parameter errors are 1-sigma errors as
        ...
        * For **Bayesian estimations ('bayes')** errors are determined from the likelihood and represent the
          ...
          If likelyhood and prior are correctly weighted ( for each :math:`\bm{\chi^2}` is 1 ) both contribute equally
        ...
        * Each dataArray in a dataList can be fit individually (same model function) like this ::
        ...
        Most important **additional kwargs for 'least_square'** methods 'lm', 'trf', 'dogbox' ::
        ...
        **Basic examples with synthetic data.**
        ...
        - **Comparison of fit methods** ::
           ...
           # use only a single dataArray from i5 and reduce tolerance
           i5[5].fit(model=diffusion,freepar={'D':0.2,'A':1}, fixpar={'elastic':0.0},
                  mapNames= {'t':'X','wavevector':'q'},
                  condition=lambda a:a.X>0.01, method='bayes', tolerance=20, bayesnsteps=1000)
           # >8000 evaluations; error YES
           # tolerance should be >= 50 not smaller
           # For full dataset it takes quite long
        ...
        # this is the fit
        if self._output: print('^^^^^^^^^^^^^^ start fit ^^^^^^^^^^^^^^')
        ...
        if method in ['lm', 'trf', 'dogbox']:
            ...
            res = scipy.optimize.differential_evolution(func=pool_error,
                                                        bounds=bounds, workers=workers, disp=disp, args=(0,), **kw)
            ...
            with multiprocessing.Pool() as pool:
                sampler = emcee.EnsembleSampler(nwalkers=nwalkers, ndim=ndim,
                                                log_prob_fn=pool_error, pool=pool, args=(0,), **kw)
                ...
                while True:
                    state = sampler.run_mcmc(initial_state=init, nsteps=nsteps, progress=True)
        ...
        if self._output: print('degrees of freedom = ', self._dof)
        if cov is not None:
            ...
            invcii = 1/cov.diagonal()**0.5  # inverse sigma from diagonal
    ...
    def estimateError(self, output=True):
        """
        Estimate error using a refined fit (``method='lm'``).

        The .fit method is used (``method='lm'``) to result in an error. A previous fit determines the
...
class dataArray(dataArrayBase):
    ...
    def simulate(self, *args, **kwargs):
        if self._asdataList is None:
            self._asdataList = dataList(self)
        self._asdataList.simulate(*args, **kwargs)
