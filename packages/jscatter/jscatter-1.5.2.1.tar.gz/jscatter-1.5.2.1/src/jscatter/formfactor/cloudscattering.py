...
def _scattering(point, r, q, blength, iff, formfactoramp=None, rms=0, ffpolydispersity=0):
    ...
    r = formel.shared_recover(r)
    blength = formel.shared_recover(blength)
    iff = formel.shared_recover(iff)
    formfactoramp = formel.shared_recover(formfactoramp)
    ...
    **Examples**
    ...
    if relError == 0:
        ...
        if useFortran:
            ...
            cloud_sh = formel.shared_create(cloud)
            blength_sh = formel.shared_create(blength)
            iff_sh = formel.shared_create(iff)
            fa_sh = formel.shared_create(fa)
        ...
        if useFortran:
            res = formel.doForList(_sphaverage_scattering, q,
                                     r=cloud_sh, blength=blength_sh, iff=iff_sh, formfactoramp=fa_sh,
