...
class bioTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.Sxres = js.bio.xscatIntUniv(self.arg61.residues, output=0,refreshVolume=False)
        self.Sn = js.bio.nscatIntUniv(self.arg61.atoms, output=0, refreshVolume=False)
        # test explicit deuteration
        self.arg61.select_atoms('name H').deuteration =1
        self.Sndeut1 = js.bio.nscatIntUniv(self.arg61.atoms, output=0, refreshVolume=False)
        ...
        self.Dq = js.bio.diffusionTRUnivTensor(self.arg61.atoms, Dtrans=Dtrans, Drot=Drot, refreshVolume=False,output=0)
        self.Dqylm = js.bio.diffusionTRUnivYlm(self.arg61.atoms, Dtrans=Dtrans, Drot=Drot, refreshVolume=False)
    ...
    def test_formfactor(self):
        ...
        self.assertAlmostEqual(self.Sn.edensitySolvent, 332.36, 2)
        self.assertAlmostEqual(self.Sn.bcDensitySol, 0.0006363, 7)
        self.assertAlmostEqual(self.Sn.sldExclSolvent, 0.0006363, 7)
        self.assertAlmostEqual(self.Sn.massdensity, 1.343, 2)
        self.assertAlmostEqual(self.Sn.RgInt, 2.622, 2)

    def test_diffusion(self):
        # diffusion constant
        nptest.assert_array_almost_equal(self.Dqylm.Y, [0.000122, 0.000122, 0.000144, 0.000149], 6)
        nptest.assert_array_almost_equal(self.Dq.Y,    [0.000122, 0.000126, 0.000344, 0.000418], 6)

    def test_normalmode(self):
        # normal mode eigenvalue first nontrivial
        _ = self.vnm[6]
        # zero eigenvalues
        nptest.assert_almost_equal(self.vnm.eigenvalues[:6], 0, 5)
        # first eigenvalues
        nptest.assert_almost_equal(self.vnm.eigenvalues[6:11], [0.0099, 0.0099, 0.0469, 0.068, 0.0687], 3)
        vnm6 = js.bio.intScatFuncPMode(self.vnm, 6, error=30, output=0, refreshVolume=False)
        nptest.assert_array_almost_equal(vnm6.Y, [2.61e-21, 6.42e-16, 1.23e-10, 4.50e-10], 2)

        # test normal mode options
        nmresidues = js.bio.vibNM(self.arg61.residues).allatommode(6)
        nmallatoms = js.bio.vibNM(self.arg61.residues[:10].atoms).allatommode(6)
        nmaCaatoms = js.bio.vibNM(self.arg61.select_atoms('name CA')).allatommode(6)
...
def doTest2(verbosity=1):
    ...
    suite.addTest(loader.loadTestsFromTestCase(plotTest))
