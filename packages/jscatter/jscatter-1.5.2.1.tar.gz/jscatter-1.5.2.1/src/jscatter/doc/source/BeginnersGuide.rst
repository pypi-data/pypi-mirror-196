...
This tells plot and fit commands which data to use. See :ref:`dataArray` how to change used columns.
...
**Example**
A typical example of several datasets from a SAXS measurement (real data of a polymer in solution).

The first model can still be improved to include the chain stiffness within the wormlikeChain model.
In a Kratky plot (YX² vs. X) the high Q increase is observed.
One may reduce the Q range to lower Q where the chain stiffness is not visible.
...
- You may run into a local minimum which also depends on the noise in your data.
  Try :
    ...
    - For 'lm' e.g increase step width e.g. ``diff_step=0.01`` (like jumping over the noise/local mininma)
