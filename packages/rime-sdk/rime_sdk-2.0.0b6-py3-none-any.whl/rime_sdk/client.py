...
class Client:
    ...
    def create_project(
        ...
        general_access_permission: Optional[str] = "None",
    ) -> Project:
        ...
        Args:
            ...
            profiling_config: Optional[dict]
                ...
                runs.
            general_access_permission: Optional[str],
                Project permission assigned to the workspace members.
                Possible Values: View, Edit, None.
        ...
        """
        api = swagger_client.ProjectServiceApi(self._api_client)
        project_actor_role = RimeActorRole.NONE
        if general_access_permission == "View":
            project_actor_role = RimeActorRole.VIEWER
        elif general_access_permission == "Edit":
            project_actor_role = RimeActorRole.USER
        elif general_access_permission != "None":
            raise ValueError(
                "optional parameter general_access_permission"
                " can only be set to one of View, Edit or None"
            )

        with RESTErrorHandler():
            model_task_enum = _get_model_task_swagger(model_task)
            swagger_profiling = (
            ...
            if project_actor_role != RimeActorRole.NONE:
                role_pairs = []
                parent_roles = [
                    RimeActorRole.ADMIN,
                    RimeActorRole.VP,
                    RimeActorRole.USER,
                    RimeActorRole.VIEWER,
                ]
                for parent_role in parent_roles:
                    role_pairs.append(
                        RimeParentRoleSubjectRolePair(
                            parent_role=parent_role, subject_role=project_actor_role,
                        )
                    )
    ...
    def create_managed_image(
        ...
        Args:
            ...
            package_requirements: Optional[List[ManagedImagePackageRequirement]]
                ...
                Create a ``ManagedImage.PackageRequirement`` parameter with
    ...
    def start_stress_test(
        ...
        """Start a RIME model Stress Test.
