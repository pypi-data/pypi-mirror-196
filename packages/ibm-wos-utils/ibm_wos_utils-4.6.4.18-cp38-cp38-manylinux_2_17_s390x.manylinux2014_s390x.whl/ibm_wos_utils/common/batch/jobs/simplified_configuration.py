        ...
        if self.enable_explainability:
            ...
            with tarfile.open(fileobj=explainability_perturbations_filename_archive, mode="w:gz") as perturbation_tar:
                if config_obj.explain_perturbation_df is not None:
                    explain_perturbation = content = BytesIO(
                        config_obj.explain_perturbation_df.to_csv(index=False).encode("utf8"))
            ...
            explainability_perturbations_archive_path = self.output_file_path + \
                "/explainability_perturbations.tar.gz"
