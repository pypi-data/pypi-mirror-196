...
class FactorReader():
    ...
    def read_factor(
        ...
        @retry(stop=stop_after_attempt(100),wait=wait_fixed(3))
        def read_in(start_date,end_date):
        ...
        if cuts is None:
            return read_in(start_date=start_date,end_date=end_date)
        else:
            print('您读取的数据过长，正在分段读取，请稍候……')
            ress=[]
            for start_in,end_in in tqdm.tqdm(list(zip(cuts[:-1],cuts[1:]))):
                ress.append(read_in(start_in,end_in))
            print('读取完成，正在拼接，请稍等')
            res=pd.concat(ress)
            return res
