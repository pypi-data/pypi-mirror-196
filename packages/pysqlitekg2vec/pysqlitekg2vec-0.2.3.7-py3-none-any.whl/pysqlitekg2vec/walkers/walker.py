...
class Walker(ABC):
    ...
    def extract(
        ...
        with alive_bar(len(entities), title='Generate walks') as bar:

            def read_results():
                while not (done.value and r_queue.empty()):
                    res = r_queue.get()
                    for entity_id in res.keys():
                        importer.add_walks(entity_id, res[entity_id])
                        bar()

            t = Thread(target=read_results)
            t.start()
            ...
            with multiprocessing.Pool(process, self._init_worker,
                ...
                for _ in pool.map(self._proc, splits):
                    continue
    ...
    def _proc(self, entities: Sequence[str]) -> None:
        ...
        for entity in entities:
            result_queue.put(self._extract(kg, Vertex(entity)))  # type: ignore
