...
from pysqlitekg2vec.embedders import Embedder
from pysqlitekg2vec.typings import Embeddings, Entities, SWalk
from pysqlitekg2vec.walkers.vault import CorpusVault
...
class Word2Vec(Embedder):
    ...
    def fit(
        self, vault: CorpusVault, is_update: bool = False
    ) -> Embedder:
        """Fits the Word2Vec model based on provided walks.

        Args:
            vault: The corpus vault with all the generated walks.
        ...
        """
#        corpus = [walk for entity_walks in vault.walks() for
#                  walk in entity_walks]
        self._model.build_vocab(vault, update=is_update)
        self._model.train(
            vault,
