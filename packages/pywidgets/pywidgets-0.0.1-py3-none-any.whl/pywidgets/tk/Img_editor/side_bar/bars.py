...
class Frame_erode(Slider_1_Reset):
    def __init__(self,app,mask,target,range=(1,30),**kwargs):
        ...
        self.mask=mask
        self.target=target
        self.var.trace_add("write",lambda f,s,t:self._minimizer())
    def _minimizer(self):
        self.erode(self.var.get())
    def erode(self,num):
        mask=self.mask.copy()
        kernel=np.ones((num,num))
        result=cv2.erode(self.mask,kernel)
        self.target(result)
        self.mask=mask.copy()
        
class Frame_dilate(Slider_1_Reset):
    def __init__(self,app,mask,target,range=(1,30),**kwargs):
        super().__init__(app,1,range,**kwargs)
        self.mask=mask
        self.target=target
        self.var.trace_add("write",lambda f,s,t:self._minimizer())
    def _minimizer(self):
        self.dilate(self.var.get())
    def dilate(self,num):
        mask=self.mask.copy()
        kernel=np.ones((num,num))
        result=cv2.dilate(self.mask,kernel)
        self.target(result)
        self.mask=mask.copy()
...
class Thresholding_Frame(Frame):
    def __init__(self,app,src,target,default_type=cv2.THRESH_BINARY,*args, **kwargs):
        ...
        self.value_scale._minimizer=self._minimizer
    def _minimizer(self):
    ...
    def thresh(self,thresh,type:int=None):
        ...
        thresh_img=cv2.threshold(self.gray,thresh,255,type)[1]
        self.target(thresh_img)
        return thresh_img

class Adaptive_Thresh_Frame(Thresholding_Frame): 
    def __init__(self, app, src, target, default_type=cv2.THRESH_BINARY, *args, **kwargs):
        super().__init__(app, src, target, default_type=default_type, *args, **kwargs)
    ...
    def adaptive_threshold(self, thresh,type:int=None):
        ...
        thresh_img=cv2.adaptiveThreshold(self.gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,type_var,11,thresh)
        self.target(thresh_img)
        return thresh_img
...
class Frame_Canny(Slider_2): 
    ...
    def canny(self, thresh, maxthresh):
        thresh_img=cv2.Canny(self.gray,thresh,maxthresh)
        self.target(thresh_img)
        return thresh_img
...
class Bars_BGR(Slider_3x2):
    def __init__(self, app,imgcv,target, orient=..., range: tuple = ..., **kwargs):
        ...
        for scale in [self.side_1,self.side_1,self.side_1]:
            def minimizer():
                Slider_2._minimizer(scale)
                self.mask_creator()
            scale._minimizer=minimizer
