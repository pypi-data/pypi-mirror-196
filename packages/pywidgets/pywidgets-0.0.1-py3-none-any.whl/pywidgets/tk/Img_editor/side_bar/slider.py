...
class Scale_reset(Scale):
    def __init__(self,app=None,default_value=0,variable:IntVar=...,**kwargs):
        ...
        def tracker_add(mode,funct):
            def track(*args,**kwargs):
                if self.variable.get()!=default_value:
                    funct(*args,**kwargs)
...
class Slider_1_Reset(Frame):
    def __init__(self,app=None,default_number=0,range:tuple=(0,100),orient=HORIZONTAL,**kwargs):
        super().__init__(app,**kwargs)
        self.range=range=dict(from_=range[0],to=range[1])
        self.var=IntVar()
        self.scale=Scale_reset(self,default_number,variable=self.var,orient=orient,**range)
        self.scale.pack(fill=X,expand=YES)
...
class Slider_2(Frame):
    def __init__(self,app=None,range: tuple=(0,100),orient=HORIZONTAL,**kwargs):
        ...
        self.var_1.trace_add("write",lambda f,s,t:self._minimizer())
        self.var_2.trace_add("write",lambda f,s,t:self._minimizer())
    def _minimizer(self):
...
class Slider_3x2(Frame):
    def __init__(self,app=None,orient=HORIZONTAL,range:tuple=(0,255),**kwargs):
        ...
        self.mainNoteBook=range=dict(from_=range[0],to=range[1])
    def values(self):
        values=[]
...
class SwitchBox(Frame):
    def __init__(self,app,target,default_state=True,orient=VERTICAL,*args,**kwargs):
        super().__init__(app,*args,**kwargs)
        self.target=target
        self._laststate=default_state
        ...
        self._buttons_state(default_state)
