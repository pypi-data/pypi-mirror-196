...
from pywidgets.tk.Img_editor.tecnologies.brushing import Switcher_mask,Filling_mask,Brush_with_Mask
from pywidgets.tk.Img_editor.tecnologies.selecting_object import Adding_imgbackground_edges,Control_Select_region


class buttons_detectors(EMViewer):    
...
class Add_background(Background_added):
    ...
    def __init__(self, app=None, imgcv=None, mask=None, points=None,radius_selcting=10,centerstate=True, **kwargs):
        ...
        self.con_back_box=Select_box(self.points,None,radius_selecting=radius_selcting,centerState=centerstate)
    ...
    def multi_resize_background(self):
        ...
        def define_image(boxs=None):
            ...
            for id,box in enumerate(boxs):
                ...
                the_result_image=draw_box_moving(the_result_image,new_circles)
...
class COLORING_Brush(Viewer_coloredbackground,Coloring_brsuh):
    ...
    def coloring_mode(self):
        ...
        def show_viewer(mask,coloerbackground):
            self.show_viewers(False,mask=mask,coloerbackground=coloerbackground)
        self.target=show_viewer
        show_viewer(self.mask,self.colorBackground)
        self.bind_brush_coloring()

class Text_putter(EMViewer,_Text_putter):
