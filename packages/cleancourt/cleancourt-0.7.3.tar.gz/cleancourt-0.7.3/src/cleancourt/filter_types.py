...
def filter_companies(name):
    ...
    try:
        return ((pp.tag(name)[1] != 'Person') or ("|and|" in name) or (name.count(" ") == 0) or (re.search(' bank$', name)) or ("bennington crossing" in name)) and not name[0].isdigit() 
    except:
        return True
...
def filter_people(name):
    ...
    try:
        val = pp.tag(name)
        return (val[1] == 'Person') and ("|and|" not in name) and (name.count(" ") != 0) and not (re.search(' bank$', name)) and ("bennington crossing" not in name)
    except:
        return False