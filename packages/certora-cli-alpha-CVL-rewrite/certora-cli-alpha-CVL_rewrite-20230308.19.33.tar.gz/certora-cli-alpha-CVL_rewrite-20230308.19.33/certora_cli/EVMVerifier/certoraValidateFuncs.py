...
def validate_sanity_value(value: str) -> str:
    legal_values = [e.name.lower() for e in RuleSanityValue]
    if not value.lower() in legal_values:
        raise Util.CertoraUserInputError(
            f"sanity rule value {value} should be one of the following {legal_values}")
    return value
...
def validate_multi_example_value(value: str) -> str:
    ...
    if not value.lower() in legal_values:
        raise Util.CertoraUserInputError(
            f"multi-example value {value} should be one of the following {legal_values}")
    return value
