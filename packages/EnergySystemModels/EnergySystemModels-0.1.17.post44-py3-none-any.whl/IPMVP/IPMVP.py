...
def Mathematical_Models(y,X,start_baseline_period,end_baseline_period,start_reporting_period,end_reporting_period):
    ...
    coefficients=', '.join(str(nombre) for nombre in coefficients)
    ...
    print(serr)
    const_coef=(pd.DataFrame([[const,coefficients]],columns=['const',', '.join(str(col_name) for col_name in  X_bl.columns)]).rename(index={0: 'ANTE-POST'})).astype(float)
    print(const_coef)
    ...
    df=pd.DataFrame([[coefficients,const,r2,rmse,cv_rmse,ddof]], columns=[', '.join(str(col_name) for col_name in  X_bl.columns),'const',"r2","rmse","cv_rmse","ddof"], index=["ANTE-POST"])
