...
class MltsConan(ConanFile):
    ...
    def source(self):
        self.run('git clone git@gitlab.gz.cvte.cn:1602/rtcbase/mlts.git -b v5 .')
    ...
    def package_info(self):
        if self.options.shared:
            if(self.settings.compiler == 'Visual Studio'):
                self.cpp_info.libs = ['MltsTransportShared.lib']
            else:
                self.cpp_info.libs = ['MltsTransportShared']
        else:
            if(self.settings.compiler == 'Visual Studio'):
                self.cpp_info.libs = ['MltsTransportStatic.lib', 'MltsUtilStatic.lib', 'nattools.lib']
            else:
                self.cpp_info.libs = ['MltsTransportStatic', 'MltsUtilStatic', 'nattools']