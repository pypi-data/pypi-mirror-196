...
from ...util.helpers import hash_password, has_permission_any
...
@api.route('/users/<username>/roles/<role_name>', endpoint='userrole')
class UserRoleAPI(Resource):
    @auth_required('session', 'basic')
    @permissions_accepted('roles_manage')
    @api.response(200, "Success")
    @api.response(404, "User or Role not found")
    def put(self, username, role_name):
        role = models.Role.query.filter(models.Role.name == role_name).one_or_none()
        ...
        return {"role": role.name, "user": user.username, "has_role": user in role.users}

    @auth_required('session', 'basic')
    @permissions_accepted('roles_manage')
    @api.response(200, "Success")
    @api.response(404, "User or Role not found")
    def delete(self, username, role_name):
        role = models.Role.query.filter(models.Role.name == role_name).one_or_none()
        user = models.User.query.filter(models.User.username == username).one_or_none()
