...
from ...fields import ObjectUrl, OptionalDatetime
from ...util.helpers import get_object_from_arg, http_auth_optional

db = models.db


callback_list = api.model('CallbackList', {
    'callbacks': fields.List(ObjectUrl('api_v1.callback', attribute='id'))
})


callback_get = api.model('CallbackGet', {
    'uri': fields.Url('api_v1.callback'),
    'state': ObjectUrl('api_v1.state', attribute='state_id'),
    'label': fields.String,
    'executions': fields.List(ObjectUrl('api_v1.callback_execution', attribute='id')),
...
class CallbackListAPI(Resource):
    ...
    def get(self):
        callbacks = models.Callback.query.all()
        return {"callbacks": callbacks}


@api.route('/callbacks/<id>', endpoint='callback')
class CallbackListAPI(Resource):
    ...
    def get(self, id):
        callback = get_object_from_arg(id, models.Callback)
        return callback
...
callback_execution_get = api.model('CallbackExecutionGet', {
    'uri': ObjectUrl('api_v1.callback_execution', attribute='id'),
...
@api.route('/callback_executions/<id>', endpoint='callback_execution')
class CallbackExecutionAPI(Resource):
    ...
    def get(self, id):
        callback_execution = get_object_from_arg(id, models.CallbackExecution)
