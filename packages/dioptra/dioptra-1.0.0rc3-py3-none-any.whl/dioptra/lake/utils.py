...
def upload_to_lake_via_object_store(records, custom_path=''):
    """
    Uploading metadata to the data lake via an s3 bucket.
    Should be the prefered uplaod method for large records
    ...
    return upload_to_lake_from_s3(object_store_bucket, file_name, storage_type)
...
def _format_prediction(logits, pred_type, class_names=None):
    ...
    Parameters:
        ...
        pred_type: the type of gt. Supported types CLASSIFIER, SEMANTIC_SEGMENTATION
        class_names: a list of class names. If the groundtruth contains indexes, it will be used to convert them to names

    """
    if pred_type in ['CLASSIFIER', 'SEMANTIC_SEGMENTATION']:
        if isinstance(logits, np.array):
