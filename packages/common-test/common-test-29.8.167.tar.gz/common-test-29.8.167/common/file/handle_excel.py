...
from tools.handle_path import  adjust_path
from common.data.handle_common import get_system_key, set_system_key, format_caseName,print_info
...
def check_excel_data(testdata:dict, _mysql, _filter):
    ...
    if DataProcess.isNotNull(get_system_key(Constant.TEST_SRTCYCLE_ID)) and _mysql is not None:
        ...
        _sql = f"select * from `traffic_test`.`test_autotest_run` where `status` = '{Constant.STATUS_AUTOTEST}' and cycleId='{cycleId}' and casename='{casename}'"
        ...
        if len(_info) > 0:
            ...
            _sql1 = f"select * from `traffic_test`.`test_autotest_run` where `status` = '{Constant.STATUS_AUTOTEST}' and cycleId='{cycleId}' and caseid='{caseNo}'"
            _info1 = _mysql.execute(_sql).fetchall()
            if len(_info1) > 0:
                return True
            else:
                logger.warning(f'参数化用例:{casename} 执行周期:{cycleId} 未发现')
                return False
...
def write_to_excel(excel_name, sheet_name, row, col, content):
    ...
    # 打开xls文件
    _rb = xlrd.open_workbook(file_path, formatting_info=True)
    #_wb = copy(_rb)
    # 获取指定sheet页对象
    _sheet = _rb.sheet_by_name(sheet_name)
    # 写入内容
    _sheet.write(row, col, content)
    # 保存
    _wb.save(file_path)
