...
class RotatingLogReader:
    ...
    def GetRelatedLogFilePaths(self) -> list[str]:
        ...
        """
        import re
        import src.SimpleWorkspace.IO.Directory
        import src.SimpleWorkspace.Utility.Regex
        ...
        def logFilesFiltering(path: str):
            ...
            match = src.SimpleWorkspace.Utility.Regex.Match(f"/^{escLogName}\.(\d+)/", currentLogname)[0]
            logFiles[int(match[1])] = path
            return
        src.SimpleWorkspace.IO.Directory.List(self.logBaseDirectory, callback=logFilesFiltering, includeDirs=False, maxRecursionDepth=1)
