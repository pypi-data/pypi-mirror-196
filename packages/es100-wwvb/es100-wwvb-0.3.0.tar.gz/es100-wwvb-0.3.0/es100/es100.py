...
class ES100:
    ...
    def __init__(self, en=None, irq=None, bus=None, address=None, debug=False, verbose=False):
        """ __init """
    ...
    def wait_till_55seconds(self):
        ...
        self._log.info('sleeping %.1f seconds till :55 point', remaining_seconds)
    ...
    def es100_receive(self, antenna=None, tracking=False):
        ...
        # initialize gpio's (this should be already done)
        # self._disable()
        # time.sleep(ES100.ENABLE_DELAY_SEC)
        ...
        while True:
            # read status0 and interrupt status
            ...
            rx_complete = bool(self._irq_status & ES100.IRQSTATUS.RX_COMPLETE)
            cycle_complete = bool(self._irq_status & ES100.IRQSTATUS.CYCLE_COMPLETE)

            self._log.info('irq_status = 0x%02x <-,-,-,-,-,%s,-,%s> | status0 = 0x%02x <%s,...,%s,-,%s>',
                                self._irq_status,
                                'CYCLE_COMPLETE' if cycle_complete else '-',
                                'RX_COMPLETE' if rx_complete else '-',
                                self._status0,
            ...
            self._irq_wait()
            # save away the current time use in case - i.e. time of decoded reception
            self._sys_time_received = datetime.utcnow().replace(tzinfo=timezone.utc)
            # round down to milliseconds
            # WWVB is accurate; but our reception isn't down to the microsecond ('cause linux)
            msec = int(self._sys_time_received.microsecond/1000.0)
            self._sys_time_received.replace(microsecond=msec*1000)
    ...
    def time(self, antenna=None, tracking=False):
        ...
        try:
            ...
            self.es100_receive(antenna, tracking)
        except ES100Error as err:
            self._log.warning('read/receive failed: %s', err)
            return None

        self._log.debug('Status register = 0x%02x', self._status0)
        ...
        if self._status0 & ES100.STATUS0.TRACKING:
            ...
            self._log.info('tracking operation successful, second=%02d at second=%02d.%03d, Antenna%d',
                                ES100._bcd(self._recv_time['SECOND'] & 0x3f),
                                self._sys_time_received.second,
                                int(self._sys_time_received.microsecond / 1000),
                                rx_antenna
                            )
            ...
            self._wwvb_time_received = datetime(
                                    1, 1, 1,
                                    0, 0, ES100._bcd(self._recv_time['SECOND'] & 0x3f),
                                    microsecond=0,
                                    tzinfo=timezone.utc
                            )
        ...
        self._log.info('reception of %s at system time %s, Antenna%d',
                                self._wwvb_time_received,
                                self._sys_time_received,
                                rx_antenna
                        )
