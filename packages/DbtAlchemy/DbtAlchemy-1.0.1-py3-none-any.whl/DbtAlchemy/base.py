# +--------------------------------------------------------------------------+
# |  Licensed Materials - Property of IBM & OpenInformix & GBase             |
# |                                                                          |
...
from . import reflection as dbt_reflection
...
class _DBT_Boolean(sa_types.Boolean):
...
colspecs = {
    sa_types.Boolean: _DBT_Boolean,
    sa_types.Date: _DBT_Date
}
...
class DbtTypeCompiler(compiler.GenericTypeCompiler):
...
class DbtCompiler(compiler.SQLCompiler):
    ...
    # TODO: this is wrong but need to know what GBase 8s is expecting here
    ...
    def visit_cast(self, cast, **kw):
        ...
        if isinstance(type_, (
                    ...
                    sa_types.DECIMAL)):
            return super(DbtCompiler, self).visit_cast(cast, **kw)
        else:
    ...
    def visit_unary(self, unary, **kw):
        if (unary.operator == operators.exists)  and kw.get('within_columns_clause', False):
            usql = super(DbtCompiler, self).visit_unary(unary, **kw)
            usql = "CASE WHEN " + usql + " THEN 1 ELSE 0 END"
            return usql
        else:
            return super(DbtCompiler, self).visit_unary(unary, **kw)

class DbtDDLCompiler(compiler.DDLCompiler):

    def get_server_version_info(self, dialect):
        """Returns the GBase 8s major and minor version as a list of ints."""
    ...
    def _is_nullable_unique_constraint_supported(self, dialect):
        """Checks to see if the GBase 8s version. 
        ...
        if hasattr(dialect, 'dbms_name'):
           if dbms_name != None and (dbms_name.find('GBase/') != -1):
    ...
    def define_constraint_cascades(self, constraint):
        ...
        if constraint.onupdate is not None:
            util.warn(
                "GBase 8s does not support UPDATE CASCADE for foreign keys.")
    ...
    def create_table_constraints(self, table, **kw):
        ...
        result = super( DbtDDLCompiler, self ).create_table_constraints(table, **kw)
        return result

    def visit_create_index(self, create, include_schema=True, include_table_schema=True):
        if SA_Version < [0, 8]:
            sql = super( DbtDDLCompiler, self ).visit_create_index(create)
        else:
            sql = super( DbtDDLCompiler, self ).visit_create_index(create, include_schema, include_table_schema)
        ...
        if self._is_nullable_unique_constraint_supported(self.dialect):
            if isinstance(create.element, sa_schema.UniqueConstraint):
                ...
                if getattr(create.element, 'uConstraint_as_index', None):
                    ...
                    return sql
        sql = super( DbtDDLCompiler, self ).visit_add_constraint(create)
        return sql

class DbtIdentifierPreparer(compiler.IdentifierPreparer):

    reserved_words = RESERVED_WORDS
    illegal_initial_characters = set(range(0, 10)).union(["_", "$"])


class DbtExecutionContext(default.DefaultExecutionContext):
    ...
    def post_exec(self):
        ...
        if self._select_lastrowid:
            conn._cursor_execute(self.cursor,
                    "SELECT IDENTITY_VAL_LOCAL() FROM DUAL",
                    (), self)
...
class DbtDialect(default.DefaultDialect):
    ...
    statement_compiler = DbtCompiler
    ddl_compiler = DbtDDLCompiler
    type_compiler = DbtTypeCompiler
    preparer = DbtIdentifierPreparer
    execution_ctx_cls = DbtExecutionContext

    _reflector_cls = dbt_reflection.DbtReflector

    def __init__(self, **kw):
        super(DbtDialect, self).__init__(**kw)

        self._reflector = self._reflector_cls(self)

    # reflection: these all defer to an BaseDbtReflector
    # object which selects between Informix and AS/400 schemas
    def initialize(self, connection):
        super(DbtDialect, self).initialize(connection)
...
# legacy naming
DBT_DBCompiler = DbtCompiler
DBT_DBDDLCompiler = DbtDDLCompiler
DBT_DBIdentifierPreparer = DbtIdentifierPreparer
DBT_DBExecutionContext = DbtExecutionContext
DBT_DBDialect = DbtDialect
