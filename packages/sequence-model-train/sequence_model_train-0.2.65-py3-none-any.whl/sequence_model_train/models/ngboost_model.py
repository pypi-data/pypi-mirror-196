...
class NGBoostModel:
    ...
    def fit(self, X, y):
        ...
        print('x shape: ', X[columns_].shape)
        print('y shape: ', X[self.columns].shape)
        ...
        for i in range(self.n_out):
            if (i+1-self.n_out == 0):
                print(y[columns_].values[-1:, :].shape)
                pred_y.append(
                    self.models[i].predict(y[columns_].values[-1:, :])[0]
                )
            else:
                print(y[columns_].values[i-self.n_out:i+1-self.n_out, :].shape)
                pred_y.append(
        ...
        print(pred_y)
        print(y[self.columns].values[0:1, :].shape)
        print(y[self.columns])
