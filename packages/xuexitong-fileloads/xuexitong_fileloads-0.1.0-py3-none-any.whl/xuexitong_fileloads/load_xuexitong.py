...
def get_chapter_link(path_file):
    with open(path_file, 'r', encoding='utf-8') as fp:
        return json.load(fp)

def load_loop(chapter_link_list:list,start_chapter=0, end_chapter=0, file_class=1,start_inPage = 0,end_inPage = 1):
    if start_chapter<0:
        start_chapter=0
    if end_chapter <= 0:
    ...
    for url in chapter_link_list[start_chapter,end_chapter]:
        ...
        time.sleep(0.5)
        try:
            get_img_pdf(basedir, url, start_inPage=start_inPage, end_inPage=end_inPage, file_class=file_class)
        except Exception as result:
...
def cookies_login(url,exe=True):
    """
    Cookies登录方法
        ...
        try:
            ...
            if '2' in loop_op:
                if '9' in mode_op:
                    ...
                    start_chapter = config_dict['start_chapter']
                    end_chapter = config_dict['end_chapter']
                print('获取各章链接中...')
                get_all_chapter_link(basedir, url)
                chapter_link_list = get_chapter_link(basedir + 'chapter_link.json')
                load_loop(chapter_link_list,start_chapter=start_chapter, end=end_chapter,file_class=file_class)
            else:
                isNext = '1'
                while True:
                    get_img_pdf(basedir, url, start_inPage=start_inPage, end_inPage=end_inPage-1,file_class=file_class)
