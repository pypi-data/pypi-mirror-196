...
def test_install_one_tool():
    ...
    try:
        expected_non_admin_error = 'Non-administrators may only install Add-on Tools in Development Mode'
        with pytest.raises(Exception, match=expected_non_admin_error):
        ...
        expected_non_admin_error = 'does not have MANAGE access for the AddOnTool'
        with pytest.raises(Exception, match=expected_non_admin_error):
