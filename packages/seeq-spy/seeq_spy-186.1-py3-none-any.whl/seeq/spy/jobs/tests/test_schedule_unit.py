...
def setup_mocks():
    ...
    _datalab.get_notebook_path = mock.Mock(return_value='notebook.ipynb')
    ...
    spy._session.SqAuthRequests.get = mock.Mock(return_value=mock_get_resp)
    mock_post_resp = requests.Response()
    mock_post_resp.json = mock.Mock(return_value={'path': 'ogenicity'})
    spy._session.SqAuthRequests.post = mock.Mock(return_value=mock_post_resp)
    mock_patch_resp = requests.Response()
    mock_patch_resp.json = mock.Mock(return_value={'path': 'ological'})
    mock_patch_resp.status_code = 200
    spy._session.SqAuthRequests.patch = mock.Mock(return_value=mock_patch_resp)
    _schedule.pickle_df_to_path = mock.Mock()
    put_pickle_resp = mock.Mock()
    put_pickle_resp.status_code = 200
    spy._session.SqAuthRequests.put = mock.Mock(return_value=put_pickle_resp)
