...
# /boot/efi --fstype="efi" --ondisk=sda --size=126 --fsoptions="umask=0077,shortname=winnt" --label=boot_efi
    ...
    def save(self, *args, **kwargs):
        ...
        print(f"Partition Count: {self.cleaned_data['drive'].partition_count}")
        # drive = Drive.objects.create()

        linux_device = LinuxDevice.objects.create(device='/dev/sda1', type='efi')
        # partition = Partition.objects.create(drive=self.cleaned_data['drive'].id, device=linux_device, size=self.cleaned_data['size'], type='vfat')
