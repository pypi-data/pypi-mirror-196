...
class LVMTemplateForm(NetBoxModelForm):
    ...
    def save(self, *args, **kwargs):
        ...
        # mounted_volume = MountedVolume.objects.create(mount_point=self.cleaned_data['mountpoint'],
        #                                               fs_type=self.cleaned_data['fs_type'],
        #                                               options=self.cleaned_data['fs_options'])
        # mounted_volume.save()
        mounted_volume = super().save(*args, **kwargs)
        print(f"Mounted Volume ID: {mounted_volume.id}")

        linux_device = LinuxDevice.objects.create(device=device_name, type='efi')
        linux_device.save()
        print(f"Linux Device ID: {linux_device.id}")

        partition = Partition.objects.create(drive=self.cleaned_data['drive'], device=linux_device,
                                             size=self.cleaned_data['size'], type='Linux LVM')
        partition.save()
        print(f"Partition ID: {partition.id}")

        volume_group = VolumeGroup.objects.create(vg_name=self.cleaned_data['vg_name'])
        volume_group.save()
        print(f"Volume Group ID: {volume_group.id}")

        pv = PhysicalVolume.objects.create(device=linux_device, vg=volume_group)
        pv.save()
        print(f"Physical Volume ID: {pv.id}")

        # Add linux_device for lvm
        linux_device_mapper = LinuxDevice.objects.create(device='/dev/mapper/vg_system-lv_home',
                                                         type='efi')
        linux_device_mapper.save()
        print(f"Linux Device ID: {linux_device_mapper.id}")

        lv = LogicalVolume.objects.create(vg=volume_group, lv_name=self.cleaned_data['lv_name'],
                                          size=self.cleaned_data['size'], device=linux_device_mapper)
        lv.save()
        print(f"{lv}")
