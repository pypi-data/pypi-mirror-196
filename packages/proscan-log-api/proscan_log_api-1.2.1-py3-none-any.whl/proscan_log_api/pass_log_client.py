            ...
            with mmap.mmap(
                ...
                for event in events:
                    ...
                    if post_event_list:
                        ...
                        first_event_dt = (
                            datetime.fromisoformat(pre_event_list[0]["datetime"])
                            if pre_event_list
                            else datetime.fromisoformat(event["datetime"])
                        )
                        last_event_dt = datetime.fromisoformat(
                            post_event_list[-1]["datetime"]
                        )

                    if (
                        first_event_dt
                        and first_event_dt > start_datetime.replace(tzinfo=None)
                        and last_event_dt
                        and last_event_dt < end_datetime.replace(tzinfo=None)
                    ) or timestamp:
