...
from . import get_seed, metrics
from .base import LearnersPool
from .hpo import find_best_params
from .metrics import normalized_absolute_error
...
class RegressorsPool(LearnersPool):
    ...
    def score(self, metric: str, y_true: np.ndarray, y_pred: np.ndarray):
        z = self._call_function(module=metrics, name=metric, y_true=y_true, y_pred=y_pred)
        return z, self._estimate_proba(y_true=y_true, y_pred=y_pred)

    @staticmethod
    def _estimate_proba(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
        norm_abs_err = normalized_absolute_error(y_true, y_pred)
        return np.exp(-norm_abs_err)
    ...
    def run(
        ...
        Args:
            algo (str): regressor (standard scikit-learn regressor class)
            metric (str): classification performance metric. Either `absolute_error` (default) or `absolute_error`
            ...
            **kwargs: fixed regressor parameters, which won't be optimized
    ...
    def run_all(
        ...
        """
        Wrapper function that runs all the regressors in the dataset.

        Args:
            metric (str): performance metric (default `absolute_error`)
            n_folds (int): number of cross-validation folds for evaluating algorithm performance
            n_iter (int): number of times the cross-validation is repeated. Instance metric is the mean over the
                iterations
            ...
            hpo_evals (int): maximum number of evaluations
            hpo_timeout (int): timeout (seconds) for a single regressor HPO
            verbose (bool): turn verbose mode on

        Returns:
            pd.DataFrame: An `(n_instances, n_regressors)` dataframe with regressors performance by instance
        """
