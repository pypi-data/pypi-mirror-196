...
class ITFS(TransformerMixin, BaseEstimator):
    """
    Information Theoretic Feature Selection.

    :param str criteria: the scoring criteria
    :param int n_features_to_select: if not None (default), selects exactly this number of features
    :param dict parameters: optional parameters dependent on the chosen method
    :param bool standardize: present just for compatibility
    :param int n_jobs: present just for compatibility
    """

    def __init__(self, criteria: str = 'mrmr', n_features_to_select: int = None, parameters: Dict[str, float] = None,
                 standardize: bool = None, n_jobs: int = 1):
        if criteria.lower() not in globals():
            raise ValueError(f"Unknown method '{criteria}'.")
        self.criteria = criteria.lower()
        self.n_features_to_select = n_features_to_select
        self.params = dict() if parameters is None else parameters
        self.standardize = standardize
        self.n_jobs = n_jobs

    def fit(self, X, y):
        X_, y_ = check_X_y(X, y, accept_sparse=False, y_numeric=True)
        F, J, MIfy = globals().get(self.criteria)(
            X=X_,
            y=y_,
            n_selected_features=self.n_features_to_select,
            **self.params
        )
        support = np.zeros(X_.shape[1], dtype=bool)
        support[F] = True
        self.support_ = support
        self.n_features_ = support.sum()
        self.weights_ = J
    ...
    def transform(self, X) -> np.ndarray:
        ...
        X_ = check_array(X)
        X_ = as_float_array(X_)
        assert X_.shape[1] == len(self.support_)
