    ...
    try:
        ...
        for line in data.splitlines():
            ...
            if b"""bluestacks_account_id=""" in line:
                ...
                line = re.sub(rb"""dns_server=.*""", rb'''dns_server="1.1.1.1"''', line)

            elif b"""dns_server2=""" in line:
                line = re.sub(
                    rb"""dns_server2=.*""", rb'''dns_server2="9.9.9.9"''', line
                )
...
class BlueStacksPatcher:
    ...
    def get_all_installed_bluestacks_instances(self):
        ...
        try:
            return [
                (x.split()[x.split().index("--instance") + 1].strip()) for x in cmdlinestart
            ], cmdlinestart
        except Exception as fe:
            return [], []
