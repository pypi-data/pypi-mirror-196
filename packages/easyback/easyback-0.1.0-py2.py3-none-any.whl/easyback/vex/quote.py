...
# from easyback.context import Context
# 数据类型  
import easyback.context as cs


def convert_datetime_column(x: object):
    '''https://www.cnblogs.com/treasury-manager/p/13885947.html'''
    if isinstance(x, str):
        x = x.replace('-', '') + '000000'
    return datetime.datetime.strptime(x,"%Y%m%d%H%M%S")  #字符串转化为date形式
