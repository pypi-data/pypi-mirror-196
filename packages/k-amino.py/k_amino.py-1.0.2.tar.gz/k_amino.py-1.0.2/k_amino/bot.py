...
class Bot(Events, AsyncEvents):
    def __init__(self, prefix: str, is_async: bool=False):
        if is_async:
            AsyncEvents.__init__(self, prefix=prefix, str_only=True, first_parameter_object=True)
        else:
    ...
    async def build_parameters(self, obj_data):
        data = Parameters(data=obj_data.message.content, prefix=self.prefix)
