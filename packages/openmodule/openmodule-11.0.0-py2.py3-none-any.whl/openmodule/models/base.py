...
class ZMQMessage(OpenModuleModel):
    ...
    def publish_on_topic(self, pub_socket: zmq.Socket, topic: str):
        ...
        if isinstance(topic, bytes):
            warnings.warn('\n\nPassing topic as bytes is deprecated. Please pass as strings.\n',
...
class GateType(str, Enum):
    ...
    def is_outward(self):
        return self.value in [GateType.exit, GateType.bidirectional]


class Gate(OpenModuleModel):
    gate: str
    name: str = ""
    type: GateType = GateType.unknown
...
class CostEntryData(OpenModuleModel):
    ...
    def set_value_type(cls, values):
        if values["entry_type"] in ["rate_change", "rate_reduction", "partial_rate_change"]:
