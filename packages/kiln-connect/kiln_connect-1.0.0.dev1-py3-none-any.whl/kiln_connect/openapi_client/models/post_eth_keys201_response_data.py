...
POSTETHKEYS201RESPONSEDATA_ONE_OF_SCHEMAS = ["EthereumPostKeysBatchResponse", "List[EthereumPostKeysCliResponseInner]"]
...
class PostEthKeys201ResponseData(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        error_messages = []
        match = 0
        # validate data type: List[EthereumPostKeysCliResponseInner]
        if type(v) is not List[EthereumPostKeysCliResponseInner]:
            error_messages.append(f"Error! Input type `{type(v)}` is not `List[EthereumPostKeysCliResponseInner]`")
        else:
            match += 1

        # validate data type: EthereumPostKeysBatchResponse
        if type(v) is not EthereumPostKeysBatchResponse:
            error_messages.append(f"Error! Input type `{type(v)}` is not `EthereumPostKeysBatchResponse`")
        else:
            match += 1
    ...
    def from_json(cls, json_str: str) -> PostEthKeys201ResponseData:
        ...
        # deserialize data into List[EthereumPostKeysCliResponseInner]
        try:
            instance.actual_instance = List[EthereumPostKeysCliResponseInner].from_json(json_str)
            match += 1
        except ValidationError as e:
            error_messages.append(str(e))
        # deserialize data into EthereumPostKeysBatchResponse
        try:
            instance.actual_instance = EthereumPostKeysBatchResponse.from_json(json_str)
            match += 1
        except ValidationError as e:
            error_messages.append(str(e))
