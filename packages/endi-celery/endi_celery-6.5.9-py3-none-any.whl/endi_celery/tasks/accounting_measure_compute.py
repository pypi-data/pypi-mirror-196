...
def collect_operations_by_fiscal_year(
    upload_id: int, upload_date: datetime.date, exclude_future_operations: bool = False
    ...
    """
    logger.info(
        "  + Collecting operations for {} (upload_id={})".format(
            upload_date.year, upload_id
        )
    )

    # Get current exercice dates
    (
        current_exercice_begining_date,
    ...
    first_operation_date = current_exercice_begining_date
    first_exercice = (
        DBSESSION().query(extract("year", func.min(AccountingOperation.date))).scalar()
    )
    previous_exercice = upload_date.year - 1
    for exercice in range(previous_exercice, first_exercice - 1, -1):
        ...
        if closure_is_done:
            ...
            first_operation_date = current_exercice_begining_date - relativedelta(
                years=(upload_date.year - exercice)
            )

    # Compute last operation date (exclude future operations if needed)
    last_operation_date = current_exercice_end_date
    if exclude_future_operations:
        last_operation_date = min(current_exercice_end_date, date.today())
...
def collect_operations(
    ...
    if compiler_type != "treasury":
        ...
        return collect_operations_by_fiscal_year(
            upload.id, upload.date, exclude_future_operations=True
        )
