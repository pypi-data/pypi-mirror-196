...
def gera_dif_arquivos_csv(
    ...
    tenham o mesmo formato. São gerados dois arquivos de saída: um arquivo com
    os registros que foram excluídos de um CSV para outro e outro com os
    registros que foram incluídos. Os registros que foram apenas alterados
    entre um CSV e outro aparecem tanto no arquivo de registros excluídos
    quanto no arquivo de registros incluídos.

    Args:
    arq_csv_antigo:     arquivo CSV com os dados antigos
    arq_csv_atualizado: arquivo CSV com os novos dados
    ...
    sort_command:       nome do programa sort que será utilizado, com seu path
                        ...
                        Se não informado, utiliza o sort que houver no path da
                        máquina
    Ret:
    Os dois nomes dos arquivos de diferença: primeiro o nome do arquivo com os
    registros incluídos e depois o nome do arquivo com os registros excluídos.
    Se não houver registros incluídos ou excluídos, o retorno correspondente é
    None.
    """
    ...
    os.system(  "diff " + arq_csv_antigo + " " + arq_csv_atualizado + 
                " > diff.txt")
