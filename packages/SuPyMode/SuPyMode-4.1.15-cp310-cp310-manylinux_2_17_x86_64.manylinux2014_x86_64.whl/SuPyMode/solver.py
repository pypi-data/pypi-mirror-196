...
# Local imports
from SuPyMode.symmetries import get_modes_label
from SuPyMode.superset import SuperSet
from SuPyMode.supermode import SuperMode
from SuPyMode.boundaries import Boundaries
...
class SuPySolver(object):
    ...
    def add_modes(self,
        ...
        # if auto_labeling:
        #     mode_label = get_modes_label(
        #         n_mode=cpp_solver.n_sorted_mode,
        #         boundaries=boundaries
        #     )
        # else:
        # mode_label = [str([self.solver_number, n]) for n in range(cpp_solver.n_sorted_mode)]
        # mode_label = ["mode_{self.mode_number}" for n in range(cpp_solver.n_sorted_mode)]

        for binding_number in range(cpp_solver.n_sorted_mode):
            label = f"mode_{self.mode_number}"
            supermode = SuperMode(
