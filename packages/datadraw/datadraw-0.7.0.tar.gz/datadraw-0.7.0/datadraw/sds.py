...
import math
from .sdsmisc import  FindRange, getinc, _catinfo, _numinfo, catspace,  \
...
class DataDraw:
    ...
    def svgresult(self, tofile=None, noclose=False):
        ...
        if tofile:
            self.write_svgfile(result, tofile)
        return result
    ...
    def write_svgfile(svgcode, filename):
    ...
    def test_pattern(self, width, height):
        ...
        self.settext(ptsize=11, color='#d88')
        ...
        if (tics or grid) and not stublist:
            ...
            while val <= valend:
                if not self.inrange(ax, val) or \
                  (stubcull and math.fabs(self.nu(ax, val)-prevdrawn) < stubcull):  
                    val += valinc    
                    continue
            ...
            if stublist:        # list of val (or val,label pairs) for irregular placement
                ...
                for mem in stublist:
                    ...
                    if not self.inrange(ax, val):
                        continue
                    if stubcull and math.fabs(self.nu(ax,val)-prevdrawn) < stubcull:
                        continue
                    prevdrawn = self.nu(ax, val)
                    if iax == 0:
    ...
    def legendrender(self, location='top', format='down', sampsize=6, lnlen=20, title=None, 
        ...
        for row in self.leg:
            ...
            if row['sample'] == 'circle': 
                ...
                self.rect( sx, y, sx+sampsize, y+(sampsize), color=row['color'], 
