...
def story(name: str, parent_name: str = ''):
    def decorator(cls): 
        parent = parent_name if len(parent_name) > 0 else cls.__mro__[1].__name__
        item_id = Launch.create_report_item(
            name=name,
            parent_item=parent,
            type='story',
...
def attachment(*, item: str, name: str, attachment: Union[str, bytes], attachment_type: str, level: Literal["ERROR", "INFO", "DEBUG"] = "ERROR"):
    ...
    """
    Launch.add_attachment(item=item, message=name, level=level, attachment_path=attachment, attachment_type=attachment_type)
