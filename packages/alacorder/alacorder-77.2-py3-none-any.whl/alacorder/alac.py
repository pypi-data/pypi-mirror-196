...
def filing(conf):
   ...
   df['CourtAction'] = df['CourtAction'].map(lambda x: x[0])
            ...
            try:
               ...
               try:
                  ...
                  try:
                     ...
                     click.echo("Fallback export to " + os.path.splitext(conf.OUTPUT_PATH)[0] + "-cases.json.zip due to Excel engine failure, usually caused by exceeding max row limit for .xls/.xlsx files!")
                  except (ImportError, FileNotFoundError, IndexError, ValueError):
