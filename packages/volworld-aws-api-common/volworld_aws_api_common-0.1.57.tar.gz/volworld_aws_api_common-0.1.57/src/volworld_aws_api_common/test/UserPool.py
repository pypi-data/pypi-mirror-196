...
class UserInfo:
    def __init__(self, name: str, password: str = 'default_password', rand_user: bool = True):
        self.name = name
...
class UserPool:
    ...
    def add_user(self, name: str, password: str = None, rand_user: bool = True) -> UserInfo:
        name = BehaveUtil.clear_string(name)
        assert name not in self.users
        self.users[name] = UserInfo(name, password, rand_user=rand_user)
        return self.users[name]

    def add_signup_user(self, name: str, password: str = None, rand_user: bool = True) -> UserInfo:
        name = BehaveUtil.clear_string(name)
        assert name not in self.users
