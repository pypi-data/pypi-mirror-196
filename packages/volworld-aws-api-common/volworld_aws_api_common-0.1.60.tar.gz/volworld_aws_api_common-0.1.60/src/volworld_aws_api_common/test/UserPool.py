...
class UserPool:
    ...
    def add_user(self, name: str, password: str = DEFAULT_PASSWORD, rand_user: bool = True) -> UserInfo:
        name = BehaveUtil.clear_string(name)
        assert name not in self.users
        self.users[name] = UserInfo(name, password, rand_user=rand_user)
        return self.users[name]

    def add_signup_user(self, name: str, password: str = DEFAULT_PASSWORD, rand_user: bool = True) -> UserInfo:
        name = BehaveUtil.clear_string(name)
        assert name not in self.users
