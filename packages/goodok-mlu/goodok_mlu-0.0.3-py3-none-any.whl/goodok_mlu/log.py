...
scalar_types = (int, np.int32, np.int64, np.uint, np.uint32, np.uint64)
...
def log(text, array=None, indent=''):
    ...
    if isinstance(text, dict) or isinstance(array, dict):
        log_dict(text, array, indent=indent)
        return
    if isinstance(array, (list, tuple)):
        d = dict([(str(i), v) for i, v in enumerate(array)])
        log_dict(text, d, indent=indent)
        return
        ...
        if isinstance(array, scalar_types):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        s_mean = '{:10.5f}'.format(array.float().mean()) if array.size else ""
                    except:
                        s_mean = ""
...
def log_dict(text, d=None, indent=''):
    if d is None:
        d = text
    else:
        print(f'{text}:')
    ...
    for key in d:
        try:
            ...
            if isinstance(value, str):
                ...
                # print(f"{indent}{s_key}'{value}'")
                d_list = dict([(str(i), v) for i, v in enumerate(value)])
                log(key, d_list, indent=indent + ' ' * 3)
            else:
