...
def s3_read_ids_json_files_from_cache(bucket_name: str = 'scrapautoscout-bucket'):
    path_to_json = f'{config.DIR_CACHE}/get_all_ids_for_search_url/'
    json_files = [pos_json for pos_json in os.listdir(path_to_json) if pos_json.endswith('.json')]
    ...
    for jfile in json_files:
        ...
        dir_name = jfile.replace('.json', '/')
        s3_create_folder_with_jsons_ids(list_ids, dir_name, bucket_name)
...
def s3_create_folder_with_jsons_ids(car_ids: List[str], folder_path: str, bucket_name: str):
    ...
    if 'Contents' not in folder_response:
        ...
        proxies_valid_ips = get_valid_proxies_multithreading()
        for id in car_ids:
            key_file_name = f'{folder_path}{id}.json'
            found = False
            request_try = 0
            while not found:
                # case proxies list is empty, we go further only if we have at least 3 valid proxies
                ...
                # check if request was successful
                if json_text is not None:
                    ''''creare fisier si upload in s3 '''
                    # create file with car info
                    data_string = json.dumps(json_text, indent=2, default=str)
                    client.put_object(
                        Bucket=bucket_name,
                        Key=key_file_name,
                        Body=data_string
