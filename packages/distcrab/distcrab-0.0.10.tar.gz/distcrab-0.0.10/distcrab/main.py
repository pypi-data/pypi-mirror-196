...
def distcrab(download=False, dump=False, ip='192.168.1.200', port=22, username='root', password=None, firmware=None, version=None, branch=None):
    ...
    if download:
        ...
        Client(ip=ip, port=port, username=username, password=password).putfo(src).exec_command([
        ...
        Client(ip=ip, port=port, username=username, password=password).exec_command([
            '/usr/local/bin/elite_local_stop.sh',
        ]).putfo(src).exec_command([
