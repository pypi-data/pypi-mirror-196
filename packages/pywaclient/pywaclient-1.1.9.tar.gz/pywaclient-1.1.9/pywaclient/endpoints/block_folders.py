...
class BlockFolderEndpoint(Endpoint):
    ...
    def blocks(self, world_id: str, folder_id: str, complete: bool = True,  limit: int = 50, offset: int = 0) -> Iterable[Dict[str, Any]]:
        """
        A complete list of the statblocks within a folder.

        :param world_id:    This is the identifier of the parent world.
        ...
        """
        if complete:
            return self._scroll_collection(self.paths, {'id': world_id}, 'entities', 'blockfolder', folder_id)
        return self._post_request(self.path,
                                  {'id': world_id},
                                  {'limit': limit, 'offset': offset, 'folder': {'id': folder_id}})['entities']
