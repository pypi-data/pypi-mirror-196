...
class PoolBackend(BackendBase):
   ...
   # API call
    def add_service_log_copy_cmds(self, cmds, dest_dir, args):
        ...
        if self.gen_for_windows:
            self.append(cmds, "xcopy logs\* {}\\".format(dest_dir))
        else:
            if docker_cmd:
                self.append(cmds, "mkdir {} && cp /usr/src/logs/* {}".format(dest_dir, dest_dir))
            else:
                self.append(cmds, "mkdir {} && cp logs/* {}".format(dest_dir, dest_dir))
    # API call
