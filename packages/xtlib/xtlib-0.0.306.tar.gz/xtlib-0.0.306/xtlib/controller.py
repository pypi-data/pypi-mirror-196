...
class XTController(rpyc.Service):
    ...
    def __init__(self, concurrent=1, my_ip_addr=None, multi_run_context_fn=None, multi_run_hold_open=False, 
        ...
        #     print("---------------")
        #     print("Controller detected abnormal exit", flush=True)
        #     self.cleanup_on_error_exit()
        ...
        # print("Controller is running and has hooked SIGTERM and SIGINT", flush=True)
    ...
    def bg_queue_worker(self):
        ...
        while True:
            ...
            if max_dur_text:
                ...
                if max_secs and (node_elapsed_secs > max_secs):

                    suffix = max_dur_text[-1]
                    max_actual_text = utils.secs_to_shell_time(node_elapsed_secs, suffix)
                    console.print("** Error: specified node max duration ({}) exceeded node's actual elapsed running time ({})".format( \
                        max_dur_text, max_actual_text))
    ...
    def process_queue_entry(self, run_info):
        log_info("process_queue_entry: run_info:", run_info)
