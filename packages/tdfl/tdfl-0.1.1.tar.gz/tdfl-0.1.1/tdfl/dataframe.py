...
class DataFrameGroupBy:
    ...
    def agg(self, *args, **kwargs):
        ...
        for arg in args:
            ...
            column, func = arg
            new_args[column].append(func)
        
        new_kwargs = collections.defaultdict(dict)
        for k, v in kwargs.items():
            if not (isinstance(v, tuple) and len(v) == 2):
                raise TypeError(f"v must be a 2-tuple of (column_name, func), but {v=}.")
                
            column, func = v
            new_kwargs[column][k] = func
        
        results = {}
        for column, args, kwargs in lib.itertools.dict_zip(new_args, new_kwargs, mode='union'):
            args = [] if args is None else args
            kwargs = {} if kwargs is None else kwargs
            result = self[column].agg(*args, **kwargs)
            if isinstance(result, DataFrame):
                results = results | {k if k in kwargs else f'{column}_{k}': v for k, v in result.items()}
            else:
...
class ColumnGroupBy:
    ...
    def _agg(self, func, results):
        ...
        _row_to_group_idx = self._row_to_group_idx.like(self.df[self.column], dtype=False)
        ...
        if func == 'count':
            ...
            result = _row_to_group_idx.bincount(weights=self.df[self.column])
            ...
            result = (_row_to_group_idx.bincount(
                weights=(self.df[self.column] - mean[_row_to_group_idx].data)**2
            ))/(count-1)
            ...
            v = ar.Array(self.df[self.column])
