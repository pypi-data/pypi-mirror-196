...
async def test_logging_service(application, logger_settings, handler_settings, formatter, settings):
    ...
    service.logger.debug('Testing DEBUG message {} / {extra_data}.', 'test', extra_data=True)
    service.logger.info('Testing INFO message {} / {extra_data}.', 'test', extra_data=True)
    service.logger.warning('Testing WARNING message {} / {extra_data}.', 'test', extra_data=True)
    service.logger.error('Testing ERROR message {} / {extra_data}.', 'test', extra_data=True)
