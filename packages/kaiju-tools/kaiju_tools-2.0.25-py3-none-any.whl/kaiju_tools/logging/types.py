...
class _LogRecord(logging.LogRecord):
    ...
    def getMessage(self) -> str:
        """Get log message."""
        msg = str(self.msg)
        if self.args or self._data:
            return msg.format(*self.args, **self._data)
        return msg
...
class DataFormatter(TextFormatter):
    ...
    @staticmethod
    def create_message(record: _LogRecord) -> LogMessage:
        ...
        if record.exc_info:
            ...
            msg['error'] = {'cls': error_cls.__qualname__, 'message': str(error)}
            if stack:
                msg['error']['trace'] = {
                    'stack': traceback.format_exc(),
                    'lineno': stack.tb_lineno,
                    # 'locals': stack.tb_frame.f_locals,
                }
        return msg  # noqa
