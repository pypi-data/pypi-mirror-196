...
class XGBoostBaseLocalTrainingHandler(LocalTrainingHandler, ABC):
    ...
    def update_model(self, model_update):
        ...
        try:
            ...
            if self.fl_model is None:
                ...
                logger.info(
                    'update_model: Initialized a new FL model object')

            # Update Model Object
            if model_update is not None:
                status = self.fl_model.update_model(model_update)
                logger.info('update_model: Local model updated')
            else:
                logger.info('update_model: No model update was provided')
    ...
    def train(self, fit_params=None):
        ...
        """
        logger.info('train: Begin [func={}]'.format(
    ...
    def initialize(self, params, last_step='update_raw_preds'):
        """
        ...
        logger.info('initialize: Received and initializing local hyper-parameters')
        ...
        logger.info(
            'initialize: Performing preliminary checks, validation, and initialization.')
    ...
    def check_prng_state(self):
        ...
        :return: `None`
        """
        logger.info('check_prng_state: Begin')
        # Check PRNG Random State
        rng = check_random_state(self.random_state)
        self._random_seed = rng.randint(np.iinfo(np.uint32).max, dtype='u8')

    def generate_sketch(self, party_id):
        logger.info('generate_sketch: Generate Local Party Data Sketch')
    ...
    def set_cat_bitsets(self, known_cat_bitsets, f_idx_map):
        ...
        :return: `None`
        """
        logger.info('set_cat_bitsets: Begin')
    ...
    def init_null_preds(self, baseline):
        ...
        logger.info('init_null_preds: Generating Initial Predictions from Null Model')
    ...
    def collect_hist(self, party_id, k):
        ...
        """
        logger.info('collect_hist: Begin')
    ...
    def update_raw_preds(self, k, predictor):
        ...
        :return: `None`
        """
        logger.info('update_raw_preds: Begin')
        logger.debug('update_raw_preds: [k={}] [predictor={}]'.format(
            k, predictor))
    ...
    def data_val_enc(self):
        ...
        :return: `None`
        """
        logging.info('data_val_enc: Performing local worker data encoding and validation')
        ...
        if message_type is MessageType.TRAIN.value:
            ...
            if hasattr(self,'last_step'):
                if 'update_raw_preds' == self.last_step:
                    last_step = (payload.get('func') == self.last_step) and (self.n_trees-1) == payload.get('args').get('k')
                else: 
...
class XGBRegressorLocalTrainingHandler(XGBoostBaseLocalTrainingHandler):
    ...
    def get_avg_stats(self):
        ...
        """
        logger.info('get_avg_stats: Compute Local Party Average Statistics')
