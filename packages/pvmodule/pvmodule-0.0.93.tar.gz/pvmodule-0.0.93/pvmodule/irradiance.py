...
#
  ...
  def _PV_SOL_Rear(self, data, albedo, module, panel_distance, panel_tilt, azimuth, Elevation):
    ...
    tan_delta_1v = (h + b*np.sin(panel_tilt*np.pi/180))/(b * np.cos(panel_tilt*np.pi/180) - d - x)
    tan_delta_1h = h / x

    tan_delta_2v = (h + b*np.sin(panel_tilt*np.pi/180))/(x + b*np.cos(panel_tilt*np.pi/180))
    tan_delta_2h =  h / (x + d)
