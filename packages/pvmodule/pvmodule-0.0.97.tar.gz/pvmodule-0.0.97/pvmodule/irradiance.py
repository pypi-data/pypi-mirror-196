...
class Irradiance():
  ...
  def _get_TMY(self, location, panel_tilt:float, azimuth:float, startyear:int=2020, endyear:int=2020 ):
    ...
    data = df_res.sort_values(by=['month'])
    ...
    df_date['date'] = pd.date_range(f"{2030}-01-01 00:00:00", f"{2030}-12-31 23:00:00", freq='5T').to_frame()
    data =  data.rename_axis('time').sort_values(by = ['month', 'time'], ascending = [True, True])


    data['Time'] = pd.to_datetime(data.index).time


    df_date['Time'] = pd.to_datetime(df_date['date']).dt.time
    df_date['Day'] = df_date.index.day
    df_date['Month'] = df_date.index.month

    final_df = pd.merge(data, df_date, how = 'inner', left_on = ['Time','month'], right_on = ['Time','Month'])
    final_df.index = final_df['date']
    final_df = final_df.sort_index()

    df_both = pd.date_range(f"{2030}-01-01 00:00:00", f"{2030}-12-31 23:00:00", freq='5T').to_frame()
    df_both = df_both.drop([0], axis=1)
    final_df = df_both.merge(final_df, left_index=True, right_index=True, how='left')
    ...
    final_df['Time_H'] = final_df.index.strftime("%H").astype(float)
    final_df['Time_M'] = final_df.index.strftime("%M").astype(float)
    final_df['Time_S'] = final_df.index.strftime("%S").astype(float)

    final_df['Month']= final_df['Month'].fillna(method='ffill')
    final_df['Day']= final_df['Day'].fillna(method='ffill')
    final_df = final_df.drop(['date','Time','Gd(i)','G(n)','Gd(n)'], axis=1)
