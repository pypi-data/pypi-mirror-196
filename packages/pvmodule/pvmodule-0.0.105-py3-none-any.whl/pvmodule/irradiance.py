...
class Irradiance():
  ...
  def _get_TMY(self, location, panel_tilt:float, azimuth:float, _month, startyear:int=2020, endyear:int=2020 ):
    ...
    df = pd.DataFrame({'year': [2030],

                      'month': [_month],

                      'day': [15]})
    
    df['day'] = df.index.values.astype('datetime64[Y]')
    df.index = df['day'].values.astype('datetime64[M]') + 3

    date = pd.to_datetime(df)
    DOY =  pd.DatetimeIndex(date.values).day_of_year[0] 

    data['Declination'] = (23.45*np.sin(np.deg2rad((360/365)*(284+DOY))))

    omega = abs((data['Time_H']*60+data['Time_M']+data['Time_S']/60)/4 - 180)
    
    data = data.drop(['Time_H', 'Time_M','Time_S','day'], axis=1)
