...
class Irradiance():
  ...
  def irradiance(self, module, location,_month, panel_tilt:float=35, albedo:float=0.2, azimuth:float = 0, Elevation=2, panel_distance:float = None):
    ...
    import pandas as pd
    n = 3
    new_index = pd.RangeIndex(len(self.pvmodule_irradiance_jan)*(n+1))
    new_df = pd.DataFrame(np.nan, index=new_index, columns=self.pvmodule_irradiance_jan.columns)
    ids = np.arange(len(self.pvmodule_irradiance_jan))*(n+1)
    new_df.loc[ids] = self.pvmodule_irradiance_jan.values
