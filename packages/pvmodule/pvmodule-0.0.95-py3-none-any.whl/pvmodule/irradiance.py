...
class Irradiance():
  ...
  def irradiance(self, module, location, panel_tilt:float=35, albedo:float=0.2, azimuth:float = 0, Elevation=2, panel_distance:float = None):
    ...
    G_Rear = Irradiance()._PV_SOL_Rear(data, albedo, module, panel_distance, panel_tilt, azimuth,Elevation)
    data['G_Rear'] = G_Rear
...
#
  ...
  def _PV_SOL_Rear(self, data, albedo, module, panel_distance, panel_tilt, azimuth, Elevation):
    #https://help.valentin-software.com/pvsol/en/calculation/pv-modules/bifacial-modules/
    import numpy as np
    ...
    x_start = Xm + - (h/ np.tan(panel_tilt*np.pi/180))
    x_end = Xm + d + (h*(d - b * np.cos(panel_tilt*np.pi/180)))/(b*np.sin(panel_tilt*np.pi/180))

    x = x_end - x_start

    tan_delta_1v = np.arctan((h + b*np.sin(panel_tilt*np.pi/180))/(b * np.cos(panel_tilt*np.pi/180) - d - x))
    tan_delta_1h =  np.arctan(h / x)

    tan_delta_2v =  np.arctan((h + b*np.sin(panel_tilt*np.pi/180))/(x + b*np.cos(panel_tilt*np.pi/180)))
    tan_delta_2h =   np.arctan(h / (x + d))

    E_diffuse_test_point = data['DHI']*((tan_delta_1h - tan_delta_1v) + (tan_delta_2h + tan_delta_2v)) / (np.pi/2)

    E_in_test_point = data['DHI'] + data['GHI']

    E_out_test_point = albedo * E_in_test_point

    E_rear_ground = E_out_test_point * (tan_delta_1h + tan_delta_2v)/(np.pi/180) * ((tan_delta_1h + tan_delta_2v)/(2)-panel_tilt*np.pi/180)

    tany_next = (b*np.sin(panel_tilt*np.pi/180))/(d + b*np.cos(panel_tilt*np.pi/180))

    E_rear_diffuse = data['DHI'] * (panel_tilt*np.pi/180 - tany_next)/(np.pi/180)

    E_rear = E_rear_diffuse + E_rear_ground
