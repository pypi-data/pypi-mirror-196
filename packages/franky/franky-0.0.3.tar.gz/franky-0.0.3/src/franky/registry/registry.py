...
class Registry:
    ...
    def infer_scope() -> str:
        ...
        if module is not None:
            ...
            # use "franky" to handle some cases which can not infer the scope
            # like initializing Registry in interactive mode
            scope = 'franky'
            print_log(
                'set scope as "franky" when scope can not be inferred. You '
    ...
    def switch_scope_and_registry(self, scope: Optional[str]) -> Generator:
        ...
        Examples:
            ...
            custom
            >>> # Switch to mmcls scope and get `OPCLS_MODELS` registry.
