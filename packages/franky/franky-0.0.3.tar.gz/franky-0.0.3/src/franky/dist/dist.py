...
import franky
from franky.device import is_npu_available
from franky.utils import digit_version
from franky.utils.dl_utils import TORCH_VERSION
from .utils import (get_world_size, get_rank, get_backend, get_dist_info, get_default_group, barrier, get_data_device,
...
def all_gather(data: Tensor,
    ...
    Note:
        ...
        Franky does not pass in an empty list ``gather_list`` and returns
        the ``gather_list`` directly, which is more convenient. The difference
        between their interfaces is as below:

        - Franky: all_gather(data, group) -> gather_list
...
def gather(data: Tensor,
    ...
    Note:
        ...
        - Franky: gather(data, dst, group) -> gather_list
...
def all_gather_object(data: Any,
    ...
    Note:
        ...
        :meth:`all_gather_object` in Franky does not pass in an empty list
        ...
        - Franky: all_gather_object(data, group) -> gather_list
...
def gather_object(data: Any,
    ...
    Note:
        ...
        :meth:`gather_object` in Franky does not pass in an empty list
        ``gather_list`` and returns the ``gather_list`` directly, which is
        more convenient. The difference between their interfaces is as below:

        - Franky: gather_object(data, dst, group) -> gather_list
...
def collect_results_cpu(result_part: list,
    ...
    if tmpdir is None:
        ...
        if rank == 0:
            franky.mkdir_or_exist('.dist_test')
