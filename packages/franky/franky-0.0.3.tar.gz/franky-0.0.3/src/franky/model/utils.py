...
def revert_sync_batchnorm(module: nn.Module) -> nn.Module:
    """Helper function to convert all `SyncBatchNorm` (SyncBN) in the model to
    `BatchNormXd` layers.
...
def convert_sync_batchnorm(module: nn.Module,
    ...
    `SyncBatchNorm` (SyncBN). Adapted from `PyTorch convert sync batchnorm`_.
    ...
    if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
        if implementation == 'torch':
            ...
            raise ValueError('sync_bn should be "torch", but got '
                             f'{implementation}')
