...
class FrankyDistributedDataParallel(DistributedDataParallel):
    """A distributed model wrapper used for training, testing and validation in
    loop.

    Different from DistributedDataParallel, OPDistributedDataParallel
    ...
    Note:
        ...
        :class:`OPSeparateDistributedDataParallel` should be used to wrap
        the model.

    Note:
        If model itself has custom optimization strategy, rather than
        simply forward model and update model. A custom model wrapper
        inherit from ``OPDistributedDataParallel`` should be defined and
    ...
    def train_step(self, data: Union[dict, tuple, list],
        ...
        with optim_wrapper.optim_context(self):
            losses = self._run_forward(data, mode='train')
