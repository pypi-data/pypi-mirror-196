...
if TYPE_CHECKING:
    ...
    from franky.optim.scheduler import _ParamScheduler
...
def build_from_cfg(
    ...
    Examples:
        >>> from franky.registry import Registry, build_from_cfg
    ...
    if not isinstance(registry, Registry):
        raise TypeError('registry must be a franky.Registry object, '
        ...
        if isinstance(obj_type, str):
            ...
            if obj_cls is None:
                raise KeyError(
                    ...
                    'can be found at '
                    'https://franky.readthedocs.io/en/latest/advanced_tutorials/config.html#import-the-custom-module'
                    # noqa: E501
                )
...
def build_runner_from_cfg(cfg: Union[dict, ConfigDict, Config],
    ...
    Examples:
        >>> from franky.registry import Registry, build_runner_from_cfg
    ...
    assert isinstance(
        registry, Registry), ('registry should be a franky.Registry object',
                              f'but got {type(registry)}')
    ...
    scope = args.pop('_scope_', None)
    with registry.switch_scope_and_registry(scope) as registry:
        obj_type = args.get('runner_type', 'franky.Runner')
        if isinstance(obj_type, str):
            ...
            if runner_cls is None:
                raise KeyError(
                    ...
                    f'Please check whether the value of `{obj_type}` is '
                    'correct or it was registered as expected. More details '
                    'can be found at https://franky.readthedocs.io/en/latest/tutorials/config.html#import-custom-python-modules'
                    # noqa: E501
                )
