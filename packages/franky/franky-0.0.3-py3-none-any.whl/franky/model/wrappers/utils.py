...
def is_model_wrapper(model: nn.Module, registry: Registry = MODEL_WRAPPERS):
    ...
    The following 4 model in Franky (and their subclasses) are regarded as
    model wrappers: DataParallel, DistributedDataParallel,
    OPDataParallel, OPDistributedDataParallel. You may add you own
    model wrapper by registering it to ``franky.registry.MODEL_WRAPPERS``.
