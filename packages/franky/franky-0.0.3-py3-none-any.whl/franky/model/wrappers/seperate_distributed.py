...
class FrankySeparateDistributedDataParallel(DistributedDataParallel):
    ...
    it will cause errors during training, because when we update the parameters
    of the generator (or discriminator), the parameters of the discriminator
    (or generator) is not updated, which is not allowed for DistributedDataParallel.
    So we design this wrapper to separately wrap DistributedDataParallel for
    generator and discriminator.
    In this wrapper, we perform two operations:

    1. Wraps each module in the models with separate OPDistributedDataParallel.
    ...
    Args:
        ...
        **kwargs: Keyword arguments passed to ``OPDistributedDataParallel``.
    ...
    def __init__(self,
        ...
        for name, sub_module in module._modules.items():
            ...
            if next(sub_module.parameters(), None) is None:
                ...
                sub_module = OPDistributedDataParallel(
    ...
    @contextmanager
    def no_sync(self):
        """Enables ``no_sync`` context of all sub ``OPDistributedDataParallel``
        modules."""
    ...
    def train(self, mode: bool = True) -> 'OPSeparateDistributedDataParallel':
        """Sets the module in training mode.

        In order to make the ddp wrapper inheritance hierarchy more uniform,
        ``OPSeparateDistributedDataParallel`` inherits from
