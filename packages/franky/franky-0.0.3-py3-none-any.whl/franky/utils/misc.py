...
def is_seq_of(seq: Any, expected_type: Union[Type, tuple], seq_type: Type = None) -> bool:
    ...
    Examples:
        >>> from franky.utils import is_seq_of
        >>> seq = ['a', 'b', 'c']
        >>> is_seq_of(seq, str)
        True
        >>> is_seq_of(seq, int)
...
def check_prerequisites(prerequisites, checker, msg_tmpl='Prerequisites "{}" are required in method "{}" but not '
            ...
            if args:
                ...
                for src_arg_name, dst_arg_name in name_dict.items():
                    if src_arg_name in arg_names:
                        warnings.warn(f'"{src_arg_name}" is deprecated in '
            ...
            if kwargs:
                for src_arg_name, dst_arg_name in name_dict.items():
                    if src_arg_name in kwargs:
                        assert dst_arg_name not in kwargs, (f'The expected behavior is to replace '
...
def is_method_overridden(method: str, base_class: type, derived_class: Union[type, Any]) -> bool:
...
def deprecated_function(since: str, removed_in: str, instructions: str) -> Callable:
    ...
    def decorator(function):
        ...
        def wrapper(*args, **kwargs):
            print_log(f"'{function.__module__}.{function.__name__}' "
                      f'is deprecated in version {since} and will be '
                      f'removed in version {removed_in}. Please {instructions}.', logger='current',
