...
Franky currently supports two file backends:

- LocalBackend
- HTTPBackend

Note that this module provide a union of all of the above two backends so
...
Examples:
    ...
    >>> import franky.fileio as fileio
...
from franky.utils import is_filepath, is_str
...
def copyfile_from_local(
    ...
    Examples:
        >>> # dst is a file
        >>> src = '/path/of/file'
        >>> dst = 's3://openmmlab/franky/file1'
        >>> # src will be copied to 's3://openmmlab/franky/file1'
        >>> copyfile_from_local(src, dst)
        s3://openmmlab/franky/file1

        >>> # dst is a directory
        >>> dst = 's3://openmmlab/franky'
        >>> # src will be copied to 's3://openmmlab/franky/file''
        >>> copyfile_from_local(src, dst)
        's3://openmmlab/franky/file'
    """
    backend = get_file_backend(
...
def copytree_from_local(
    ...
    Examples:
        >>> src = '/path/of/dir'
        >>> dst = 's3://openmmlab/franky/dir'
        >>> copyfile_from_local(src, dst)
        's3://openmmlab/franky/dir'
    """
    backend = get_file_backend(
        dst, backend_args=backend_args, enable_singleton=True)
