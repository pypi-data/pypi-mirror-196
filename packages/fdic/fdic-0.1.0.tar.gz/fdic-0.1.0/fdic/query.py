...
>>> from examples import query_fdic_api
>>> data = query_fdic_api.InstData.get_sorted_inst_data()
    """
    ...
    @classmethod
    def get_ubpr_data(cls, ubpr_zip_file, rssd_filter=None, codes=(
    ...
    @classmethod
    def create_ubpr_csv(cls, ubpr_zip_file, **kwargs):
        inst_data = cls.get_sorted_inst_data()
        rssd_map = {i['FED_RSSD']: i for i in inst_data}
        ubpr_data = cls.get_ubpr_data(ubpr_zip_file, **kwargs)
    ...
    def download_inst_file(cls, ifile):
        ...
        if not ifile:
            ifile = os.path.join(tempfile.gettmpedir(), 'fdic_institutions.csv')
            logging.warning('Using %s for institutions file location.')
            cls.ifile = ifile
