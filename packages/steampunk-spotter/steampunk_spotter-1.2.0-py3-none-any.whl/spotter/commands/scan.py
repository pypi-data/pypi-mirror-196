...
class ScanResult:
    ...
    def _parse_known_check_result(self, element: Dict[str, Any], input_items: Dict[str, Dict[str, Any]]
        ...
        result = CheckResult(
            correlation_id, original_item, item_metadata_object,
            CheckCatalogInfo.from_api_response_element(element), display_level, element.get("message", ""),
            suggestion_object, element.get("doc_url", ""), check_type
        )
        return result
    ...
    def _parse_unknown_check_result(self, element: Dict[str, Any]) -> CheckResult:
        ...
        result = CheckResult(
            "", {}, None,
            check_catalog_info, display_level, element.get("message", ""),
            None, element.get("doc_url", ""), check_type
        )
        return result
        ...
        if len(self.check_results) > 0:
            ...
            if not disable_colors:
                time_message = f"Spotter took {Style.BRIGHT}{self.summary.scan_time:.3f} s{Style.NORMAL} to scan " \
                               f"your input."
                stats_message = f"It resulted in {Fore.RED}{self.summary.num_errors} error(s){Fore.RESET}, " \
                                f"{Fore.YELLOW}{self.summary.num_warnings} warning(s){Fore.RESET} and " \
                                f"{Style.BRIGHT}{self.summary.num_hints} hint(s){Style.NORMAL}."

                if worst_level == DisplayLevel.ERROR:
                    overall_status_message = f"Overall status: {Fore.RED}{worst_level.name.upper()}{Fore.RESET}"
                elif worst_level == DisplayLevel.WARNING:
                    overall_status_message = f"Overall status: {Fore.YELLOW}{worst_level.name.upper()}{Fore.RESET}"
                else:
        ...
        try:
            ...
            for check_class, check_results in itertools.groupby(sorted(self.check_results, key=get_check_class),
                ...
                for result in check_results:
                    ...
                    test_case.file = result.metadata.file_name if result.metadata else ""
                    test_case.result = [
