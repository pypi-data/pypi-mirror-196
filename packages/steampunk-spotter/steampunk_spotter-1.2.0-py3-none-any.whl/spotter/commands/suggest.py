...
def add_parser(subparsers: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    ...
    parser = subparsers.add_parser(
        "suggest", argument_default=argparse.SUPPRESS
    )
    parser.add_argument(
...
def _parser_callback(args: argparse.Namespace) -> None:
    ...
    """
    api_endpoint = os.environ.get("SPOTTER_ENDPOINT", None)
    ...
    suggestions = suggest(api_endpoint, storage_path, username, password, args.query, args.num_results)
...
def suggest(api_endpoint: Optional[str], storage_path: Path, username: Optional[str], password: Optional[str],
    ...
    api_client = ApiClient(endpoint, storage, username, password)
