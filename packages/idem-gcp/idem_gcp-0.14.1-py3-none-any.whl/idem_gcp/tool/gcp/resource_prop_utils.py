...
def get_get_properties(hub, resource_type: str, convert_to_present: bool = True) -> Set:
...
def get_elements_from_resource_id(hub, resource_type: str, resource_id: str) -> Dict:
    ...
    if not result:
        msg = f"Resource ID {resource_id} matches no resource paths of {resource_type}"
        hub.log.warning(msg)
        return {}
...
def parse_link_to_resource_id(hub, link, resource_type: str) -> str:
    resource_paths = hub.tool.gcp.resource_prop_utils.get_resource_paths(resource_type)
    result = None
    for path in resource_paths:
