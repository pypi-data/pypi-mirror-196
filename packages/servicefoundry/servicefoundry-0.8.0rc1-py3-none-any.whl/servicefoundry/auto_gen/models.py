...
#   timestamp: 2023-03-10T10:36:26+00:00
...
class Recreate(BaseModel):
    """
    +docs=This strategy brings down all the older pods and then replaces with the newer version.
    To learn more you can go [here](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#recreate-deployment)
    +label=Recreate strategy
    """

    type: constr(regex=r"^recreate$") = Field(..., description="+value=recreate")
...
class Job(BaseModel):
    ...
    timeout: conint(ge=0, le=86400) = Field(
        43200,
...
class Service(BaseModel):
    ...
    rollout_strategy: Optional[Union[Rolling, Recreate]] = Field(
        None,
