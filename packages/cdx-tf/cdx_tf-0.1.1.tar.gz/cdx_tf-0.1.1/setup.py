...
init_file = "./cdx_tf/__init__.py"
with open(init_file, "rt") as fh:
    init = fh.read()
    find_str = '__version__ = "'
    i = init.find(find_str)
    assert i>0, "Error: cannot find string '%s' in cdx_tf/__init__.py" % find_str
    i += len(find_str)
    data = init[i:]
    start = init[:i]
    i = data.find('"')
...
setuptools.setup(
    name="cdx_tf", 
    version=version,    # found in cdx_tf/__init__.py
    author="Hans Buehler",
    author_email="github@buehler.london",
    description="Basic Tensorflow Python tools",
