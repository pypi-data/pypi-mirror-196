...
@click.option('--input', default=None, type=click.Path(allow_dash=True), help='Input CSV file. For compile action, this is the input file to expand the dictionary. For process action, this is the input CSV to process.')
@click.option('--output', default=None, type=click.Path(allow_dash=True, writable=True), help='Output file. For compile action, this is the output file to write the dictionary. For process action, this is the output file to write the processed input file.')
@click.option('--dictionary', required=False, type=click.Path(allow_dash=True), default="builtin", help='Dictionary file. For "compile" action, this is the dictionary file to read and expand. For "process" and "filter" action, this is the dictionary file to use. If not specified, a built-in dictionary is used.')
@click.option('--words', default='', help='String to filter.')
@click.option('--columns', default='*', help='Columns to process in the input CSV. Separated by commas. Only for process action, this is the columns to process.')
...
def cli(action, input, output, columns, words, dictionary, csv, encoding, delimiter, profile):
    """Procesador de nombres y apellidos. Pasa de todo mayúsculas a capitalizado con tildes y minúsculas.\n
