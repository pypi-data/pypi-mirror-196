...
class KuriSyncBase(object):
    ...
    def __sync_chain_block_number(self, filters:dict={}):
        ...
        logging.info(f"{sync_location} : syncing chain at block number {block_number} ...")
        to_sync = {
            f"chain": f"{block_number}"
        }
        logging.debug(f"pre-sync : {synced}")
        logging.debug(f"to sync : {to_sync}")
        synced.update(to_sync)
        with open(f"{sync_location}", "w") as writable:
            json.dump(synced, writable)
        logging.debug(f"post-sync : {synced}")

    def __sync_kuri_block_number(self, kuri:str, filters:dict={}):
        synced, sync_location = self.__synced(filters=filters)
        block_number = self.__cached(self.CACHE_KEY_CURRENT_BLOCK_NUMBER, default=0)
        logging.info(f"{sync_location} : syncing kuri {kuri} at block number {block_number} ...")
        to_sync = {
    ...
    def __sync(self, 
        ...
        logging.info(f"start syncing from {start_block_hash} ...")
            ...
            for kuri_info in block["extrinsics"]:
                ...
                if self.__synced_kuri_block_number(kuri=kuri, filters=filters) <= self.__cached(self.CACHE_KEY_CURRENT_BLOCK_NUMBER, default=0):
                    ...
                    try:
                        ...
                        logging.exception(f"storage error : {err}")
    ...
    def sync(self,
        ...
        if start_block_number and start_block_number > synced_chain_block_number:
            logging.warn(f"start_block_number {start_block_number} is later than synced_chain_block_number {synced_chain_block_number}")
            logging.info(f"resetting start_block_number to {synced_chain_block_number}")
        ...
        if synced_chain_block_number > tip_block_number:
            logging.warn(f"synced_chain_block_number {synced_chain_block_number} is later than tip_block_number {tip_block_number}")
            logging.info(f"resetting start_block_number to {tip_block_number}")
        ...
        # log:DEBUG
        logging.debug(f"tip_block_number : {tip_block_number}")
        logging.debug(f"synced_chain_block_number : {synced_chain_block_number}")
        logging.debug(f"start_block_hash : {start_block_hash}")
        logging.debug(f"query : {query}")
        logging.debug(f"storage_info : {storage_info}")
        ...
        try:
            ...
            if str(err) == "{'code': -32000, 'message': 'Client error: UnknownBlock: State already discarded for "+start_block_hash+"'}":
                logging.error(f"State already discarded for {start_block_hash}")
                logging.info(f"resetting start_block_number to {tip_block_number - 200}")
