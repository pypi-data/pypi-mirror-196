...
class TopicListener(KuriSyncBase, IPFSPubSub, StorageBase, HasherBase, IPFSClient):
    ...
    def __init__(self, node_url:str=None, chain_swarm_key_path:str=None, listener_swarm_key_path:str=None, path:str=None, ipfs_directory_path=None, **encoder_kwargs) -> None:
    ...
    def __setup(self, node_url:str=None, path:str=None, ipfs_directory_path:str=None, chain_swarm_key_path:str=None, listener_swarm_key_path:str=None):
        assert node_url is not None
    ...
    async def publish_status(self, topic_id:int=None) -> str:
        ...
        try:
            ...
            logging.error(f"IPFS pubsub publish error: {error}")
            raise error
        finally:
    ...
    def __handle_subscription_topic_arikuris_from_committer(self, data, seqno, topic_ids, cid, subscription):
        ...
        logging.debug(f"Seqno: {seqno}")
        logging.debug(f"Topic IDs: {topic_ids}")
        logging.debug(f"Message CID: {cid}")
        logging.info(f"\n\n")
        ...
        with open(f"{self.data_directory}/mappings.json", "r") as f:
            ...
            if kuri not in mappings:
                ...
                try:
                    # pin the file
                    logging.info(f"Pinning file CID ({kuri_cid}) for kuri {kuri} ...")
                    res = self._ipfs_client.pin.add(kuri_cid)
                    logging.debug(f"pin response : {res}")
                except TimeoutError:
                    raise StorageError(
                        f"\n\n\n\nEncountered an error while saving kuri {kuri}\n\nIs the CID {kuri_cid} visible on your IPFS client?\n\n"
                    )
                # add the file from cid to data/topic_id directory
                # create the data/topic_id directory if it does not exist
                topic_data_directory = f"{self.data_directory}/{topic_id}"
                if not os.path.exists(topic_data_directory):
                    os.makedirs(topic_data_directory)
                logging.info(f"Adding file CID ({kuri_cid}) for kuri {kuri} to {topic_data_directory} directory ...")
                with open(f"{topic_data_directory}/{kuri_file_name}", "wb") as f:
                    f.write(self._ipfs_client.cat(kuri_cid))
                # add the kuri to mappings
                mappings[kuri] = f"{topic_data_directory}/{kuri_file_name}"
                with open(f"{self.data_directory}/mappings.json", "w") as f:
                    json.dump(mappings, f)
                logging.info(f"KURI {kuri} added to mappings")
    ...
    def save_kuri(self, kuri, filters:dict={}):
        ...
        logging.debug(f"{kuris_in_status = }")
            ...
            if kuri not in mappings:
                ...
                if kuri not in kuris_in_rff:
                    with open(f"{self.sync_directory}/rff.txt", "a") as f:
                        f.write(f"\n{kuri}")
                logging.info(f"SUBSCRIBING TO KURI: {kuri}")
                ...
                try:
                    ...
                    logging.error(f"Error subscribing to {kuri}: {error}")
