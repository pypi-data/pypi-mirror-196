...
class TopicCommitter(IPFSPubSub, StorageBase, HasherBase, IPFSClient):
    ...
    def switch_to_chain_swarm(self):
        ...
        logging.info(f"Switched to chain swarm key: {self.chain_swarm_key_hash}")
    ...
    def switch_to_listener_swarm(self, listener_swarm_key_hash:str=None):
        ...
        self._set_ipfs_swarm_key()
        logging.info(f"Switched to listener swarm key: {listener_swarm_key_hash}")
    ...
    def connect_to_listener_node(self, listener_ip_address:str=None, listener_ipfs_id:str=None, ip:str="127.0.0.1", port:int=5001):
        ...
        logging.info(f"Adding listener node to swarm peers: {listener_ip_address}")
        ...
        logging.info(f"Connecting to listener node: {listener_ip_address}")
    ...
    def disconnect_from_listener_node(self, listener_ip_address:str=None, listener_ipfs_id:str=None, ip:str="127.0.0.1", port:int=5001):
        ...
        logging.info(f"Removing listener node from swarm peers: {listener_ip_address}")
        ...
        logging.info(f"Disconnecting from listener node: {listener_ip_address}")
    ...
    def watch_data_directory(self, topic_id:int=None):
        ...
        for file in os.listdir(topic_data_directory):
            ...
            if file not in self.data_file_set_per_topic[topic_id]:
                ...
                logging.debug(f"New file found: {file}")
                logging.debug(f"{self.data_file_set_per_topic = }")
                kuri_data = {
                    "topic_id": topic_id,
                    "type": "file",
                ...
                try:
                    ...
                    if transaction_hash:
                        logging.info(f"New file uploaded: {file}")
    ...
    def __handle_subscription_topic_updates_from_listener(self, data, seqno, topic_ids, cid, subscription):
        ...
        with open(f"{listener_directory}/rff.txt", "w") as f:
            f.write(rff)
        logging.info(f"Topic Listener status updated: {status}")
        logging.info(f"Topic Listener rff updated: {rff}")
        ...
        try:
            ...
            logging.debug(f"KURIS TO PUBLISH:\n{kuris_to_pubish}\n")
            with open(f"{self.data_directory}/mappings.json", "r") as f:
                mappings = json.load(f)
                ...
                for kuri in kuris_to_pubish:
                    ...
                    logging.debug(f"KURI IN MAPPINGS: {kuri in mappings}")
                    if kuri in mappings:
                        ...
                        logging.info(f"IPFS CID CREATED : {ipfs_cid['Hash']}")
            ...
            logging.error(f"IPFS timeout error. Is your IPFS client connected to the Listener's IPFS client?\n{TimeoutError}")
