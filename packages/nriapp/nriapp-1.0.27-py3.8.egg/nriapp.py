...
#            data["computerName"]    = computerName
    ...
    def get_timely_report(self, lookBackInDays):
        incidents = self.sentinel.get_incidents(incidentId=0, alertStatus=['New','InProgress', 'Resolved'] , severity=[256,128,64,32], pageIndex=1, lookBackInDays=lookBackInDays, pageSize=3000, sourceFilter=[16384, 1048576], titleFilter=["eDiscovery"])


        pass
                                

    def get_full_report(self, args=None, lookBackInDays=30):
...
#            xls = WorkBook(self.output + "\\" + args + "xlsx")
            ...
            for i in range(1):
                ...
                try:
                    ...
                    dbgPrint.error("Days is out of range. You may expand the range up to 180 days. Use -d")
                    sys.exit(0)
...
#                self.excel_tracker(out)
            ...
            doc.traverse([{count: ipquery.check_endpoint(i)} for count, i in enumerate(ip_list)])
            doc.insertHR(doc.insert_paragraph())
            doc.traverse([{count: self.sentinel.get_file_info(i)} for count, i in enumerate(hash_list)])
            doc.insertHR(doc.insert_paragraph())
            doc.traverse([{count:self.user_summary(i)} for count, i in enumerate(recipient)])
            doc.insertHR(doc.insert_paragraph())
...
#            user_info = self.msgraph.search_user(args.get("user"))
            ...
            self.get_full_report(args.get("incidentId"), lookBackInDays=args.get("daysago",30))
            end = time.time()
            ...
            self.get_timely_report(args.get("timely"))
        return
...
#    parser.add_argument('-c','--clear', help = "Clear cache", action="store_true")
    ...
    parser.add_argument('-t', '--timely', help="Creates a timely report based on the number of days")
