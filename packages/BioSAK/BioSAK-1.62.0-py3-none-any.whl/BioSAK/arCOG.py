...
# How it works:
1. arCOG module uses Blast+/Diamond to get the best hits of query genes in the database 
...
# Note!!!
If you run arCOG for multiple files in a batch manner and want to have their depth info incorporated into the results, 
...
def arCOG(args):
    ...
    for each_cog in open(cog_des_txt, encoding="ISO-8859-1"):
        ...
        cog_id_to_description_dict[each_cog_split[0]] = each_cog_split[3]
        cog_id_to_category_dict[each_cog_split[0]] = each_cog_split[1]
        cog_category_set.add(each_cog_split[1])
