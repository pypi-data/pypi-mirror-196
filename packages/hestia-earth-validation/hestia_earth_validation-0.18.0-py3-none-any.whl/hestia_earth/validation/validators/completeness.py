...
def _validate_animalFeed(cycle: dict, site: dict):
    ...
    site_type = site.get('siteType')
    is_liveAnimal = any([
        len(filter_list_term_type(cycle.get('animals', []), TermTermType.LIVEANIMAL)) > 0,
        len(filter_list_term_type(cycle.get('products', []), TermTermType.LIVEANIMAL)) > 0
    ])
    return not is_complete or site_type not in [
        SiteSiteType.CROPLAND.value,
        SiteSiteType.PERMANENT_PASTURE.value
    ] or not is_liveAnimal or _has_terms(cycle, get_forage_terms(), allow_no_value=False) or {
        'level': 'error',
