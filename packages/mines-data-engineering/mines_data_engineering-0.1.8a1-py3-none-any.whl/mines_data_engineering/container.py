...
DBDIR: Path = Path(os.getenv("USER")) / "/scratch/tmp"
...
class TimescaleDB(Service):
    ...
    def run(cls, image_file: str = _SINGULARITY_TIMESCALE_IMAGE):
        if USE_DOCKER:
            ...
            inst = client.instance.instance(image=image_file, options=[f"-B {DBDIR}:/var/lib/postgresql/data", f"-B {DBDIR}:/var/run/postgresql", f"--env=POSTGRES_PASSWORD"], name=_TIMESCALE_INSTANCE)
            def _run_timescale():
    ...
    def conn_string(cls):
        if USE_DOCKER:
            ...
            return f"user=postgres password=password host={DBDIR}"
    ...
    def sqlmagic(cls):
        if USE_DOCKER:
            ...
            return f"postgresql:///postgres?user=postgres&password=password&host={DBDIR}"
