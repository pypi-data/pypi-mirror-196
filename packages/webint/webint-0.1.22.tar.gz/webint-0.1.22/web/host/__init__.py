...
class Machine(MachineBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.runinenv = "/home/gaea/runinenv"
        self.projects_dir = self.system_dir / "projects"
        self.env_dir = "/home/gaea/app"
        self.run_dir = f"{self.env_dir}/run"
    ...
    def setup_python(self):
        ...
        self.run(f"chown gaea:gaea {self.runinenv}")
    ...
    def setup_nginx(self):
        ...
        self.run(
            ...
            stdin=str(templates.nginx(self.address, [self.onion], {})),
        )
        self.run(f"chown gaea:gaea {self.nginx_conf}/nginx.conf")

        self.user = "gaea"

        # streaming server
        streaming_dir = "/home/gaea/app/run/media/streaming"
        ...
        self.run(f"mkdir {self.run_dir}/certs -p")
        ...
        self.run(
            f"cat > {self.run_dir}/certs/selfsigned-ip.cnf",
            stdin=config_output.getvalue(),
        )
        self.run(
            "sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048"
            " -keyout /etc/ssl/private/selfsigned-ip.key"
            " -out /etc/ssl/certs/selfsigned-ip.crt"
            f" -config {self.run_dir}/certs/selfsigned-ip.cnf"
        )
        ...
        # git server
        self.run(
            "git config --file=/lib/systemd/system/fcgiwrap.service Service.User gaea"
        )
        self.run(
            "git config --file=/lib/systemd/system/fcgiwrap.service Service.Group gaea"
        )
        self.run("systemctl daemon-reload")
        self.run("service fcgiwrap restart")

        # get onion
        self.run("cp /var/lib/tor/main/hostname /home/gaea/app/run/onion")
        self.run("chown gaea:gaea /home/gaea/app/run/onion")
    ...
    def setup_app(self, package, app, do_token=None):
        ...
        if do_token:
            self.run(
                ...
                stdin=f'DIGITALOCEAN_TOKEN = "{do_token}"',
            )
