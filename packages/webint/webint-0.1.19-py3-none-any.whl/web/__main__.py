...
class Bootstrap:
    ...
    def run(self, stdin, log):
        ...
        with console.status(
            ...
            machine.setup_tor()
            # XXX machine = web.host.Machine("143.110.230.47", "root", "gaea_key")
            # XXX machine.onion = (
            # XXX   "izxwrqchdaqpj2iphrqoh6govp4dqolem4epoxv2nijqvlo5q3kbd5qd.onion"
            # XXX )
            console.print("Installing nginx")
            machine.setup_nginx()
            console.print(f"Installing application: {self.app} of {self.package}")
            machine.setup_app(self.package, self.app)
            time.sleep(2)
        console.rule(
            f"[green]Bootstrapped {self.package} ({self.app}) at {machine.address}"
        )
        webbrowser.open(f"https://{machine.address}")
