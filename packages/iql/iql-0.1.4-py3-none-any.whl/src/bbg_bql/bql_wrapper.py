...
def error_callback(o, error_list, suppress_warning_log: bool = False, q=None):
...
def execute_bql_str_list_async_q(
    ...
    if len(queries_input) == 0:
        logger.info("No queries to run")
        return

    query_groups = [
        queries_input[i * max_queries : (i + 1) * max_queries]
        for i in range((len(queries_input) + max_queries - 1) // max_queries)
    ]
...
def _execute_bql_str_list_async_orig(
    ...
    if submit_fetch_many_available():
        ...
        gen = bqService.submit_fetch_many(query_strings)
    else:
...
def _process_single_item_response(res: bql.SingleItemResponse, result_table, shift_duplicates: bool = False, replaceNanInfsWithNone: bool = True):
    name = res.name
    logger.debug(f"Processing {name}")

    values = res._SingleItemResponse__result_dict.get("valuesColumn").get("values")
    values_type = res._SingleItemResponse__result_dict.get("valuesColumn").get("type")

    secondary_cols = res._SingleItemResponse__result_dict.get("secondaryColumns")
    # dates = [col for col in secondary_cols if col.get("name") == "DATE"]

    id_col = res._SingleItemResponse__result_dict.get("idColumn")
    ...
    for i in range(0, len(values)):
        ...
        elif replaceNanInfsWithNone and values_type == "DOUBLE" and values[i] in ["NaN", "Infinity", "-Infinity"]:
            row["value"] = None
        else:
            row["value"] = values[i]
