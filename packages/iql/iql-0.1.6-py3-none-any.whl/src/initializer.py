...
def initialize():
    ...
    keys.read(f"{os.path.dirname(__file__)}/keys.ini")
    ...
    if dbmodule_name is None:
        ...
        from db_connectors import duckdb_connector as ddc
    ...
    if config.get("settings", "FRED_ENABLE", fallback=False):
        ...
        if fred_key is None:
            ...
            from extensions import fred_extension as fe
    ...
    if config.get("settings", "KAGGLE_ENABLE", fallback=False):
        ...
        if kaggle_username is None or kaggle_key is None:
            ...
            from extensions import kaggle_extension as ke
