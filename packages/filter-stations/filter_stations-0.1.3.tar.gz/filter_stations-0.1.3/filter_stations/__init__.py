...
def create_animation(data, columns, day=100, T=10, interval=500):
    '''
    T days giving the range 
    valid sensors
    '''
    
    data = np.array(data[columns].iloc[day:day+1]).T
    data /= np.max(data)
    ...
    def update(i):
        # Generate new data
        new_data =  np.array(data[columns].iloc[day:day+i+1]).T
...
def plot_station(ws, df_rainfall):
    ...
    # ws = random.choice(western_stations)

    # select sensors and clog flags for the chosen station
    ...
    for i, sensor in enumerate(sensors):
        ax[i].plot(df_rainfall[sensor])
        ax[i].set_title(sensor)

    # plot data for clog flags
    for i, flag in enumerate(clog_flags):
        ax[len(sensors)+i].plot(df_rainfall[flag])
        ax[len(sensors)+i].set_title(flag)
...
# Draw the map
def get_map(subset1, subset2, start_date=None, end_date=None, csv_file='KEcheck3.csv'):
    # Read the csv file 
    ...
    # create a map centered on a specific location
    my_map = folium.Map(map_center, min_zoom=8, max_zoom=12, zoom_start=12, tiles='cartodbpositron')
