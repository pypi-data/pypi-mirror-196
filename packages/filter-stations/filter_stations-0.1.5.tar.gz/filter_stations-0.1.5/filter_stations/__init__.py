...
def get_map(subset1, subset2, start_date=None, end_date=None, data_values=False, csv_file='KEcheck3.csv', min_zoom=8, max_zoom=11, width=850, height=850):
    # Read the csv file 
    ...
    for _, row in stations.iterrows():
        if row['code'] in subset1_stations:
            ...
            if data_values:
                popup_iframe = folium.IFrame(html=f"{encode_image(row['code'], df_rainfall)}", width=width, height=height)
