...
class DynaMine(B2bIo):
  ...
  def __init__(self,predictionTypes=('backbone', 'sidechain', 'ppII', 'coil', 'sheet', 'helix')):
    ...
    self.references = ['doi: 10.1038/ncomms3741 (2013)', 'doi: 10.1093/nar/gku270 (2014)']
  ...
  def adjustModels(self):
    ...
    for modelKey, model in self.modelDict.items():
      ...
      for i in range(len(model)):
        newdistribution = self.filterOutliers(list(model[i].values())) # remove outliers
  ...
  def predictSeqs(self, seqs, force_windowsize=None):
    ...
    for (seqId,sequence) in seqs:
      ...
      # print(f'Predicting dynamics from {seqId}...\n')
      ...
      for (predictionType,suffix) in self.types_:
        self.allPredictions[seqId][predictionType] = self.computePredictions(sequence, predictionType)
