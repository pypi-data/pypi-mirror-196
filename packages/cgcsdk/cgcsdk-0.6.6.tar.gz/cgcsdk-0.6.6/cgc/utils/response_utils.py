...
def _get_response_error_message(response: requests.Response):
            ...
            try:
                click.echo(
                    prepare_error_message(
                        CUSTOM_EXCEPTIONS[response.status_code][response_dict["reason"]]
                    )
                )
            except KeyError:
                error_message = _get_response_error_message(response)
