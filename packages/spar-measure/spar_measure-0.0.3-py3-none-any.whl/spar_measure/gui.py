...
class Meaurement:
    ...
    def read_csv_cols(self, file_obj):
        if file_obj is not None:
            self.input_df = pd.read_csv(file_obj[0].name, encoding="ISO-8859-1")
    ...
    def embed_texts(self, sentences, progress, batch_size=8):
    ...
    def embed_df(self, progress=gr.Progress()):
        try:
            self.embeddings = self.embed_texts(
                self.input_df[self.doc_col_name], progress=progress
            )
            np.save(Path(self.path_mgt.out_dir, "embeddings.npy"), self.embeddings)
            if (self.doc_col_name is not None) and (
            ...
            return (
                gr.Textbox.update(value=f"Embedding failed! \nError message: {e}"),
    ...
    def semantic_search(self, query, n_results, progress=gr.Progress()):
        ...
        try:
            query_embedding = self.embed_texts(queries, progress=progress)
            ...
            result_str = (
                f"Error. Make sure the query is not empty. \nError message: {e}"
            )
        return gr.Textbox.update(value=result_str)

    def save_dims(self, progress=gr.Progress(), *dims_boxes):
        try:
            ...
            for i in range(self.n_dims):
                ...
                dim_embedding = self.embed_texts(dim_queries, progress=progress)
            ...
            return (
                ...
                + [
                    gr.Textbox.update(
                        visible=True,
                        value=f"Please check that you have filled in all the dimension names and queries. \n  Error message: {e}.",
                    )
                ]
                + [gr.File.update(visible=False)]
                + [None]
        ...
        try:
            ...
            return (
                [
                    gr.Textbox.update(
                        visible=True,
                        value=f"Error: Please check your scale definitions. Make sure that you have clicked the Embed Queries and Save Dimensions button in Tab 2, and there is no empty scales or dimensions. \nError message: {e}",
                    )
                ]
                + [gr.File.update(visible=False)]
                + [None]
        ...
        for scale in list(self.scale_definitions.keys()):
            measures = (
                util_funcs.pytorch_cos_sim(
                    self.embeddings, self.scale_embeddings[scale]
                )
                .cpu()
                .numpy()
            )
...
def run_gui(
    ...
    **kwargs,
    ...
    if mode == "public":
        ...
        demo.launch(favicon_path=path_mgt.sample_data_dir / "favicon.png", **kwargs)
    else:
