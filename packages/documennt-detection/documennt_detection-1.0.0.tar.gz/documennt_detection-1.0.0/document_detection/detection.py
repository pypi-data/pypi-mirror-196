...
class Detection:
    def __init__(self):
        #self.device_dict = dict() #{"device_name":"device_standard_name"}
        self.device_dict = {"综合告警灯盘":"综合告警灯盒", "活动光镜":"反光镜", "塞尺":"千分尺","综合告警灯盒":"综合告警灯盒","反光镜":"反光镜", "千分尺":"千分尺"}
        self.device_names = set(self.device_dict.keys())
        self.state_words = utils.read_words("state_word.txt")
        self.scope_words = utils.read_scope_words("scope_table")
        self.scope_words1 = "|".join(self.scope_words["case1"])
        self.scope_words2 = "|".join(self.scope_words["case2"])
