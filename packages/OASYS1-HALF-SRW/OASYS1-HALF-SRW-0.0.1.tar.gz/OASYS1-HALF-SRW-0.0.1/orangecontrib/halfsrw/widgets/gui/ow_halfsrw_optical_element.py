...
from orangecontrib.halfsrw.util.halfsrw_objects import HALFSRWData
from orangecontrib.halfsrw.widgets.gui.ow_halfsrw_wavefront_viewer import HALFSRWWavefrontViewer
from wofrysrw.beamline.optical_elements.srw_optical_element import Orientation


class OWHALFSRWOpticalElement(HALFSRWWavefrontViewer, WidgetDecorator):

    #maintainer = "Luca Rebuffi"
    #maintainer_email = "lrebuffi(@at@)anl.gov"
    keywords = ["data", "file", "load", "read"]
    category = "HALFSRW Optical Elements"

    outputs = [{"name":"HALFSRWData",
                "type":HALFSRWData,
                "doc":"HALFSRW Optical Element Data",
                "id":"data"},
               {"name":"Trigger",
                "type": TriggerIn,
    ...
    inputs = [("HALFSRWData", HALFSRWData, "set_input"),
    ...
    def show_propagator_info(self):
        try:
            dialog = OWHALFSRWOpticalElement.PropagatorInfoDialog(parent=self)
            dialog.show()
        ...
        try:
            if trigger and trigger.new_object == True:
                if trigger.has_additional_parameter("variable_name"):
                    if self.input_halfsrw_data is None: raise Exception("No Input Data")
                    ...
                    self.input_halfsrw_data.get_halfsrw_wavefront().setScanningData(SRWWavefront.ScanningData(variable_name, variable_value, variable_display_name, variable_um))
    ...
    def propagate_wavefront(self):
        try:
            ...
            input_wavefront = self.input_halfsrw_data.get_halfsrw_wavefront()
            halfsrw_beamline = self.input_halfsrw_data.get_halfsrw_beamline().duplicate()
            working_halfsrw_beamline = self.input_halfsrw_data.get_working_halfsrw_beamline().duplicate()
            ...
            halfsrw_beamline.append_beamline_element(beamline_element)
            working_halfsrw_beamline.append_beamline_element(beamline_element)

            self.progressBarSet(20)

            if propagation_mode == SRWPropagationMode.WHOLE_BEAMLINE:
                self.set_additional_parameters(beamline_element, None, halfsrw_beamline)
                self.set_additional_parameters(beamline_element, None, working_halfsrw_beamline)
                ...
                if hasattr(self, "is_final_screen") and self.is_final_screen == 1:
                    ...
                    propagation_parameters.set_additional_parameters("working_beamline", working_halfsrw_beamline)
                    ...
                    output_halfsrw_data = HALFSRWData(halfsrw_beamline=halfsrw_beamline,
                                              halfsrw_wavefront=output_wavefront)
                    output_halfsrw_data.reset_working_halfsrw_beamline()
                else:
                    output_wavefront = None

                    output_halfsrw_data = HALFSRWData(halfsrw_beamline=halfsrw_beamline,
                                              halfsrw_wavefront=input_wavefront)
            else:
                ...
                self.set_additional_parameters(beamline_element, propagation_parameters, halfsrw_beamline)
            ...
            if not output_wavefront is None:
                output_wavefront.setScanningData(self.input_halfsrw_data.get_halfsrw_wavefront().scanned_variable_data)
            ...
            self.send("HALFSRWData", output_halfsrw_data)
    ...
    def set_additional_parameters(self, beamline_element, propagation_parameters=None, beamline=None):
        ...
        halfsrw_drift_before_wavefront_propagation_parameters = None
        halfsrw_drift_before_wavefront_propagation_optional_parameters = None

        # DRIFT BEFORE
        if beamline_element.get_coordinates().p() != 0:
            halfsrw_drift_before_wavefront_propagation_parameters = WavefrontPropagationParameters(
            ...
            if not propagation_parameters is None: propagation_parameters.set_additional_parameters("halfsrw_drift_before_wavefront_propagation_parameters", halfsrw_drift_before_wavefront_propagation_parameters)

            if self.has_drift_before_wavefront_propagation_optional_parameters():
                halfsrw_drift_before_wavefront_propagation_optional_parameters = WavefrontPropagationOptionalParameters(
                ...
                if not propagation_parameters is None: propagation_parameters.set_additional_parameters("halfsrw_drift_before_wavefront_propagation_optional_parameters", halfsrw_drift_before_wavefront_propagation_optional_parameters)

        if not beamline is None: beamline.append_wavefront_propagation_parameters(halfsrw_drift_before_wavefront_propagation_parameters, halfsrw_drift_before_wavefront_propagation_optional_parameters, Where.DRIFT_BEFORE)

        # OE
        halfsrw_oe_wavefront_propagation_parameters = None
        halfsrw_oe_wavefront_propagation_optional_parameters = None

        if self.has_oe_wavefront_propagation_parameters_tab:
            halfsrw_oe_wavefront_propagation_parameters = WavefrontPropagationParameters(
            ...
            if not propagation_parameters is None: propagation_parameters.set_additional_parameters("halfsrw_oe_wavefront_propagation_parameters", halfsrw_oe_wavefront_propagation_parameters)

            if self.has_oe_wavefront_propagation_optional_parameters():
                halfsrw_oe_wavefront_propagation_optional_parameters = WavefrontPropagationOptionalParameters(
                ...
                if not propagation_parameters is None: propagation_parameters.set_additional_parameters("halfsrw_oe_wavefront_propagation_optional_parameters", halfsrw_oe_wavefront_propagation_optional_parameters)

        if not beamline is None: beamline.append_wavefront_propagation_parameters(halfsrw_oe_wavefront_propagation_parameters, halfsrw_oe_wavefront_propagation_optional_parameters, Where.OE)

        # DRIFT AFTER
        halfsrw_drift_after_wavefront_propagation_parameters = None
        halfsrw_drift_after_wavefront_propagation_optional_parameters = None

        if beamline_element.get_coordinates().q():
            halfsrw_drift_after_wavefront_propagation_parameters = WavefrontPropagationParameters(
            ...
            if not propagation_parameters is None: propagation_parameters.set_additional_parameters("halfsrw_drift_after_wavefront_propagation_parameters", halfsrw_drift_after_wavefront_propagation_parameters)

            if self.has_drift_after_wavefront_propagation_optional_parameters():
                halfsrw_drift_after_wavefront_propagation_optional_parameters = WavefrontPropagationOptionalParameters(
                ...
                if not propagation_parameters is None: propagation_parameters.set_additional_parameters("halfsrw_drift_after_wavefront_propagation_optional_parameters", halfsrw_drift_after_wavefront_propagation_optional_parameters)

        if not beamline is None: beamline.append_wavefront_propagation_parameters(halfsrw_drift_after_wavefront_propagation_parameters, halfsrw_drift_after_wavefront_propagation_optional_parameters, Where.DRIFT_AFTER)
    ...
    def set_input(self, halfsrw_data):
        if not halfsrw_data is None:
            self.input_halfsrw_data = halfsrw_data
    ...
    def run_calculation_for_plots(self, tickets, progress_bar_value):
        if self.view_type==2:
            ...
            HALFSRWWavefrontViewer.add_2D_wavefront_plot(e, h, v, i, tickets)
            ...
            HALFSRWWavefrontViewer.add_2D_wavefront_plot(e, h, v, p, tickets, int_phase=1)
