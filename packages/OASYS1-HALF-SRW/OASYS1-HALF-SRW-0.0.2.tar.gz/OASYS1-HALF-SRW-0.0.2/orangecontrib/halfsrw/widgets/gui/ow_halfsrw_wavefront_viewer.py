...
from orangecontrib.halfsrw.util.halfsrw_util import HALFSRWPlot
from orangecontrib.halfsrw.widgets.gui.ow_halfsrw_widget import HALFSRWWidget
...
def initialize_propagator_2D():
    ...
    if not propagation_manager.is_initialized(SRW_APPLICATION):
        ...
        propagation_mode = QSettings().value("output/halfsrw-default-propagation-mode", 1, int)
...
class HALFSRWWavefrontViewer(HALFSRWWidget):
    ...
    def __init__(self, show_general_option_box=True, show_automatic_box=True, show_view_box=True):
        ...
        self.le_halfsrw_live_propagation_mode = gui.lineEdit(propagation_box, self, "halfsrw_live_propagation_mode", "Propagation Mode", labelWidth=150, valueType=str, orientation="horizontal")
        self.le_halfsrw_live_propagation_mode.setAlignment(Qt.AlignCenter)
        self.le_halfsrw_live_propagation_mode.setReadOnly(True)
        font = QFont(self.le_halfsrw_live_propagation_mode.font())
        font.setBold(True)
        self.le_halfsrw_live_propagation_mode.setFont(font)

        self.set_halfsrw_live_propagation_mode()
        ...
        self.halfsrw_output = oasysgui.textArea(580, 800)

        out_box = gui.widgetBox(out_tab, "System Output", addSpace=True, orientation="horizontal")
        out_box.layout().addWidget(self.halfsrw_output)

        self.set_PlotQuality()

    def set_halfsrw_live_propagation_mode(self):
        propagation_mode = PropagationManager.Instance().get_propagation_mode(SRW_APPLICATION)

        self.halfsrw_live_propagation_mode = "Element by Element (Native)" if              propagation_mode == SRWPropagationMode.STEP_BY_STEP  else \
        ...
        self.le_halfsrw_live_propagation_mode.setPalette(palette)
    ...
    def add_2D_wavefront_plot(cls, e, h, v, i, tickets, int_phase=0):
        if len(e) <= 1:
            tickets.append(HALFSRWPlot.get_ticket_2D(h * 1000, v * 1000, i[int(e.size / 2)]))
        else:
            if int_phase == 0:
                delta_e = e[1] - e[0]
                for j in range(len(e)): i[j, :, :] *= delta_e / (e[j] * 1e-3) # change to fixed bw for integration
                tickets.append(HALFSRWPlot.get_ticket_2D(h * 1000, v * 1000, numpy.sum(i, axis=0), is_multi_energy=True))
            else:
                tickets.append(HALFSRWPlot.get_ticket_2D(h * 1000, v * 1000, numpy.average(i, axis=0), is_multi_energy=True))
    ...
    def plot_1D(self, ticket, progressBarValue, var, plot_canvas_index, title, xtitle, ytitle, xum=""):
        if self.plot_canvas[plot_canvas_index] is None:
            self.plot_canvas[plot_canvas_index] = HALFSRWPlot.Detailed1DWidget(do_average=self.do_average)
    ...
    def plot_2D(self, ticket, progressBarValue, var_x, var_y, plot_canvas_index, title, xtitle, ytitle, xum="", yum="", ignore_range=False, apply_alpha_channel=False, alpha_ticket=None, do_unwrap=False):
        ...
        if self.plot_canvas[plot_canvas_index] is None:
            self.plot_canvas[plot_canvas_index] =  HALFSRWPlot.Detailed2DWidget(do_average=self.do_average)
        ...
        if self.is_do_plots():
            if not tickets is None:
                if not len(tickets) == 0:
                    ...
                    HALFSRWPlot.set_conversion_active(self.getConversionActive())
    ...
    def writeStdOut(self, text):
        cursor = self.halfsrw_output.textCursor()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.insertText(text)
        self.halfsrw_output.setTextCursor(cursor)
        self.halfsrw_output.ensureCursorVisible()
