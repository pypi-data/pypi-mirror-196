...
class OWHALFSRWSource(HALFSRWWavefrontViewer, WidgetDecorator):
    ...
    def __init__(self):
        ...
        self.runaction = widget.OWAction("Run HALFSRW", self)
        self.runaction.triggered.connect(self.runHALFSRWSource)
        self.addAction(self.runaction)

        button_box = oasysgui.widgetBox(self.controlArea, "", addSpace=False, orientation="horizontal")

        button = gui.button(button_box, self, "Run HALFSRW Source", callback=self.runHALFSRWSource)
    ...
    def runHALFSRWSource(self):
        ...
        try:
            ...
            halfsrw_source = self.get_halfsrw_source(self.get_electron_beam())
            halfsrw_source.name = self.source_name if not self.source_name is None else self.windowTitle(),

            self.progressBarSet(10)

            self.setStatusMessage("Running HALFSRW")

            sys.stdout = EmittingStream(textWritten=self.writeStdOut)

            self.print_specific_infos(halfsrw_source)

            self.progressBarSet(20)

            self.setStatusMessage("")

            beamline = SRWBeamline(light_source=halfsrw_source)
            self.output_wavefront = self.calculate_wavefront_propagation(halfsrw_source)
            ...
            self.send("HALFSRWData", HALFSRWData(halfsrw_beamline=beamline, halfsrw_wavefront=self.output_wavefront))
    ...
    def get_halfsrw_source(self, electron_beam=ElectronBeam()):
    ...
    def calculate_wavefront_propagation(self, halfsrw_source):
        ...
        return halfsrw_source.get_HALFSRW_Wavefront(source_wavefront_parameters=wf_parameters)

    def get_photon_energy_for_wavefront_propagation(self, halfsrw_source):
