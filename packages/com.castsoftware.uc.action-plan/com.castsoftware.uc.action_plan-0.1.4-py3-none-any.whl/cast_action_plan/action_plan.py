...
class ActionPlan():
    ...
    def _open_database (self, config:Config) -> bool:
        try:
            conn = connect(database=config.db_database, user=config.db_user, password=config.db_password, host=config.db_host, port=config.db_port)
    ...
    def fetch_data(self, app_name):
        ...
        #delete the content of thfge viewer_action_plan
