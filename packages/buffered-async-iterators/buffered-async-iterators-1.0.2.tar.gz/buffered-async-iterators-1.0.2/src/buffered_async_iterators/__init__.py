"""
Buffers iterables, allowing iterations to be ran asynchronously with
the loop that is currently running it.
...
When using asynchronous generators, the generator is only ran between
iterations rather than during iterations. This results in untapped
potential, potentially doubling the speed or more when running
iterations can be done asynchronously with the current iteration.
...
Syntax:
    ...
    >>> # Best if the iterable does not run significantly faster than
    >>> # the loop.
    >>> async for x in buffered(async_iterable):
    ...     await loop_process(x)
    ... 
    >>> # Buffers arbitrarily far ahead.
    >>> # Best if the iterable can run faster than the loop and there
    >>> # are no issues with running ahead e.g. no issues with
    >>> # concurrency or memory.
    >>> async for x in buffered(async_iterable, 0):
    ...     await loop_process(x)
    ... 
    >>> # Buffers up to n iterations ahead.
    >>> # Best if the iterable can run faster than the loop but there
    >>> # are issues with running too far ahead e.g. concurrency or
    >>> # memory.
    >>> async for x in buffered(async_iterable):
    ...     await loop_process(x)
    ... 

In the following example, the addition of the buffer allows items to be
processed twice as fast.
    ...
    ...         await asyncio.sleep(1)  # Something slow like an API
    ...                                 # request.
    ...
    ...         await asyncio.sleep(1)  # Something slow like a
    ...                                 # database query.
...
There is a clear difference when running the two. Without the buffer,
the generator cannot run at the same time as its consumer is processing
