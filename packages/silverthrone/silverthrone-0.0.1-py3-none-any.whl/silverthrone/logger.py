...
class Logger:
  ...
  cri_prefix = f'{re}{dr}CRITICAL {separator}'
  ...
  def __init__(self, level: Literal['error', 'warn', 'info', 'debug'] = "info", /, hide_time:bool=False, timestamp_format:str="%m/%d/%YT%H:%M:%S"):
    if level == 'error':
        self.level = 0
    elif level == 'warn':
        self.level = 1
  ...
  def critical(self, message):
    print(f'{self.__time()}{self.cri_prefix}{message}{self.re}')
