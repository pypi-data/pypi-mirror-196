...
def graph_transaction_handler(eff: dict):
    ...
    # We switch over to assuming everything is new style (graph_additions)
    assert "level2_commands" in eff
    ...
    if isinstance(eff["target_glike"], FlatGraph) or (isinstance(eff["target_glike"], GraphRef) and internals.is_transactor(eff["target_glike"])):
        ...
        assert isinstance(eff["target_glike"], GraphRef)
        payload = {"cmds": eff["level2_commands"]["cmds"],
                               "keep_internal_ids": eff.get("keep_internal_ids", False)}
        glike_frame,receipt = merge(payload, eff["target_glike"])
