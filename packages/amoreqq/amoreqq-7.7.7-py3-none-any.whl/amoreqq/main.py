...
from .models import amoreqq_api
from .models.exceptions import amoreqq_api as amoreqq_api_exceptions
...
class BaseAnimeConverter(pydantic.BaseModel):
    ...
    def _get_request_body(self, picture: bytes):
        ...
        extra = amoreqq_api.AIProcessorRequestBody.Extra()
        if self.generate_api_version is not None:
            extra.version = self.generate_api_version

        return amoreqq_api.AIProcessorRequestBody(
            images=[picture_b64],
    ...
    @staticmethod
    def _raise_exception_from_response(response_raw: httpx.Response, response_parsed: amoreqq_api.AIProcessorResponseBody):
        if response_parsed.valid:
            return

        exception = amoreqq_api_exceptions.get_exception_from_response_code(response_parsed.code)
        raise exception(
...
class AnimeConverter(BaseAnimeConverter):
    ...
    def convert(
        ...
        response = amoreqq_api.AIProcessorResponseBody.parse_obj(response_body)
