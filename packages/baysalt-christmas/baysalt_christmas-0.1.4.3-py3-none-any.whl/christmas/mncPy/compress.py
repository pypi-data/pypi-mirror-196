...
from christmas.mncPy.common import get_file_paths, GlobalData
...
class mnc_Py:
    def __init__(self, f_path, x_excl=None, v_log=False, o_path=False, d_rm=False):
        self.f = f_path
        self.x = x_excl
        self.v = v_log
    ...
    def compress_files(self):
        ...
        filePaths = get_file_paths(self.f, self.comm.Get_rank(), self.x, self.v)
        ...
        if self.comm.Get_rank()==0 and self.v:
        ...
        lfiles = []
        for i in range(self.comm.Get_rank()*f_per_proc, min(nfiles, (self.comm.Get_rank()+1)*f_per_proc)):
        ...
        for lfile in lfiles:
            ...
            if self.v:
                print("rank:", self.comm.Get_rank(), "\tcompressing", lfile.name, "(", lfiles.index(lfile)+1, "of", len(lfiles), ")")
...
if __name__ == "__main__":
    args = parse_args()
    mnc = mnc_Py(args.f, args.x, args.v)
    comm = mnc.MPI_comm()
    if comm.Get_rank()==0:
        print(f"running with {str(comm.Get_size())} processors...")
    mnc.compress_files()
