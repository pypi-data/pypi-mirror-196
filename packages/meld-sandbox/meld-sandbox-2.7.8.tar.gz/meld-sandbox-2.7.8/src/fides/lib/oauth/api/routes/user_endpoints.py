...
def create_user(
    *,
    ...
    org_check: Optional[Organization] = Organization.get_by(
        db, field="organization_fides_key", value=user_data.organization_fides_key
    )

    if not org_check:
        raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="No organization found for: "+user_data.organization_fides_key)
...
def delete_user(
    *,
    ...
    if not user:
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail=f"No user found with id {user_id}."
        )
...
def user_login(
    *,
    ...
    if (
        ...
        logger.info(org_check)

        if not org_check:
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="No organization found for: "+user_data.organization_fides_key)

        user_org_check: Optional[FidesUser] = FidesUser.get_by(
            db, field="organization_fides_key", value=user_data.organization_fides_key
        )
        # This will check if user exist to that organization
        if not user_org_check:
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="No user found in organization: "+user_data.organization_fides_key)
        ...
        if not user_check:
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="No user found.")
