...
# Adding New API for getting list of users with organization (Vincent - 03/07/2023)
@router.get(
    urls.USER_ORGANIZATION,
    status_code=HTTP_200_OK,
    response_model=UserNewResponse,
    dependencies=[Security(verify_oauth_client, scopes=[USER_READ])],
)
def getuser_organization(
    *,
    ...
    """
    Return list of user by organization key
    """
    org_check: Optional[Organization] = Organization.get_by(
        db, field="organization_fides_key", value=organization_fides_key
    )
    logger.info(org_check)

    if not org_check:
        raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="No organization found: "+organization_fides_key)
    
    query = db.query(FidesUser).options(joinedload(FidesUser.client)).filter_by(organization_fides_key=organization_fides_key)
    ...
    user_responses = []
    for user in users:
        user_responses.append(UserResponse.from_orm(user))
    
    total = len(user_responses)
    page = 1
    size = 50
    
    return UserNewResponse(items=user_responses, total=total, page=page, size=size)
