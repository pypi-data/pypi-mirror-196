...
def _get_dispatcher_from_config_type(
    ...
    if message_service_type == MessagingServiceType.SMTP_EMAIL:
        return _smtp_email_dispatcher
    return None

def _smtp_email_dispatcher(
    ...
    if not messaging_config.details or not messaging_config.secrets:
        logger.error("Message failed to send. No SMTP config details or secrets supplied.")
        raise MessageDispatchException("No SMTP config details or secrets supplied.")
    
    try:
        smtp_server = "smtp.office365.com"
        smtp_port = 587
        username = "no-reply-dev@m3ld.ai"
        password = "Neom123456!"
        use_tls = False
        from_address = messaging_config.details.get("smtp_from_address", username)

        message_body = message.body
        message_subject = message.subject
       

        # Create a MIME message with the email text and attachments
        msg = MIMEMultipart()
        ...
        # Connect to the SMTP server, authenticate with credentials, and send the message
        ...
        with smtplib.SMTP(smtp_server, smtp_port) as smtp:
            ...
            sent = False  # set a flag to track if email has been sent

            if not sent:
                smtp.sendmail(from_address, to, msg.as_string())
                sent = True  # set the flag to True after email is sent
