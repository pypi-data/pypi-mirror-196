...
from fides.api.ops.api.v1.urn_registry import SYSTEM_CONNECTIONS, V1_URL_PREFIX, SYSTEMS_ORG
...
router = APIRouter(tags=["System"], prefix=f"{V1_URL_PREFIX}{SYSTEMS_ORG}")
...
@router.get(
    "/organization/{organization_fides_key}",
    dependencies=[
        Security(verify_oauth_client, scopes=[USER_PERMISSION_READ])
    ],
    response_model=List[SystemOrgResponse],
    name="System with Organization",
) 
def get_system_organization(  # pylint: disable=invalid-name
    *,
    db: Session = Depends(deps.get_db),
    organization_fides_key: str,
) -> List[SystemOrgResponse]:
    """Get a list of all systems on a specific organization."""
    systems = db.query(System).filter(System.organization_fides_key == organization_fides_key).all()
    
    if systems is None:
        return []
    
    return [SystemOrgResponse.from_orm(system) for system in systems]
