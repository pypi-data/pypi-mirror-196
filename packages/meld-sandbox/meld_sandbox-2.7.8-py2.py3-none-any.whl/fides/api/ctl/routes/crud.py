...
from fides.api.ops.util.oauth_util import verify_oauth_client # verify_oauth_client_cli (not working on production)
...
for model_type, fides_model in model_map.items():
    ...
    @router.post(
        ...
        dependencies=[
            Security(
                verify_oauth_client, scopes=[scope_registry.CLI_OBJECTS_CREATE]
            )
        ],
        responses={
    ...
    @router.get(
        "/",
        dependencies=[
            Security(verify_oauth_client, scopes=[scope_registry.CLI_OBJECTS_READ])
        ],
    ...
    @router.put(
        ...
        dependencies=[
            Security(
                verify_oauth_client, scopes=[scope_registry.CLI_OBJECTS_UPDATE]
            )
        ],
        responses={
            status.HTTP_403_FORBIDDEN: {
    ...
    @router.delete(
        ...
        dependencies=[
            Security(
                verify_oauth_client, scopes=[scope_registry.CLI_OBJECTS_DELETE]
            )
        ],
        responses={
            status.HTTP_403_FORBIDDEN: {
