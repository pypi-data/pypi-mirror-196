...
def init_wandb(offline=False):
    config = get_few_shot_params()
    wandb.login()
    if offline:
        wandb.init(WANDB_MODE= "dryrun" , project=f"few-shot-stance-{config['few-shot-size']}", config=config, name=f"{config['model-name']}-batch-size-{config['batch-size']}-learning-rate-{config['learning-rate']}")
    else:
...
def save_pre_trained_model():
    ...
    path = Path(root_path, config["path"])
