...
def get_random_str(count = 4):
    import random
    import string
    return ('%s' % ''.join(random.sample((string.ascii_lowercase + string.digits), count)))
...
def download_file_from_url(source, save_filename = None, save_filepath = None):
    ...
    if (url.scheme is None) or (url.hostname is None):
    ...
    _, filename, name, suffix = get_path_name_suffix_from_filepath(save_filename)
    ...
    if (save_filepath == None):
        ...
        save_filepath = os.path.join(save_filepath, filename)
