...
def visualize_top_components(work_dir, save_path, num_components, get_label_func=None, group_by='visual', slice=None,
    ...
    if get_label_func is not None:
        top_components.loc[:, 'label'] = all_labels
...
def do_create_similarity_gallery(similarity_file, save_path, num_images=20, lazy_load=False, get_label_func=None,
    ...
    if slice is None or slice != 'label_score':
        ...
        for i, row in tqdm(subdf.iterrows(), total=len(subdf)):
            id_from = row['from']
            if not isinstance(id_from, str):
                filename = filename_transform_func(id_from)
                assert isinstance(filename, str), "Failed to convert {id_fro} to image filename"
            else:
                filename = id_from
    ...
    if 'id_to_filename_func' in kwargs['kwargs']:
        id_to_file = kwargs['kwargs']['id_to_filename_func']
    ...
    for i, row in tqdm(subdf.iterrows(), total=min(num_images, len(subdf))):
        try:
            ...
            img = my_resize(img, max_width)
            if 'enhance_image' in kwargs['kwargs'] and kwargs['kwargs']['enhance_image']:
    ...
    import fastdup.html_writer
    img_html1 = format_image_html_string(from_paths, lazy_load, max_width, save_path)
    img_html2 = format_image_html_string(img_paths2, lazy_load, None, save_path)
...
if __name__ == "__main__":
    ...
    ret = fd.vis.similarity_gallery(load_crops=True,slice='label_score',ascending=True,num_images=100,enhance_image=True)