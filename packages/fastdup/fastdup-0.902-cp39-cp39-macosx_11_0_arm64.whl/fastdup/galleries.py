...
def visualize_top_components(work_dir, save_path, num_components, get_label_func=None, group_by='visual', slice=None,
    ...
    for i,row in tqdm(top_components.iterrows(), total = len(top_components)):
        try:
            ...
            if v1 and isinstance(files_ids[0], int):
                files = [filname_transform_func(x) for x in files_ids]
            elif v1 and isinstance(files_ids[0], str):
                files = [os.path.join(input_dir, x) for x in files_ids]
            else:
                files = files_ids
...
if __name__ == "__main__":
    ...
    #ret = fd.vis.component_gallery(load_crops=True,slice='diff',ascending=True,num_images=30,enhance_image=True,keep_aspect_ratio=True)
