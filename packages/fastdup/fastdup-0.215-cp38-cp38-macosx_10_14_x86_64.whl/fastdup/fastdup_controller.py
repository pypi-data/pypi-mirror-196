...
class FastdupController:
    ...
    def connected_components(self, data: bool = True, split: Union[str, List[str]] = None,
        ...
        assert df_info is not None and not df_info.empty, f"Missing file {FD.CC_INFO_CSV}"
    ...
    def _expand_annot_df(self):
        ...
        if self._run_mode == FD.MODE_CROP or self._run_mode == FD.MODE_ROTATED_BBOX:
            ...
            assert df_crops_annot is not None and not df_crops_annot.empty, f"Failed to find {df_crops_annot} in work_dir"
            assert 'index' in df_crops_annot.columns, f"Failed to find 'index' column in {df_crops_annot}"
        ...
        if self._run_mode == FD.MODE_CROP or self._run_mode == FD.MODE_ROTATED_BBOX:
            ...
            invalid_crop[FD.ANNOT_ERROR] = 'INVALID_CROP'
        ...
        df_mapping = self._fetch_df(FD.MAPPING_CSV)
        assert df_mapping is not None and not df_mapping.empty,  f"Failed to find {df_mapping} in work_dir"
