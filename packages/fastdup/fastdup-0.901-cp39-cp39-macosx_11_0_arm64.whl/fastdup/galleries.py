    ...
    for i, row in tqdm(subdf.iterrows(), total=min(num_images, len(subdf))):
        try:
            ...
            for im, imgpath2, to_id in zip(img2, to_impaths_, to_ids):
                ...
                imgpath2 = calc_image_path(lazy_load, save_path, imgpath2, filename_suffix=image_suffix)
                if 'enhance_image' in kwargs and kwargs['enhance_image']:
                    im = enhance_image(im)
...
if __name__ == "__main__":
    ...
    ret = fd.vis.similarity_gallery(load_crops=True,slice='label_score',ascending=True,num_images=100,enhance_image=True,keep_aspect_ratio=True)
