...
class FinanceModuleHandler:
    ...
    def gets_loan_account(self):
        return Loanaccount.objects.get(customer = self.anchor_party , program_type = self.program_type , currency = self.finance_currency)
    ...
    def calculated_interest_amount(self , amount ):
        ...
        if self.interest_type == InterestTypeChoices.FIXED:
            ...
            if calculated_date > list(interest_rate_type.period.keys())[0].split('days')[0]:
                interest_rate = self.margin + list(interest_rate_type.period.values())[0]
            # 60 days
            if calculated_date > list(interest_rate_type.period.keys())[1].split('days')[0]:
                interest_rate = self.margin + list(interest_rate_type.period.values())[1]
            # 180 
            if calculated_date > list(interest_rate_type.period.keys())[2].split('days')[0]:
                interest_rate = self.margin + list(interest_rate_type.period.values())[2]
            # 360
            if calculated_date > list(interest_rate_type.period.keys())[3].split('days')[0]:
                interest_rate = self.margin + list(interest_rate_type.period.values())[3]
    ...
    def gets_interest_account(self):
        return Interestaccount.objects.get(program_type=self.program_type , currency=self.finance_currency or self.base_currency)
    ...
    def repayment(self):
        ...
        finance_model.settlement_amount = self.repayment_amount + finance_model.settlement_amount
        finance_model.save()
        ...
        return current_interest_amount[0] , current_interest_amount[1] , finance_model.finance_date , finance_model.id
