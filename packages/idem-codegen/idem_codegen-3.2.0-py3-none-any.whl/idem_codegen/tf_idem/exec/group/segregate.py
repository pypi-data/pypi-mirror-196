...
def convert_tf_file_to_sls(
    ...
    for tf_resource in resources:
        ...
        for instance in tf_state_for_resource["instances"]:
            ...
            tags_all = attributes.get("tags_all")

            diff_in_tags = {}
            if tags and tags_all:
                diff_in_tag_keys = set(tags_all) - set(tags)
                if diff_in_tag_keys:
                    for diff_in_tag_key in diff_in_tag_keys:
                        diff_in_tags[diff_in_tag_key] = tags_all[diff_in_tag_key]
            ...
            if sls_resource:
                ...
                if diff_in_tags:
                    tf_resource_attr = tf_resource[tf_resource_type][tf_resource_name]
                    if "tags" in tf_resource_attr:
                        tf_resource_attr["tags"].update(diff_in_tags)
                idem_resource_id_tf_resource_map[
