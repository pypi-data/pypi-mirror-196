...
from . import job_run_statuses
from .api import jobs
from .backend_connection import BackendConnection
from .estimate_result import EstimateResult
from .exceptions import BQJobCouldNotCancelError, BQJobNotCompleteError
...
class BQClient:
    ...
    def run(
        ...
        if submitted_job.run_status == "FAILED_VALIDATION":
            raise BQJobNotCompleteError(submitted_job.job_id, submitted_job.run_status)
        else:
    ...
    def wait(self, job_id: str) -> JobResult:
        ...
        while True:
            ...
            if job_result.run_status in job_run_statuses.JOB_RESULTS_READY_STATES:
                return job_result
            elif (
                job_result.run_status in job_run_statuses.JOB_NO_RESULT_TERMINAL_STATES
            ):
                raise BQJobNotCompleteError(job_id, job_result.run_status)
            else:
                time.sleep(1.0)
    ...
    def cancel(self, job_id: str) -> JobResult:
        ...
        try:
            ...
            if not e.run_status == "CANCELED":
                raise BQJobCouldNotCancelError(e.job_id, e.run_status) from None
