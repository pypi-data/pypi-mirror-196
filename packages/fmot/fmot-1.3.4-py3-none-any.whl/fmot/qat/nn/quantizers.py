...
from ..annotated_tensors import annotate, copy_dim_annotations, set_dim_annotations, tag_dim, ANNOS
...
class FixedRangeWrappedObserver(ObserverBase):
    ...
    def _calculate_quanta(self, bits, verbose=False):
        ...
        lmin, lmax = [torch.ones_like(q_w)*l if l is not None else None for l in self.limits]
        ...
        if lmax is not None:
            if vmax > lmax:
                vmax = lmax
        quanta_neg = torch.ceil(torch.log2(-vmin) + 1 - bits)
        if self.hard_maximum:
...
class GaussianObserver(ObserverBase):
    ...
    def reset(self):
        ...
        self.register_buffer('running_x', torch.tensor(0., device=dev))
        self.register_buffer('running_x2', torch.tensor(0., device=dev))
        self.register_buffer('maxabs', torch.tensor(0., device=dev))
    ...
    def _calculate_quanta(self, bits, verbose=False):
        ...
        if qopt is None:
            qopt = qinit
            print(f'WARNING: GaussianObserver set qopt to None, qinit={qinit}, quantas={quantas}')
        return qopt
...
class Quantizer(nn.Module):
    ...
    """
    def __init__(self, bitwidth, observer=DEFAULT_OBSERVERS['default'],
                 rounded=False, dimensions=None, **observer_kwargs):
        super().__init__()
