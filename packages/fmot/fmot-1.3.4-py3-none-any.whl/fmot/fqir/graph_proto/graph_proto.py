...
class GraphProto:
    ...
    * :obj:`DEQUANT`: This subgraph defines how to cast integer outputs to floating-point
    ...
    def printout(self, constants=True, subgraphs=True, indent=None,
        energy=False):
        ...
        if len(self.parameters) > 0:
            ...
            rep += '\n'
        rep += 'nodes:\n' + indent
        rep += f'\n{indent}'.join([n.printout(
            constants=constants, subgraph=subgraphs, indent=indent,
            energy=energy) for n in self.nodes])
    ...
    def run(self, *inputs, return_objs=False, dequant=False, state=None,
            return_dict=False, reshape=True):
        ...
        if 'INIT' in self.subgraphs:
            return run_sequential_graph(
                self, *inputs, return_objs=return_objs, dequant=dequant,
                objs=state, return_dict=return_dict)
        else:
            return run_graph(
                self, *inputs, return_objs=return_objs, dequant=dequant,
                objs=state, return_dict=return_dict)
    ...
    def iterate_inputs(self, *args, return_dict=False, quant=True, reshape=True):
        ...
        if 'INIT' in self.subgraphs:
            assert np.max(ndims) == 2, f"Please remove the batch dimension from your inputs. dims: {ndims}"
        else:
            assert np.max(ndims) == 1, f"Please remove the batch dimension from your inputs. dims: {ndims}"
        if np.max(ndims) == 2:
    ...
    def dequantize(self, *args):
        ...
        assert 'DEQUANT' in self.subgraphs, \
            "This graph doesn't have a dequant subgraph, try the main graph"
        if len(args) == 1:
            args = args[0]
        if isinstance(args, dict):
            tuple_in = tuple([args[tproto.name] for tproto in self.subgraphs['DEQUANT'].inputs])
    ...
    def energy(self, st_ld_pessimism=None, config=None):
        raise NotImplementedError('Energy estimation not enabled -- please use the behavioral simulator instead.')
    ...
    def sorted_energies(self, group_by='op', st_ld_pessimism=None):
        opcounts = self.sorted_opcounts(group_by=group_by, 
            st_ld_pessimism=st_ld_pessimism)
