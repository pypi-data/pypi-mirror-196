...
def set_tolerances(mode: Literal['overwrite', 'max', 'min'], alpha: float=1.5,
        force=False):
    
    print(f'Setting new quantization error tolerances in "{mode}" mode')

    print(f'Measuring quantization errors..., new tolerances values will be {alpha:.2f} x current errors')
    ...
    if mode == 'overwrite':
        print('WARNING: if you continue, all quantization tolerances will be set to new values')
    ...
    # raise bounds on errors
        ...
        for name, newval in new_tols.items():
            ...
            if curval is not None:
                if (
                    newval > curval and 
                    (force or ask_to_continue(f'Raise tolerance of {name} from {curval:.3E} to {newval:.3E}?'))
                ):
                    pass
