...
class Std(nn.Module):
    ...
    def forward(self, x):
        var = torch.var(x, dim=self.dim, keepdim=self.keepdim,
...
class DepthWiseConvSummer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bias):
        ...
        if self.bias:
            ...
            self.has_bias = [not(i % self.kernel_size) for i in range(self.linlist_len)]
        else:
            self.has_bias = [False for _ in range(self.linlist_len)]


        self.lin_list = nn.ModuleList(
            nn.Linear(1, self.in_channels, bias=self.has_bias[i]) for i in range(self.linlist_len))
...
class _BlockDiagMasking(prune.BasePruningMethod):
    ...
    def compute_mask(self, tensor, default_mask):
        ...
        assert nrows % self.num_blocks == 0, 'Number of blocks needs to divide the tensor size'
        assert ncols % self.num_blocks == 0, 'Number of blocks needs to divide the tensor size'
