...
class _Sky130(tch.Technology):
    ...
    def __init__(self):
        prims = prm.Primitives(prm.Base(type_=prm.pBaseT))
        ...
        prims += (
            ...
            *(
                ...
                for name, wire, marker, implant, in (
                    ("ndiff_res", difftap, diffres, implants["nsdm"]),
                    ("pdiff_res", difftap, diffres, implants["nsdm"]),
                    ("poly_res", poly, polyres, None),
                )
            ),
            # capacitors
            prm.MIMCapacitor(
        ...
        bipolars =  tuple(
            ...
            for name, type_, ind in (
                ("npn_05v5_w1u00l1u00", prm.npnBipolarT, npn),
                ("npn_05v5_w1u00l2u00", prm.npnBipolarT, npn),
                ("pnp_05v5_w0u68l0u68", prm.pnpBipolarT, pnp),
                ("pnp_05v5_w3u40l3u40", prm.pnpBipolarT, pnp),
            )
        )
        prims += bipolars
