...
def getVoltageRange(voltage_range_str:str) -> float:
    if voltage_range_str == '+-1V':
        return 1.0
    elif voltage_range_str == '+-2V':
        return 2.0
...
class NITB4300_ai_voltage_trace(ArrayParameter):
...
class NITB4300_ai_voltage_channel(InstrumentChannel):
    """
    ...
    def __init__(self, parent: Instrument, name: str,
        ...
        self.add_parameter('term_config',
                           ...
                           vals= vals.Enum('default','Differential'))
        ...
        self.add_parameter('trace',
                           ...
                           parameter_class=NITB4300_ai_voltage_trace)
...
class NITB4300(Instrument):
    """
    This is the qcodes driver for NITB4300 multi function data collection device.
    ...
    def __init__(self, name: str, ai_chans: List[int]=[0],
        ...
        for ch in self.ai_chans:
            chan = NITB4300_ai_voltage_channel(self, 'analog_input{}'.format(ch), ch)
    ...
    def prepare_array_measurement(self):
        ...
        if self._trigger_mode == 'start':
            ...
            total_points = points * sample_per_point
        else:
        ...
        setpoints = tuple(np.arange(points, dtype=float) * sample_per_point/sample_rate)
        ...
        # Configure buffer
        self.read_task.in_stream.input_buf_size = total_points
        self.read_task.in_stream.over_write = nidaqmx.constants.OverwriteMode.OVERWRITE_UNREAD_SAMPLES
        
        # Set the digital start trigger
        self.read_task.triggers.start_trigger.cfg_dig_edge_start_trig(trigger_source = '/{}/{}'.format(device_name, self._trigger_source),
                                                                     trigger_edge = getEdge(self._trigger_edge)
                                                                     )
        self.read_task.triggers.start_trigger.retriggerable = True
    ...
    def run_array_measurement(self):
        ...
        results = {}
        for ch in read_channels:
            results['ai{:d}'.format(ch)] = np.zeros((points,), dtype=float)
        
        values_read = np.zeros(
                    (len(read_channels), measurement_points), dtype=float)
        for i in range(repetition):
            reader.read_many_sample(
                    values_read,
                    number_of_samples_per_channel = measurement_points,
                    timeout=int(measurement_points/sample_rate)+2)
