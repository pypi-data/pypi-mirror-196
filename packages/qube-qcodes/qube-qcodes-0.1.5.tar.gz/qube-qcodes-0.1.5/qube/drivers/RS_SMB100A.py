...
class RS_SMB100A(VisaInstrument):
    """
    This is a qcodes driver for Rohde Schwarz SMB100A signal generator
    (Only few basic functionalities)
    """
    ...
    def __init__(self, name, address, **kwargs):
        ...
        self.add_parameter(name='frequency',
                           ...
                           get_parser=float,
                           set_parser=float,
                           vals=vals.Numbers(8.0e3, 41.0e9))  # it depends on the model; here use max range possible
        ...
        self.add_parameter(name='pulse_trig_impedance',
                           label='Impedance for external pulse trigger',
                           get_cmd=':PULM:TRIG:EXT:IMP?',
                           set_cmd=':PULM:TRIG:EXT:IMP G{}',
                           vals=vals.Enum('50', '10K'))
        self.add_parameter(name='pulse_trig_slope',
                           label='Polarity of the active slope of an applied trigger at the [PULSE EXT] connector',
                           get_cmd=':PULM:TRIG:EXT:SLOP?',
                           set_cmd=':PULM:TRIG:EXT:SLOP {}',
