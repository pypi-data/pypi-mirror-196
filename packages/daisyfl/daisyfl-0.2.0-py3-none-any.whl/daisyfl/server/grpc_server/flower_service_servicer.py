...
def handle_metadata(metadata: Any) -> Dict:
    dict = {}
    for m in metadata:
        # NOTE: reserved words
        if m.key not in ['cid']:
            ...
            log(WARNING, "Some keys in metadata are reserved words.")
    return dict
...
def register_client(
    ...
    client.metadata_dict = handle_metadata(context.invocation_metadata())
