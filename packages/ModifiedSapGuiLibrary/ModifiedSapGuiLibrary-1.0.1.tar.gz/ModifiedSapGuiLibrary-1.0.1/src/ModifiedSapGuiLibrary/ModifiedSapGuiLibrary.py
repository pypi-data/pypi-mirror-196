...
class ModifiedSapGuiLibrary:
    ...
    def login_sap(self, config):
        """Script For Login SAP
            return value status True if fail dan False if succes
            and messsage string
            ...
            example :
                ...
                Run Keyword If    ${status}    Fatal Error    ${msg}
        ...
        self.open_connection(config["SAPConnection"]['connection'])
        self.input_text("wnd[0]/usr/txtRSYST-BNAME",config["SAPConnection"]['user'])
        self.input_password("wnd[0]/usr/pwdRSYST-BCODE",config["SAPConnection"]['pass'])
        self.input_text("wnd[0]/usr/txtRSYST-MANDT",config["SAPConnection"]['client'])
        self.send_vkey(0)
        ...
        if status==False:
            try:
                ...
                status = True
                msg = 'Failed to login, Please create new password!'
            except:
                status = False
    ...
    def timeout_excel(self, function_module, seconds):
        """run timeout for excel, default timeout 10 seconds
        
        example :
        
            ${value}=    Timeout Excel    funct.open_macro    20
            
            Run Keyword If    ${value} == $True    Fatal Error    Macro Failed
        """        
