...
def get_decorators(cls):
    """
    Gets the decorator on the method/function
    """
    target = cls
    cls_decorators = {}

    def visit_FunctionDef(node):
        cls_decorators[node.name] = []
        for n in node.decorator_list:
            if isinstance(n, ast.Call):
                name = n.func.attr if isinstance(n.func, ast.Attribute) else n.func.id
            else:
                name = n.attr if isinstance(n, ast.Attribute) else n.id
            cls_decorators[node.name].append(name)

    node_iter = ast.NodeVisitor()
    node_iter.visit_FunctionDef = visit_FunctionDef
    node_iter.visit_AsyncFunctionDef = visit_FunctionDef
    node_iter.generic_visit(ast.parse(getsource(target)))
    return cls_decorators
...
def _do(func, decorator_name, args, kwargs, opts: Dict = None) -> Any:
    ...
    if _DecoratorsCache.has_cache(func):
        ...
        decorator_name_list = get_decorators(clz).get(func.__name__)
