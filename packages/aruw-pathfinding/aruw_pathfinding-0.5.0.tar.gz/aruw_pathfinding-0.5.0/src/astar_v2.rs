...
impl AStarV2 {
    ...
    pub fn get_path_rust(
        ...
        old_occupied_squares: Vec<Vec<(usize, usize)>>,
        debug: bool,
        ...
        old_occupied_squares.iter().flatten().for_each(
            |&pt| self.grid[pt.0][pt.1].dynamic_occupied = false
        );
...
impl AStarV2 {
    ...
    pub fn get_path(
        ...
        let path_res = Self::get_path_rust(self, start, goal, occupied_squares, old_occupied_squares, false);
        
        match path_res { // return
            Ok(path) => {
