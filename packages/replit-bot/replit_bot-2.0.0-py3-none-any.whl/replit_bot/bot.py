...
def _parse_button_commands(command, user, choice, rand_chars):
    ...
    if (request.headers["X-Replit-User-Name"] == ""):
        return render_template("index.html", html='<center><div><script authed="location.reload()" src="https://auth.util.repl.co/script.js"></script></div></center>')
    if (
...
class Bot(Client):
    ...
    def valid_command(self, resp: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        if parsed == {} or parsed["ping statement"].strip("@") != self.user.username.lower():
            return (False, {"None": None})
        return (True, parsed)
