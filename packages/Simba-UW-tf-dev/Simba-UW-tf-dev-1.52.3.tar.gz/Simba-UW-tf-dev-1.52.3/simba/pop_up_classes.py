...
class AnalyzeSeverityPopUp(PopUpMixin):
    ...
    def run(self):
        ...
        if (not self.seconds_cnt_var.get()) and (not self.frame_cnt_var.get()):
            print('SIMBA ERROR: Please select frames and/or time output metrics')
            raise ValueError('SIMBA ERROR: Please select frames and/or time output metrics')
        severity_processor = SeverityProcessor(config_path=self.config_path,
                                               settings=settings)
        severity_processor.run()
        severity_processor.save()

# _ = AnalyzeSeverityPopUp(config_path='/Users/simon/Desktop/envs/troubleshooting/Two_animals_16bps/project_folder/project_config.ini')
#

# class TimeBinsMovementPopUp(PopUpMixin):
#     def __init__(self,
#                  config_path: str):
#
#         super().__init__(config_path=config_path, title='TIME BINS: DISTANCE/VELOCITY')
#         self.create_choose_animal_cnt_dropdown()
#         self.create_time_bin_entry()
#         self.create_plots = BooleanVar(value=False)
#         self.plots_label_frame = LabelFrame(self.main_frm, text="CREATE PLOTS", font=Formats.LABELFRAME_HEADER_FORMAT.value)
#         self.plots_cb = Checkbutton(self.plots_label_frame, text='Create plots', variable=self.create_plots)
#         self.plots_label_frame.grid(row=self.children_cnt_main(), column=0, sticky=NW)
#         self.plots_cb.grid(row=0, column=0, sticky=NW)
#         self.create_run_frm(run_function=self.run)
#
#     def run(self):
#         check_int(name='Time bin', value=str(self.time_bin_entrybox.entry_get))
#         self.config.set(ReadConfig.PROCESS_MOVEMENT_SETTINGS.value, ReadConfig.ROI_ANIMAL_CNT.value, str(self.animal_cnt_dropdown.getChoices()))
#         for cnt, (name, data) in enumerate(self.body_parts_dropdown_dict.items()):
#             self.config.set(ReadConfig.PROCESS_MOVEMENT_SETTINGS.value, 'animal_{}_bp'.format(str(cnt + 1)), str(self.body_parts_dropdown_dict[name].getChoices()))
#         self.update_config()
#         time_bin_movement_analyzer = TimeBinsMovementAnalyzer(config_path=self.config_path, bin_length=int(self.time_bin_entrybox.entry_get), plots=self.create_plots.get())
#         time_bin_movement_analyzer.analyze_movement()

# _ = TimeBinsMovementPopUp(config_path='/Users/simon/Desktop/envs/troubleshooting/two_black_animals_14bp/project_folder/project_config.ini')

#TimeBinsMovementPopUp()
