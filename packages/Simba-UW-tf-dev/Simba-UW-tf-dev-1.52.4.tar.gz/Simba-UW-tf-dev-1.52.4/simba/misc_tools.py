...
def get_video_meta_data(video_path: str):
    ...
    for k, v in video_data.items():
        if v == 0:
            ...
            raise ValueError(f'SIMBA WARNING: Video {video_data["video_name"]} has {k} of {str(v)}.')
...
def archive_processed_files(config_path: str,
    ...
    if len(file_lst) < 1:
        print(
            'SIMBA ERROR: No data files located in your project_folder/csv sub-directories in the worflow file format {}'.format(
                file_type))
        raise ValueError()
...
def copy_single_video_to_project(simba_ini_path: str,
    ...
    if file_ext[1:].lower().strip() not in allowed_video_formats:
        print(
            'SIMBA ERROR: SimBA works best with avi and mp4 videofiles. Or please convert your videos to mp4 or avi to continue before importing it. ')
        raise ValueError()
    ...
    if os.path.isfile(destination):
        print(
            'SIMBA ERROR: {} already exist in SimBA project. To import, delete this video file before importing the new video file with the same name.'.format(
                file_name))
...
def run_user_defined_feature_extraction_class(file_path: str,
    ...
    if file_extension != '.py':
        print(f'SIMBA ERROR: The user-defined feature extraction file ({file_path}) is not a .py file-extension')
        raise ImportError(f'SIMBA ERROR: The user-defined feature extraction file ({file_path}) is not a .py file-extension')
    ...
    if len(class_name) < 1:
        print(f'SIMBA ERROR: The user-defined feature extraction file ({file_path}) contains no python classes')
        raise ImportError(f'SIMBA ERROR: The user-defined feature extraction file ({file_path}) contains no python classes')
    else:
