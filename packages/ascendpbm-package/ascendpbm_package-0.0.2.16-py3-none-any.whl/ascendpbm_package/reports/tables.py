...
def add_rebate_dollarcol(df_claims, df_formulary, claim_ndc, claim_days, form_ndc='ndc',
                         form_id='formulary_id', form_status='rebate_status',
                         ...
                         rebate_guar = pd.DataFrame({'Formulary':['REMEDYONE HIGHLY MANAGED FORMULARY',
                                                     ...
                                                     'rebate_method':['BRAND']*3 + ['FLAG']*3,
                                                     '<= 84 Days':[129.11, 120.43, 117.42] + [np.nan, np.nan, np.nan],
                                                     '> 84 Days':[543.88, 523.22, 476.08] + [np.nan, np.nan, np.nan],
                                                     'Specialty':[2432.51, 2400.97, 1620.69] + [np.nan, np.nan, np.nan]
                                                     }
                                                    )
                         ):
    '''
    ...
    form_status : str, optional
        Column name for the df_formulary rebate status col (preferred, excluded, etc).
        Note we had to change this from original formulary exclusions due to contract specifying
        some extra exclusions of brands with generics available. We coded a view for this called
        formulary_with_rebate_exclusions that includes the rebate_status col.
        The default is 'rebate_status'.
    ...
    positive : str, optional
        Although a string, could be anything that matches a positive indication in a col.
        The default is "Y".
    exclude : str, optional
    ...
    rebate_guar : pandas dataframe, optional
        ...
        The default is pd.DataFrame({'Formulary':['REMEDYONE HIGHLY MANAGED FORMULARY',
                                                 'REMEDYONE MANAGED FORMULARY',
                                                 'REMEDYONE OPEN FORMULARY']*2,
                                    'rebate_method':['BRAND']*3 + ['FLAG']*3,
                                    '<= 84 Days':[129.11, 120.43, 117.42] + [np.nan, np.nan, np.nan],
    ...
    for idx, row in rebate_guar.iterrows():
        if row['rebate_method'] == 'FLAG': # For the flag method
            ...
            rebates.loc[(rebates[mony].isin(['M', 'N', 'O'])) & (rebates[specialty_flag] == 'N')
                        & (rebates[days_flag] == 'Y') & (rebates[form_id] == row[form_id])
            ...
            rebates.loc[(rebates[mony].isin(['M', 'N', 'O'])) & (rebates[specialty_flag] == 'N')
                        ...
                        & (rebates[brand] == trademark) & (rebates[form_status] != exclude),
                        [rebate_amnt_brand, rebate_brand]] = [row['<= 84 Days'], 'Y']
            rebates.loc[(rebates[mony].isin(['M', 'N', 'O'])) & (rebates[specialty_flag] == 'Y')
...
def time_table(dff, n, start_date, end_date, df_members=None, df_employees=None,
    ...
    dff.loc[(dff[time].isnull()) | (dff[time].astype(str).str.lower() == 'nan'), time] = "" # replacing time variable's nan value for summary row
    return dff
