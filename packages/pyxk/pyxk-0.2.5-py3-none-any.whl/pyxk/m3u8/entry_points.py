...
def main():
    ...
    parser.add_argument(
        "-f", "--file", help="媒体文件名称")
    parser.add_argument(
        "-r", "--rerequest", help="重新下载(default: False)", action="store_true")
    parser.add_argument(
        "--delete", help="删除m3u8文本文件(default: False)", action="store_true")
    parser.add_argument(
        "--reserve", help="合并完成删除ts文件(default: False)", action="store_true")
    parser.add_argument(
        "-l", "--limit", help="异步aiohttp并发连接(default: 8)", default=8, type=int)
    parser.add_argument(
    ...
    m3u8_downloader.load_m3u8_uri(
        ...
        delete_m3u8file=args.delete_m3u8file,
        reserve_segments=args.reserve_segments,
        **args.parameter
    )
