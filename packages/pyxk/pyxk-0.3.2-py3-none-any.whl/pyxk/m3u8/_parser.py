...
class M3U8():
    ...
    def _file_abspath(self, file, /):
        ...
        file = utils.rename_file(os.path.normpath(os.path.abspath(file)), suffix="mp4")
        self._file = file[0]
    ...
    def _start(self, content, uri=None):
        ...
        if self._delete_m3u8file:
            for file in os.walk(self._temp).__next__()[-1]:
                os.remove(os.path.join(self._temp, file))
