...
class Search():
    ...
    def fit_mxl(self, sol):
        ...
        """
        logger.info("GPU DEBUG: START MIXED LOGIT")
        as_vars = sol['asvars']
        is_vars = sol['isvars']
        asc_ind = sol['asc_ind']
        bcvars = sol['bcvars']
        ...
        try:
            ...
            if self.multi_objective:
                ...
                logger.info(f"MAE: {MAE}")
                logger.info(f"Out of sample log-likelihood: {model.loglikelihood}")
                logger.info("GPU DEBUG: (5)")
                if self.multi_objective_variable == "MAE":
                    multi_val = MAE
                else:
                    multi_val = model.bic
                logger.info("GPU DEBUG: (6)")
        except Exception as e:
        ...
        logger.debug("model convergence: {}".format(conv))
        logger.debug("dof: {}".format(len(coef_names)))
        logger.info("GPU DEBUG: (FINAL RETURN)")
    ...
    def fit_lccm(self, sol):
        ...
        return (bic, ll, multi_var, as_vars, is_vars, rand_vars, transvars, corvars,
