...
class Test:
    ...
    def copy(self):
        ...
        shutil.copyfile( self.tdd.project / self.code, self.home / Path(self.code).name)

        test_sut_code = []
        for file in self.sut:
            filename = Path(file).name
            shutil.copyfile( self.tdd.project / file, self.home / filename )
            file_suffix = Path(filename).suffix
            if file_suffix == '.c' or file_suffix == '.cpp':
                test_sut_code.append(filename)

        self.generate(cmake_template, "CMakeLists.txt", name=self.name, code=code_name, sut_code=test_sut_code)
