...
class DiscreteIdentityPrincipleConjunctionsCounter(DiscreteAggregationFunctionsCounter):
    ...
    def count_operators(self) -> int:
        ...
        count = self.__staircase_plane_partition(r=self.n, l=self.n+1, k=1, m=self.n+1)-self.__staircase_plane_partition(r=self.n, l=self.n+1, k=1, m=self.n)
    ...
    def __staircase_plane_partition(r: int, l: int, k: int, m: int) -> decimal.Decimal:
        ...
        for i in range(1, r + 1):
            up_fact = DiscreteIdentityPrincipleConjunctionsCounter.__upper_factorial(decimal.Decimal(m + (l - k * i) / (k + 1)), i - 1)
