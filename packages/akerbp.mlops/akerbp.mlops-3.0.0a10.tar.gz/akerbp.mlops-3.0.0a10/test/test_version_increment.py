...
def test_get_current_version_return_zero_pre_release_number():
    version = increment_version(test="0.1.0", commit_msg="prerelease")
...
def test_get_current_version_return_incremented_pre_release_number():
    version = increment_version(test="0.1.0a0", commit_msg="prerelease")
    pre_release_number = version.split("a")[1]
...
def test_increment_version_major_increment_in_test():
    ...
    new_version = increment_version(test=version, commit_msg=commit_msg)
...
def test_increment_version_major_increment_in_prod():
    ...
    incremented_version = increment_version(test=version, commit_msg=commit_msg)
    os.environ["ENV"] = "prod"
    new_version_in_prod = increment_version(test=incremented_version)
