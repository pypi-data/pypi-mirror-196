...
env_vars = {k.upper(): str(v) for k, v in config.read_env_vars().__dict__.items() if v}
...
def garbage_collection(
    ...
    if models_to_keep is not None and models_to_keep >= len(external_ids):
        ...
        if remove_artifacts:
            # Remove any non versioned models too
            non_versioned_models = [
                f.external_id
            ...
            for external_id in non_versioned_models:
                ...
                deployed_model = client.functions.retrieve(external_id=external_id)
                try:
                    v = deployed_model.metadata["version"]  # type: ignore
                except KeyError:
                    logger.warn(
                        "No version number found in deployed models metadata, will not delete artifacts associated with this model"
                    )
                    v = "unknown"
                delete_function(function_name=external_id, confirm=False)
                m, _, e = external_id.split("-")
                if v != "unknown":
                    delete_file(id={"external_id": f"{m}/{e}/{v}"})
