...
from your local environment, e.g. : `LOCAL_DEPLOYMENT=True ENV=test deploy_prediction_service`
...
`LOCAL_DEPLOYMENT=True bash deploy_prediction_service` will not deploy models and run tests.
...
`TESTING_ONLY=True bash deploy_prediction_service` instead.
