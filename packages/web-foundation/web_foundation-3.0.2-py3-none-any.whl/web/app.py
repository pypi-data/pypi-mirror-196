...
class WebApp(Generic[GenConfig], ConfAble[GenConfig], SigAble, ChanAble, EnvAble):
    ...
    async def run(self, multiprocessing: bool = False,
        ...
        def sig_handler(sig, frame):
            loguru.logger.info(
                f"{self.name} Received signal {Signals(sig).name}. Shutting down.")
            self.scheduler.stop()
            self.manager.stop()
            exit(0)

        signal(SIGINT, lambda s, f: sig_handler(s, f))
        signal(SIGTERM, lambda s, f: sig_handler(s, f))
