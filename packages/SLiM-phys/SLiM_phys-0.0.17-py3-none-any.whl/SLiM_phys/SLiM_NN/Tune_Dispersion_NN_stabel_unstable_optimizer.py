...
def create_model(checkpoint_path,x_train, y_train):
    def model_builder_layers(hp):
        ...
        model.add(tf.keras.layers.Dense(units=hp_layer_2, activation='relu'))
        model.add(tf.keras.layers.Dense(units=hp_layer_3, activation='relu'))
        model.add(tf.keras.layers.Dense(units=hp_layer_3, activation='relu'))
        model.add(tf.keras.layers.Dense(units=hp_layer_2, activation='relu'))
        model.add(tf.keras.layers.Dense(units=hp_layer_1, activation='relu'))
        model.add(tf.keras.layers.Dense(units=hp_layer_0, activation='relu'))
    ...
    def model_builder_activation(hp):
        #creating the model
        model = tf.keras.Sequential()

        
        hp_activation0 = hp.Choice('activation0', values=['relu','sigmoid'])
        hp_activation1 = hp.Choice('activation1', values=['relu','sigmoid'])
        hp_activation2 = hp.Choice('activation2', values=['relu','sigmoid'])
        hp_activation3 = hp.Choice('activation2', values=['relu','sigmoid'])

        model.add(tf.keras.Input(shape=(7)))
        model.add(tf.keras.layers.Dense(units=16, activation=hp_activation0))
    ...
    def model_builder_optimizers(hp):
        #creating the model
        model = tf.keras.Sequential()

        
        hp_optimizers  =  hp.Choice('optimizers', values=['Adadelta', 'Adagrad', 'Adam', 'Adamax', 'Ftrl', 'Nadam', 'RMSprop','SGD'])
    ...
    def model_builder_dropout(hp):
        ...
        hp_dropout = hp.Choice('dropout_rate', values=[0.,0.01,0.05,0.1,0.2,0.3,0.5])
    ...
    def model_builder_learning_rate(hp):
        #creating the model
        model = tf.keras.Sequential()
    ...
    tuner = kt.Hyperband(model_builder_optimizers,
