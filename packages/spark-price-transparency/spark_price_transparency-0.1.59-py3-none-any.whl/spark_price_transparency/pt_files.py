...
class Pt_files:
        ...
        return self.files \
                   .join(self._spark.table(self._meta_ingest_tbl).groupBy(col('file_name'))
                             .agg(F.first(col('reporting_entity_name'),
                                          ignorenulls=True).alias('reporting_entity_name')), "file_name", "left") \
                   .select(col('files.*'),
    ...
    @dispatch(object, str)
    def run_import(self, url: str):
        file_meta = get_file_meta(url.split('/')[-1])
        tgt = self.path + '/' + file_meta['name'] + '.json'
        if ~path_exists(self.path):
            sh(['mkdir', '-p', self.path])
        if file_exists(tgt):
            print(tgt + ' already exists.')
            return
        if file_meta['ext'] == 'json.gz':
            with open(tgt, "w") as f:
                ...
                wget_process.wait()
        elif file_meta['ext'] == 'zip':
            zip_tgt = self.path + '/' + file_meta['name'] + '.zip'
    ...
    @dispatch(object, [str])
    def run_import(self, urls: [str]):
        for url in urls:
            self.run_import(url=url)

    @dispatch(object, Pdf)
    def run_import(self, url_df: Pdf):
        url_col = 'url' if 'url' in url_df.columns else url_df.columns[0]
        urls = url_df[url_col].values.tolist()
        self.run_import(urls=urls)

    @dispatch(object, DataFrame)
    def run_import(self, url_df: DataFrame):
        url_col = 'url' if 'url' in url_df.columns else url_df.columns[0]
    ...
    def run_import(self):
        """
        This should be overwritten with the dataframe that has un-imported urls for the given schema
        """
        pass
