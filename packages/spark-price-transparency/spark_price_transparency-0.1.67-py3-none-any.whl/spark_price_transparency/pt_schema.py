...
class Pt_schema:
    ...
    def run_ingest_pdf(self, file_pdf: Pdf):
        file_col = 'file_path' if 'file_path' in file_pdf.columns else file_pdf.columns[0]
        file_paths = file_pdf[file_col].values.tolist()
        self.run_ingest_files(file_paths=file_paths)

    def run_ingest_df(self, file_df: DataFrame):
        file_col = 'file_path' if 'file_path' in file_df.columns else file_df.columns[0]
        file_paths = [str(row[0]) for row in file_df.select(file_col).collect()]
        self.run_ingest_files(file_paths=file_paths)
