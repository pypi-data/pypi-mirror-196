...
class Pt_schema:
    ...
    def __init__(self, mth: int = None, catalog_name='hive_metastore', stage_db_name='pt_stage'):
        ...
        self._set_tables()
        for tbl_name, v in self.ingest_tables.keys():
            setattr(self, tbl_name, property(lambda x: x.ingest_tables[tbl_name]))
        for tbl_name, v in self.analytic_tables.keys():
            setattr(self, tbl_name, property(lambda x: x.analytic_tables[tbl_name]))
    ...
    def run_analytic_files(self, file_paths: [str]):
        self.run_analytic_df(self._spark.createDataFrame(data=[(f,) for f in file_paths], schema=['file_name', ]))
