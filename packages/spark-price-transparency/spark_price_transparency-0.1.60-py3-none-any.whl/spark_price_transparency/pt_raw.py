...
from .table_of_contents.files import Table_of_contents_files
...
class PTRaw:
    ...
    def __init__(self, mth: int = None):
        ...
        self.table_of_contents = Table_of_contents_files(self.mth)
        # self.in_network_rates = In_network_rates_files(self.mth)
        # self.provider_reference = Provider_reference_files(self.mth)
        # self.allowed_amounts = Allowed_amounts_files(self.mth)
    ...
    def locationUri(self):
        ...
        if len(database) == 0:
            print(f'WARNING: pt_stage database does not exist. Run Guide.initialize_pt()' +
                  ' or PTRaw.initialize_pt_raw().')
            location_uri = None
        else:
    ...
    def create_raw_database(self):
        ...
        if self.locationUri is None:
            self._spark.sql(f'CREATE DATABASE IF NOT EXISTS pt_stage')
    ...
    def create_raw_mth_directory(self):
        ...
        if os.path.exists(path):
            ...
            os.mkdir(path + '/schema=allowed-amount')
