        ...
        if years != (0,0) and self.type != "format1":
            ...
            for linha in range(len(self.dataframe)):
                for dia in range(1, 32):
                    try:
                        temp = self.dataframe[colum[0]][linha]
                        data = np.append(data, datetime(temp.year, temp.month, dia)) # check if the date is real
                        pr = np.append(pr, float(self.dataframe[colum[dia]][linha]))
                    except:
                        n = 0
            ...
            for i in range(len(data)):
                try:
                    while i < data.size-1 and np.datetime64(data[i]) != np.datetime64(a[i]):
                        data = np.insert(data, i, a[i])
                        pr = np.insert(pr, i, 0)
                        pr[i] = np.NaN
                except:
                    continue
        ...
        if self.list == False:
            ...
            if self.type == "format1":
                ...
                for c in range(self.len):
                    for l in range(1,32):
                        try:
                            y = np.append(y, self.dataframe.iloc[l,c])
                            x = np.append(x, self.dataframe.iloc[0,c+l])
                        except:
                            oi=''
                plt.title(title)
                plt.xlabel(xlabel)
