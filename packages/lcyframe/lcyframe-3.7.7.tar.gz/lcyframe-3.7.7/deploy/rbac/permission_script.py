...
def import_groups(content_type=1):
    ...
    """
    # 清空角色表
    Groups.objects.filter(content_type=content_type).delete()
    first_group = None
    ...
    if content_type == 1:
        # 清空业务用户的角色
        User.objects.filter(company_id__gt=1).update(group_id="")
        # is_superuser企业用户保留第一个角色以便登录系统
        User.objects.filter(company_id__gt=1, is_superuser=1).update(group_id=first_group.id)
    else:
        # 清空系统端用户的角色
        User.objects.filter(company_id=1).update(group_id="")
        # 系统端所有用户默认设置第一个角色
        User.objects.filter(company_id=1).update(group_id=first_group.id)
...
def set_groups_permission(content_type=1):
    ...
    """
    # 清空
    Groups.objects.filter(content_type=content_type).update(permissions={})
...
def set_company_groups(*company_ids, content_type=1):
    """
    给商户分配角色: 不含系统端
    :param company_id:
    :return:
    """
    if 1 in company_ids:
        raise Exception("id=1的属于系统端商户，不需要分配角色")
    # 清空已分配的角色
    CompanyGroups.objects.all().delete()
    groups = []
    ...
    if company_ids:
        ...
        companys = Company.objects.filter(id__gt=1)

    for company in companys:
        # for i in random.sample(groups, len(groups)-1):
        for group in groups[:3]:   # 默认前3个角色分配给商户
            g = CompanyGroups.objects.create(**{"company_id": company.id, "group_id": group["id"]})
            print("商户：%s,分配角色:%s, 关系：%s" % (company.name, group["name"], model_to_dict(g)))

if __name__ == '__main__':
    # import_groups(1)
    # import_permissions(1)
    # set_groups_permission(1)
    print(set_company_groups())
