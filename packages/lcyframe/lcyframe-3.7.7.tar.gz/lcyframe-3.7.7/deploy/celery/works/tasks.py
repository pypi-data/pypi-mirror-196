...
# 延迟执行1：从现在起，60秒后执行
eta=datetime.utcnow() + timedelta(seconds=60),          # 在指定的时间运行，这里实现了延迟10秒运行，2022-01-13 09:32:08.643081
eta=datetime.now().utcnow() + timedelta(seconds=60),    # 在指定的时间运行，这里实现了延迟10秒运行，2022-01-13 09:32:08.643081
func.apply_async(args=(x, ), eta=eta)

# 延时执行2，在指定时间执行
eta2utc = datetime.datetime.now() + datetime.timedelta(seconds=10)
eta2utc = eta2utc.astimezone(pytz.utc).replace(tzinfo=None)     # 转UTC
add3.apply_async(kwargs={"x": 99999}, eta=eta2utc)

# 延迟执行3：下次执行时间为60s后。celery会转为eta值（utc格式）放入队列
add3.apply_async(kwargs={"x": 12}, countdown=60)

# 需要注意
1、utcnow()方法是把此刻的CST时间转为UTC。
2、如果需要把指定某一CST时间转为UTC，需要这样：datetime.now(2022, 12, 12, 10, 10, 10).astimezone(pytz.utc).replace(tzinfo=None)
celery会根据配置文件设置的时区，决定是否需要+8:00。如果配置文件是utc，不需要处理。如果是cst时区，则+8小时在写入队列
# celeryconfig设置时区: timezone = "Asia/Shanghai"、enable_utc = False.不设置也可以
...
# 指定秒数
# expires_at = 60     
add3.apply_async(kwargs={"x": 60}, expires=expires_at)
