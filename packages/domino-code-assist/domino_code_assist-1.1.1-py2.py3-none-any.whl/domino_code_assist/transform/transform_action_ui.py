...
def MergePanelView(columns, dtypes, action_: action.ActionMerge, on_merge_action):
    ...
    def get_common_cols():
        ...
        common = set(columns).intersection(set(cols))
        # if not action_.complete_mapping:
        on_merge_action(replace(action_, auto=bool(common), mapping={None: None}))
        return common

    common_cols = sol.use_memo(get_common_cols, [columns, action_.var_name_other])
    ...
    with v.Sheet():
        ...
        if action_.var_name_other:
            ...
            if not action_.on_index:
                with v.BtnToggle(v_model=how_options.index(action_.how), on_v_model=lambda v: on_merge_action(replace(action_, how=how_options[v]))):
