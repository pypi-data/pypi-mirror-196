...
- Support for delay-load DLL dependencies comes with a few limitations.
  - Any delay-load DLLs are loaded immediately upon module import instead of when the first function in the DLL is called. This defeats the main purpose of a delay-load DLL but is the only known way to avoid load errors.
  - If your wheel contains a delay-load DLL that is loaded with a custom delay load helper function (https://learn.microsoft.com/en-us/cpp/build/reference/understanding-the-helper-function), then it is your responsibility to ensure that the helper function is able to load the DLL at runtime. The default helper function supplied by Microsoft is compatible with `delvewheel`.
  - If your wheel contains a top-level extension module that has a delay-load direct or transitive dependency, then we inject some code that runs at Python startup to preload the dependencies. End users who have Python processes that were running before your wheel was installed should start new Python instances before trying to use your extension module.
