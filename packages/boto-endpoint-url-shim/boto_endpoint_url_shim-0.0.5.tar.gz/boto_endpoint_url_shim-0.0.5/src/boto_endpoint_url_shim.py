...
_SERVICE_ENV_VAR_TEMPLATE = "AWS_ENDPOINT_URL_{}"
...
_LOADER: botocore.loaders.Loader = None  # type: ignore


def _init_loader(search_path_string: str | None = None) -> None:
    """
    Initialise common Loader for botocore data models.

    `search_path_string` passed as it to `botocore.loaders.create_loader`.
    """
    global _LOADER
    _LOADER = botocore.loaders.create_loader(search_path_string)


def _get_loader() -> botocore.loaders.Loader:
    """Get common Loader for botocore data models."""
    if _LOADER is None:
        _init_loader()
    return _LOADER


def service_id(service_name: str, api_version: str | None = None) -> str:
    """Get service ID from botocore data models."""
    loader = _get_loader()
...
def service_config_name(
    service_name: str, api_version: str | None = None
) -> str:
    """Get service configuration section name from botocore data models."""
    service_id_ = service_id(service_name, api_version=api_version)
    return service_id_.replace(" ", "_").lower()


def service_env_var(service_name: str, api_version: str | None = None) -> str:
    """Get service-specific environment variable from botocore data models."""
    service_id_ = service_id(service_name, api_version=api_version)
    return _SERVICE_ENV_VAR_TEMPLATE.format(
        service_id_.replace(" ", "_").upper()
    )


def read_service_env_var(session: S, service_name: str) -> str | None:
    """
    ...
    ```
    """
    return os.environ.get(service_env_var(service_name))


def read_global_env_var(session: S, service_name: str) -> str | None:
    """
...
def read_service_config_file(session: S, service_name: str) -> str | None:
    """
    Read from service configuration section from the shared configuration file.
    ...
    service_config = services_config.get(service_config_name(service_name), {})
    return service_config.get("endpoint_url")


def read_global_config_file(session: S, service_name: str) -> str | None:
    """
...
def read_mapping_env_var(session: S, service_name: str) -> str | None:
    """
    ...
    return _SERVICE_ENDPOINT_URL_MAP.get(service_config_name(service_name))
...
def read_named_top_level_config_file(
    ...
    return profile_config.get(
        f"{service_config_name(service_name)}_endpoint_url"
    )
...
def proposed_endpoint_url_resolution() -> None:  # pragma: no cover
    """
    Set endpoint URL resolution order for `botocore.session.Session`.
    ...
    class Session(botocore.session.Session):
        def _resolve_endpoint_url(
            ...
            for resolution_method in iter(ero):
                endpoint_url = resolution_method(self, service_name)
                if endpoint_url:
                    return endpoint_url
            else:
                return None
