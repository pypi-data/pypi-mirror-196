...
def test_compress():
    ...
    try:
        ...
        write(
            ...
            "tiff",
            do_compression=True,
            compression_algorithm=Compression.lzw,
        )
        lzw_size = pathlib.Path(fname).stat().st_size
        write(
            grid,
            fname,
            "tiff",
            do_compression=True,
            compression_algorithm=Compression.packbits,
        )
        packbits_size = pathlib.Path(fname).stat().st_size
        assert packbits_size > lzw_size
...
def test_tiff_nan():
    values = [-999, -999.0]
    for idx, dtype in enumerate([np.int32, np.float32]):
        data = np.arange(0, 25).reshape(5, 5).astype(dtype)
        data[0, 0] = values[idx]
        gdict = {
            "xmin": 5.0,
            "xmax": 9.0,
        ...
        fname = os.path.join(tdir, "tempfile.grd")
        try:
            write(grid, fname, "tiff", no_data_value=values[idx])
            src = rasterio.open(fname, "r")
            tdata = src.read(1)
            np.testing.assert_almost_equal(tdata, data)
        except Exception as e:
