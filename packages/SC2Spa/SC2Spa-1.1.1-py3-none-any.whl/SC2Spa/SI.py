...
def FineMapping(adata_ref, adata_query, sparse = True, model_path = None, WD_cutoff = None, JGs = None,
                root = 'Model_SI/', name = 'SI', l1_reg = 1e-5, l2_reg = 0, dropout = 0.05, epoch = 500,
                batch_size = 4096, nodes = [4096, 1024, 256, 64, 16, 4], lrr_patience = 20, ES_patience = 50,
                min_lr = 1e-5, save = True, polar = True, n_neighbors = 1000, dis_cutoff = 20,
                n_neighbors_max = 3, seed = None):
    ...
    adata_query.obs['FM_density_control'] stores the fine mapping information with density control
    ...
    ES_patience
        The patience for early stopping
    n_neighbors_max
        The number of max neighbors of a spot. This it to avoid too many single cells being placed
         around one spot. Only the closest n_neighbors_max single cells will be labelled as mapped.
         The boolean information will be stored in adata_ref.obs['FM_DC'].
    ...
    #Extract Single Cell Info and calculate the cells' distance
    # from the closest ST bead
    dis_sc = dis[n_ST:]
    ...
    #Control the density of single cells
    MappedIndices_DC = np.unique(neighbors[:, :(1+n_neighbors_max)])
    MappedIndices_DC = MappedIndices_DC[MappedIndices_DC != -1]
    MappedSCInd_DC = MappedIndices_DC[MappedIndices_DC >= n_ST] - n_ST
    ...
    #Write density control information
    adata_query.obs['FM_DC'] = False
    query_FM_DC_ind = np.where(adata_query.obs.columns == 'FM_DC')[0][0]
    adata_query.obs.iloc[MappedSCInd_DC, query_FM_DC_ind] = True
