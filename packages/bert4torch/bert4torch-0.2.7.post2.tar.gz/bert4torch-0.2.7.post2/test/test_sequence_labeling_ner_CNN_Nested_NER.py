...
warm_factor = 0.1
weight_decay = 1e-2
label2idx = {'DNA': 0, 'RNA': 1, 'cell_line': 2, 'cell_type': 3, 'protein': 4}
...
# BERT base
root_path = 'dmis-lab/biobert-v1.1'
...
# 建立分词器
tokenizer = AutoTokenizer.from_pretrained(root_path)
    ...
    def load_data(self, filename):
        ...
        with open(filename, encoding='utf-8') as f:
           for line in f:
                ...
                _raw_ents = []
                for ent in entities:
                    _raw_ents.append((ent['start'], ent['end']-1, ent['entity_type']))
                ...
                for idx, word in enumerate(_raw_words, start=0):
                    if word in word2bpes:
                        ...
                        __bpes = tokenizer.encode(' '+word if add_prefix_space else word, add_special_tokens=False)
...
# 加载数据
valid_dataloader = DataLoader(MyDataset('E:/Github/CNN_Nested_NER/preprocess/outputs/genia/dev.jsonlines'), batch_size=batch_size, collate_fn=collate_fn) 
train_dataloader = DataLoader(MyDataset('E:/Github/CNN_Nested_NER/preprocess/outputs/genia/train.jsonlines'), batch_size=batch_size, shuffle=False, collate_fn=collate_fn) 
...
class CNNNer(BaseModel):
    def __init__(self, num_ner_tag, cnn_dim=200, biaffine_size=200,
        ...
        self.pretrain_model = AutoModel.from_pretrained(root_path)
        hidden_size = 768
...
# state_dict = torch.load('E:\Github\CNN_Nested_NER\model.pt', map_location='cpu')
...
class Loss(nn.Module):
    def forward(self, scores, y_true):
        ...
        # print(flat_matrix.sum(), mask.sum(), '\n')
        return loss
        ...
        for pred, label in zip(outputs, ent_target):
            ...
            for item in range(pred.shape[-1]):
                if pred[:, :, item].sum() > 0:
                    ...
                    pred_tuple.extend([(i, j, item) for i, j in zip(*_index)])
