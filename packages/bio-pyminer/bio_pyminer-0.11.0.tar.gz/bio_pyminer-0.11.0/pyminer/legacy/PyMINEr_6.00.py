...
Changed from 5.19 to 6.00:
-added support for h5f
"""
...
-h5py
	This option will enable using the h5py library for large datasets.
	If enabled, the analysis should run substantially quicker for large
	datasets.

-ID_list </path/to/ID_list.txt>
	A text file with IDs corresponding to the rows in an hdf5
	input file (a new line separated list)
	
-rho_cutoff <float>
    An alternative to a FPR cutoff is using the spearman rho statistic.
    This option is currently only supported for the h5py format.
...
if '-h5py' in sys.argv:
    do_h5_py=True
    import h5py
    if '-ID_list_path' not in sys.argv:
...
if '-ID_list_path' in sys.argv:
    ID_list_path=sys.argv[sys.argv.index('-ID_list_path')+1]
    IDlist = read_file(ID_list_path,'lines')
    make_file('\n'.join(IDlist),temp+'/ID_list.txt')


if '-rho_cutoff' in sys.argv:
    rho_cutoff_bool = True
    rho_cutoff =float(sys.argv[sys.argv.index('-rho_cutoff')+1])
else:
    rho_cutoff_bool = False
...
if very_big_file and not do_h5_py:
...
gc.collect()
if very_big_file and big_file_do_z and not do_h5_py:
...
#########################################################################################
def write_big_file_parallel_spearman(start_row,end_row,offset,num_vars,in_file,index,script_path,status_path,FPR):
    print(script_path)
    print(status_path)
    python_commands=str("""
from subprocess import Popen
...
if very_big_file and perform_mine_analysis and not do_h5_py:
...
def write_h5_parallel_spearman(start_row,end_row,num_vars,in_file,index,script_path,status_path,ID_list_file,FPR=None,rho_cutoff=None):
    if (FPR is None) and (rho_cutoff is None):
        sys.exit("write_h5_parallel_spearman requires either an FPR or rho_cutoff")
...
rho_cutoff_bool = """+str(rho_cutoff_bool)+"""
rho_cutoff = """+str(rho_cutoff)+"""
...
for i in range(start_row,end_row):
    ...
    new_lines = get_spearman_h5(i)
    
    if new_lines != None:## this means that there were at least some detected correlations
        output_lines = output_lines + new_lines
        
        if len(output_lines)>dump_length:
            write_table(output_lines, temp_aggregate)
            ## these pass off the files circularly to add the new
            ## results to the summary file
            cmd('cp '+summary+' '+summary+'_temp.txt')
            cmd('rm '+summary)
            cmd('cat '+summary+'_temp.txt '+temp_aggregate+'>'+summary)
            cmd('rm '+summary+'_temp.txt')
            cmd('rm '+temp_aggregate)
            output_lines = []
            print("this is when we would do the temp aggregate thing")
            #sys.exit()
        print("currently we've found",(len(output_lines))/2,"unique relationships")
...
## write the the 'done' status in the status folder for the original script to read
cmd("echo '/media/scott/all_microarray/final_data_combined/pyminer_analysis/scripts/control_raw_data0.py finished' >> '/media/scott/all_microarray/final_data_combined/pyminer_analysis/status.txt'")
...
if very_big_file and perform_mine_analysis and do_h5_py:
    ## for a very big file, we need to run through the file loading in lines one at a time
    ## this takes a really long time, but there's no real way around it for files
    ## that won't fit in memory
    ...
    for program in range(0,t):
        ...
        write_h5_parallel_spearman(start_indices[program], end_indices[program], num_vars, c_file, program, c_py, temp+'/status.txt', ID_list_path, FPR=FPR, rho_cutoff = rho_cutoff)
