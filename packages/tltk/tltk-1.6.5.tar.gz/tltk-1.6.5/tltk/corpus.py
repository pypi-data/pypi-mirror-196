...
# -*- coding: utf-8 -*-
#########################################################
## Thai Language Toolkit : version  1.6.5
...
def compound(w1,w2):
    ...
    if w1+w2 not in vocabs or w1 not in vocabs or w2 not in vocabs:
        return(0)  ## not a compound or w1,w2 not exist
    else:    
        if w1 in vocabs and w2 in vocabs:
            degree1 = w2v_model.wv.similarity(w1,w1+w2)
            degree2 = w2v_model.wv.similarity(w2,w1+w2)
            if degree1 > degree2:
                return((w1,degree1,w2,degree2))
            else:    
                return((w2,degree2,w1,degree1))
