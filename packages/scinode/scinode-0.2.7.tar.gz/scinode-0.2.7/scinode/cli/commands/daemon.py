...
def daemon_not_exist(name, computer):
    return "\nWarning!!! Daemon {}@{} is not setup.\n".format(name, computer)


def start_daemon(
    ctx, name, data, method=None, concurrency=None, sleep=None, foreground=None
):
    print(f"Start daemon: {name}")
    if ctx.obj.use_celery:
        from scinode.engine.celery.daemon import CeleryDaemon
        ...
        daemon = ScinodeDaemon(
            name, method=method, concurrency=concurrency, sleep=sleep
        )
...
@click.pass_context
def start(ctx, name, method, concurrency, foreground, sleep):
...
@click.option("--sleep", default=1, help="Sleep time")
def hard_restart(name, method, concurrency, sleep):
    from scinode.daemon.daemon import ScinodeDaemon

    daemon = ScinodeDaemon(name, method=method, concurrency=concurrency, sleep=sleep)
    daemon.restart()
