...
class Engine:
    ...
    def process(self, msg):
        ...
        if catalog == "nodetree":
            try:
                ...
                scinodedb["nodetree"].update_one({"uuid": uuid}, {"error": str(error)})
        elif catalog == "node":
            try:
                ...
                scinodedb["nodetree"].update_one(
                    {"uuid": uuid}, {f"nodes.{name}.state": "FAILED"}
                )
                scinodedb["nodetree"].update_one(
                    {"uuid": uuid}, {f"nodes.{name}.error": str(error)}
                )
        elif catalog == "daemon":
            try:
    ...
    def gather_node(self, uuid, name):
        """Gather node"""
        print(f"\ngather node: {name}")
        ntdata = scinodedb["nodetree"].find_one(
            {"uuid": uuid}, {"_id": 0, f"nodes.{name}": 1}
        ...
        node = EngineNode(uuid=ntdata["nodes"][name]["uuid"], daemon_name=self.name)
        future = node.process(self.pool, self.futures.get(uuid), action="GATHER")
        self.futures[uuid] = future
    ...
    def restart_daemon(self, name):
        ...
        print(f"Restart daemon {name}...")
        scinodedb["broker"].update_one(
            {"name": name}, {"$set": {"msg": [], "indices": [0]}}
        )
        os.system("scinode daemon hard-restart")
