...
class ModelMixture(Model):
    def __init__(self, bad: float, left: int, dist='BetaNB',
                 ...
                 r_transform=None, use_masking=True, left_k=1):
        """
    ...
    def logprob_mode(self, params: jnp.ndarray,
        ...
        if self.dist == 'NB':
            ...
            lp = dist.LeftTruncatedCDNB.logprob
            lp = lp(data, r, p, self.left, r_transform=self.r_transform)
        elif self.dist.startswith('Beta'):
            k = self.get_param('k', params)
            lp = dist.LeftTruncatedBetaCDNB.logprob if 'CDNB' in self.dist else dist.LeftTruncatedBetaNB.logprob
    ...
    def prob_mode(self, params: jnp.ndarray,
        ...
        if self.dist == 'NB':
            ...
            pmf = dist.LeftTruncatedCDNB.logprob
            pmf = jnp.exp(pmf(data, r, p, self.left, r_transform=self.r_transform))
        elif self.dist.startswith('Beta'):
            k = self.get_param('k', params)
            pmf = dist.LeftTruncatedBetaCDNB.long_prob if 'CDNB' in self.dist else dist.LeftTruncatedBetaNB.long_prob
    ...
    def cdf_mode(self, params: jnp.ndarray,
        ...
        if self.dist == 'NB':
            ...
            lp = dist.LeftTruncatedCDNB.long_cdf
            lp = lp(data, r, p, self.left, r_transform=self.r_transform)
        elif self.dist.startswith('Beta'):
            k = self.get_param('k', params)
            lp = dist.LeftTruncatedBetaNB.long_cdf
            lp = dist.LeftTruncatedBetaCDNB.long_cdf if 'CDNB' in self.dist else dist.LeftTruncatedBetaNB.long_cdf
            lp = lp(data, p, k, r, self.left, r_transform=self.r_transform)
        elif self.dist == 'Binom':
            lp = dist.LeftTruncatedNB.long_cdf
            lp = lp(data, r, p, self.left, r_transform=self.r_transform)
    ...
    def mean(self, params=None, return_modes=False):
        ...
        if self.dist.startswith('Beta'):
            k = self.get_param('k', params)
            mean = dist.LeftTruncatedBetaCDNB.mean if 'CDNB' in self.dist else dist.LeftTruncatedBetaNB.mean
            ...
            l1 = dist.LeftTruncatedCDNB.mean(r, p1, self.left, r_transform=self.r_transform)
            if self.bad == 1:
                l2 = l1
            else:
                l2 = dist.LeftTruncatedCDNB.mean(r, p2, self.left, r_transform=self.r_transform)
