...
class CDNB(Distribution):
    ...
    @staticmethod
    def logprob(x, r, p, r_transform=None):
        r = CDNB.r_transform(r, p, r_transform)
    ...
    @staticmethod
    def cdf(x, r, p, r_transform=None):
        r = CDNB.r_transform(r, p, r_transform=r_transform)
        return cdnb.cdf(x, r, p)

    @staticmethod
    def sf(x, r, p, r_transform=None):
        return 1.0 - CDNB.cdf(x - 1, r, p, r_transform=r_transform)

    @staticmethod
    def logsf(x, r, p, r_transform=None):
        return jnp.log1p(-CDNB.cdf(x - 1, r, p, r_transform=r_transform))
...
class BetaCDNB(Distribution):
    ...
    def logprob(x, mu, concentration, r, r_transform=None):
        ...
        r = BetaCDNB.r_transform(mu, concentration, r, r_transform)
    ...
    def long_cdf(x, mu, concentration, r, skip=-1, ret_dict=False, r_transform=None):
        ...
        # TODO
        r = BetaCDNB.r_transform(mu, concentration, r, r_transform)
        return CDNB.long_cdf(x, mu, concentration, r, skip=skip, return_dict=ret_dict)
    ...
    def logsf(x, r, p, r_transform=None):
        ...
        # TODO
        r = CDNB.r_transform(r, p, r_transform)
        return jnp.log1p(-CDNB.cdf(x - 1, r, p))
    
    @staticmethod
    def r_transform(mu, concentration, r, r_transform):
        if not r_transform:
            return r
        mean = BetaCDNB.mean(mu, concentration, r, None)
        if r_transform == 'NB':
...
class LeftTruncatedCDNB(CDNB):
    ...
    @staticmethod
    def mean(r, p, left, r_transform=None):
        m = CDNB.mean(r, p, r_transform=r_transform) 
        m -= sum(i * jnp.exp(CDNB.logprob(i, r, p, r_transform=r_transform)) for i in range(1, left + 1)) 
        return m / CDNB.sf(left + 1, r, p, r_transform=r_transform)
    ...
    @staticmethod
    def cdf(x, r, p, left, r_transform=None):
        sm = CDNB.cdf(left, r, p, r_transform=r_transform)
        t = (CDNB.cdf(x, r, p, r_transform=r_transform) - sm) / (1 - sm)
        return t

    @staticmethod
    def sf(x, r, p, left, r_transform=None):
        return 1 - LeftTruncatedCDNB.cdf(x - 1, r, p, left, r_transform=r_transform)
...
class LeftTruncatedBetaCDNB(CDNB):
    ...
    def logprob(x, mu, concentration, r, left, r_transform=None):
        lp = BetaCDNB.logprob(x, mu, concentration, r, r_transform=r_transform)
        sm = jnp.log1p(-jnp.exp(lp.at[..., :left + 1].get()).sum(axis=-1, keepdims=True))
        lp = lp - sm
        lp = lp.at[..., :left + 1].set(-float('inf'))
        return lp
    ...
    def mean(mu, concentration, r, left, r_transform=None):
        m = BetaCDNB.mean(mu, concentration, r,)
