...
def hyp2f1(a, b, c, z, eps=1e-12, n_max=1000):
    def body(carry):
        ...
        # call(lambda x: print(*x), (i, term, jnp.log(res) + exp * jnp.log(2)))
...
def logprob_recurrent(r, p, k, upper, max_sz):
    ...
    # max_ind = jnp.argmax(res)
    # right = inds >= max_ind
    # left = ~right
    # pos = res > 0
    # min_right = jnp.where(pos & right, res, float('inf')).min()
    # min_left = jnp.where(pos & left, res, float('inf')).min()
    # res = jnp.where(pos | left, res, min_right / 1e5)
    # res = jnp.where(pos | right, res, min_left / 1e5)
    res = jnp.abs(res)
