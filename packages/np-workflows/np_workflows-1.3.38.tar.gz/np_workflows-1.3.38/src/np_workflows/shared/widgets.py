...
def probe_coords_widget(session: np_session.Session):
    
    probes = 'ABCDEF'
    
    coords = lambda: session.platform_json.manipulator_coordinates
    probe_coords = lambda img: coords().get(img, dict.fromkeys(probes, dict(x=6000.6, y=0, z=0)))
    
    field_str = lambda s: '_'.join(s.split(' ')).lower() + '_surface_image'
    ...
    def update(_):
        for probe in probes:
            textbox[probe].value = f'{probe_coords(field_str(selection.value))[probe]["z"]:06.1f}'
