...
class TTNMixin:
    ...
    def update_state(self):
        ...
        if self.ttn_session == TTNSession.PRETEST:
            return
        with contextlib.suppress(Exception):
            self.mouse.state['last_session_type'] = str(self.ttn_session)
            self.session.project.state['latest_session'] = self.session.id
    ...
    def scripts(self) -> dict[Literal["main", "mapping", "opto"], str]:
        """Local path to each script on Stim computer.
        ...
        for label in ("main", "mapping", "opto"):
            ...
            stim_copy = self.script_root_on_stim / script
...
def validate_or_overwrite(validate: str | pathlib.Path, src: str | pathlib.Path):
    ...
    while zlib.crc32(validate.read_bytes()) != zlib.crc32(pathlib.Path(src).read_bytes()):
        copy()
