...
class QuantumRegister(Entity):
    ...
    def __add_double_qubit_gate(self, operation: str, address: List[int], **params) -> None:
        ...
        ctrl, targ = int(self.units[address[0]]['qubit'].matrix[0][0]), int(self.units[address[1]]['qubit'].matrix[0][0])
    ...
    def u(self, address: int, theta: Union[float, int], phi: Union[float, int], gamma: Union[float, int],
          **condition) -> None:
