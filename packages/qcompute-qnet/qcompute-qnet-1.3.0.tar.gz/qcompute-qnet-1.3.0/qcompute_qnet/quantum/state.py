...
from qcompute_qnet.quantum.noise import Noise
from qcompute_qnet.quantum.utils import kron, dagger, to_projector, to_superoperator
...
class PureState(QuantumState):
    ...
    def evolve_by_gates(self, gate_history: list) -> None:
        ...
        """
        to_matrix = {
            'id': Gate.I, 's': Gate.S, 't': Gate.T, 'h': Gate.H, 'x': Gate.X, 'y': Gate.Y, 'z': Gate.Z,
            'u': Gate.U, 'u3': Gate.U3, 'rx': Gate.Rx, 'ry': Gate.Ry, 'rz': Gate.Rz,
            'cx': Gate.CNOT, 'cz': Gate.CZ, 'swap': Gate.SWAP,
            'bit_flip': Noise.BitFlip, 'phase_flip': Noise.PhaseFlip, 'bit_phase_flip': Noise.BitPhaseFlip,
            'amplitude_damping': Noise.AmplitudeDamping, 'phase_damping': Noise.PhaseDamping,
        ...
        for gate in gate_history:
            ...
            elif gate["name"] in ["bit_flip", "phase_flip", "bit_phase_flip",
                                  "phase_damping", "amplitude_damping", "depolarizing"]:
    ...
    class SubState(QuantumState.SubState):
        ...
        def compare_size_systems(self, other: "PureState.SubState") -> None:
            ...
            if set(self.systems) != set(other.systems):
                raise ArgumentTypeError(f"The two substates to compare have different system labels,\n"
                                        f"{self.systems}\n"
                                        f"{other.systems}.")
...
class MixedState(QuantumState):
    ...
    def random_density_matrix(cls, qubit_number: int, is_real=False) -> numpy.ndarray:
        ...
        if is_real:
            ...
            mat = random.random((2 ** qubit_number, 2 ** qubit_number)) \
                  + 1j * random.random((2 ** qubit_number, 2 ** qubit_number))
    ...
    class SubState(QuantumState.SubState):
        ...
        def measure(self, which_qubit: Any, basis: numpy.ndarray) -> Tuple[int, list]:
            ...
            for result in [0, 1]:
                ...
                measured_matrix = reshape(super_operator @ reshape(vec_state.matrix,
                                                                   [4, 2 ** (2 * n - 2)]), [2 ** (2 * n - 2), 1])
            ...
            if len(self.systems) == 1:  # single-qubit systems
                ...
                state1 = MixedState.SubState(matrix=to_projector(basis[result]), systems=[which_qubit])  # system being measured
