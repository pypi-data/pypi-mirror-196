...
from qcompute_qnet.models.qkd.resource_management import QKDRMP
...
class QKDRoutingL2(QKDRouting):
        ...
        if msg.data['type'] == QKDMessage.Type.REQUEST:
            if isinstance(self.node, RMPEndNode):
                ...
                if msg.dst in self.node.direct_nodes:
                    ...
                    self.node.env.logger.info(f"Illegal key request: "
                                              f"no RMP repeater between {msg.src.name} and {msg.dst.name}")
            ...
            # Condition 2: Process the request
                ...
                if pattern == 0:
                    ...
                    self.node.env.logger.info(f"{self.node.name} fetched the keys with upstream node "
                                              f"{upstream_node.name} from the key pool: {keys_up}")
                    self.node.env.logger.info(f"{self.node.name} fetched the keys with downstream node "
                ...
                # Pattern 1: RMPEndNode on one side and RMPRepeaterNode on the other side
                    ...
                    ciphertext_list = []
                    keys_repeater = self.node.key_pools[repeater_peer][key_pool_index]. \
                        allocate(key_num, key_length, towards=towards)
                    self.node.env.logger.info(f"{self.node.name} fetched the keys with {repeater_peer.name}"
                    ...
                    if key_end_dict[end_peer]['keys'] is None:
                        ...
                        if key_pool_index == 0:
                            self.keys_up[tuple([repeater_peer, end_peer])] = \
                                {'path': self.path, 'keys': keys_repeater}
                        else:
                        ...
                        cipher_msg = QKDMessage(src=self.node,
                                                ...
                                                data={'type': QKDMessage.Type.CIPHERTEXT,
                                                      ...
                                                      'key_length': msg.data['key_length']})
                        self.node.send_classical_msg(dst=downstream_node, msg=cipher_msg)
                        self.node.env.logger.info(f"{self.node.name} generated the ciphertext with {repeater_peer.name}"
    ...
    def receive_lower(self, lower_protocol: type, **kwargs) -> None:
        ...
        if isinstance(self.node, RMPRepeaterNode):
            if isinstance(peer, RMPRepeaterNode):
                ...
                # Record the key delivery
                self.node.env.logger.info(f"The sifted keys between {self.node.name} and {peer.name} at "
                ...
                if ready_to_deliver is True:
                    ...
                    self.node.env.logger.info(f"Key pools at {self.node.name} were all ready to allocate keys,"
                ...
                if isinstance(repeater_peer, RMPRepeaterNode):
                    if key_repeater_dict.get(tuple([repeater_peer, peer])) is not None:
                        ...
                        for key_repeater, key_end in \
                                zip(self.keys[peer], key_repeater_dict[tuple([repeater_peer, peer])]['keys']):
                        ...
                        self.node.env.logger.info(f"{self.node.name} fetched the allocated keys,"
                        ...
                        self.node.env.logger.info(f"Key generation at {self.node.name} with {peer.name} was ready,"
                    ...
                    if key_repeater_dict[repeater_peer]['keys'] is not None:
                        ...
                        for key_repeater, key_end in zip(self.keys[peer],
                                                         key_repeater_dict[repeater_peer]['keys']):
                            cipher_text_list.append(key_repeater ^ key_end)
                        ...
                        self.node.env.logger.info(f"Key generation at {self.node.name} with {peer.name} was ready, waiting"
    ...
    def receive_classical_msg(self, msg: "QKDMessage", **kwargs) -> None:
        ...
        if msg.data['type'] == QKDMessage.Type.REQUEST:
            self.node.env.logger.info(f"{self.node.name} received 'REQUEST' of QKD "
                                      f"with {msg.dst.name} from {msg.src.name}")
            ...
            self.node.env.logger.info(f"{self.node.name} received 'ACCEPT' of QKD "
                                      f"with {msg.dst.name} from {msg.src.name}")
            ...
            if err_type == 3:
                ...
                discard_msg = QKDMessage(src=msg.src,
                                         dst=msg.dst,
                                         protocol=QKDRMP,
                                         data={'type': QKDMessage.Type.REJECT,
                                               'req_id': msg.data['req_id'],
            ...
            if msg.dst == self.node:
                ...
                if isinstance(self.node, RMPRepeaterNode):
                    self.node.env.logger.info(f"{self.node.name} received ciphertext"
                                              f" from {msg.src.name} to {msg.dst.name}")
            ...
            if msg.dst == self.node:
                ...
                if self.node in msg.data['path']:
                    ...
                    self.node.env.logger.info(f"wrong node {self.node.name} receive a DONE from {msg.src.name} to {msg.dst.name}")
