...
from qcompute_qnet.models.qkd.node import EndNode, BackboneNode, RMPEndNode, RMPRepeaterNode
...
def print_traffic(network: "Network", num_color: int) -> None:
    ...
    for node in network.nodes:
        if isinstance(node, RMPRepeaterNode):
            repeaters_options = {"nodelist": [node], "node_size": 200,
                                 "node_color": color_dict[node], "edgecolors": "black"}
