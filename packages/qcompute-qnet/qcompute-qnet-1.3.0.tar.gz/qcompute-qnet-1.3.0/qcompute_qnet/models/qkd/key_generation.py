...
class KeyGeneration(Protocol, ABC):
    ...
    def finish(self) -> None:
        ...
        if self.is_top:
            ...
            handler = EventHandler(self, "send_upper", [qkd_routing],
                                   peer=self.peer, sifted_keys=self.sifted_keys, towards=towards)
...
class BB84(PrepareAndMeasure):
    ...
    def transmitter_start_protocol(self) -> None:
        ...
        photon_source = self.node.photon_source[self.peer] if isinstance(self.node.photon_source, dict)\
    ...
    def receiver_stop_reception(self) -> None:
        ...
        polar_detector = self.node.polar_detector[self.peer] if isinstance(self.node.polar_detector, dict) \
    ...
    def receiver_receive_classical_msg(self, msg: "BB84.Message", **kwargs) -> None:
        ...
        if msg.data['type'] == BB84.Message.Type.START:
            ...
            if self.round < photon_round:
                self.node.env.logger.debug(f"Message with round {photon_round} was discarded"
                                           f" at local round {self.round}.")
                return
