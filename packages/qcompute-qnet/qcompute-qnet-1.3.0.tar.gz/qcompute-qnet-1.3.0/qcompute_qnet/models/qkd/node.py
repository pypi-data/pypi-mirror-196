...
from qcompute_qnet.models.qkd.application import QKDApp, QKDAppL4
from qcompute_qnet.models.qkd.resource_management import QKDRMP
from qcompute_qnet.models.qkd.routing import QKDRouting, QKDRoutingL2
...
class RMPEndNode(EndNode):
    ...
    def random_req(self) -> None:
        ...
        end_list = [node for node in self.owner.nodes
                    if isinstance(node, RMPEndNode) and node != self and node.idle is True]
...
class RMPRepeaterNode(TrustedRepeaterNode):
    ...
    def init_key_pools(self, max_volume: int, key_length: int) -> None:
        ...
        for neighbor_node in self.links:
            if isinstance(neighbor_node, RMPRepeaterNode):
                ...
                upstream_key_pool = self.KeyPool(node=self,
                                                 peer_node=neighbor_node,
                                                 key_length=key_length,
                                                 max_volume=max_volume)
                downstream_key_pool = self.KeyPool(node=self,
                                                   peer_node=neighbor_node,
                                                   key_length=key_length,
                                                   max_volume=max_volume)
    ...
    def set_key_generation(self, peer=None, **kwargs) -> "KeyGeneration":
        ...
        for proto in qkd_routing_l2.lower_protocols:
            if proto.peer == peer and proto.role == role:
                self.env.logger.info(f"{self.name} found an existed {proto.name}.{proto.role},"
    ...
    class KeyPool(Entity):
        ...
        def __init__(self, node: "Node", peer_node: "Node", key_length: int, max_volume: int,
                     name: str = None, env=None):
        ...
        def key_gen(self, peer: "Node", towards: str, key_num: int = None) -> None:
            ...
            """
            self.node.env.logger.info(f"{self.node.name}'s {towards}stream key pool"
                                      f" started key generation with {peer.name}")
