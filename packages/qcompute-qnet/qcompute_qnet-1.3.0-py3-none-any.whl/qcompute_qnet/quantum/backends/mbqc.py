...
class MBQC:
    ...
    def __apply_cz(self, which_qubits_list: List[Tuple[int, int]]) -> None:
        ...
        for which_qubits in which_qubits_list:
            ...
            new_state.substates[0].matrix = reshape(Gate.CZ() @ reshape(new_state.matrix, [4, qua_length]), [new_state_len, 1])
    ...
    def __apply_pauli_gate(self, gate: str, which_qubit: Any) -> None:
        ...
        new_state.substates[0].matrix = \
            reshape(gate_mat @ reshape(new_state.matrix, [2, half_length]), [new_state_len, 1])
        # Update the order of active vertices and the background state
        self.vertex.active = new_state.systems
        self.__bg_state = PureState(new_state.matrix, new_state.systems)
    ...
    def __create_graph_state(self, which_qubit: Any) -> None:
        ...
        new_bg_state_vector = kron([self.__bg_state.matrix] +
                                   [self.__vertex_to_state[vertex].matrix for vertex in append_qubits])
