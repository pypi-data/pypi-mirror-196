...
class MCalculus:
    ...
    def __to_pattern(self, gate: list) -> None:
        ...
        if name == 'h':  # Hadamard gate
            ...
            commands = [Pattern.CommandM(input_[0],
    ...
    def _get_output(self) -> list:
        ...
        c_out = [(i, max(bit_net[i]) if bit_net[i] != [] else 0) for i in range(self._circuit_width) if
                 i in self._measured_qubits]
        q_out = [(i, max(bit_net[i]) if bit_net[i] != [] else 0) for i in range(self._circuit_width) if
                 i not in self._measured_qubits]
        output_ = [c_out, q_out]
        return output_
