...
from qcompute_qnet.quantum.state import PureState, Zero, Plus
from qcompute_qnet.quantum.utils import kron, find_keys_by_value
...
class Pattern:
    ...
    def to_dynamic_circuit(self, input_states=None) -> "Circuit":
        ...
        for cmd in self.commands:
            if cmd.name == 'N':
                ...
                vertex = cmd.which_qubit
                idx, qreg = _allocate_reg_unit(qreg, vertex)
                gate = {"name": 'm', "which_qubit": [idx], "signature": None,
                        "basis": {"angle": cmd.angle, "plane": cmd.plane,
                                  "domain_s": cmd.domain_s, "domain_t": cmd.domain_t},
                        "mid": vertex}  # used to identify the measurement outcome
    ...
    def run(self, shots: int, input_state=None) -> dict:
        ...
        for shot in range(shots):
            from qcompute_qnet.quantum.backends.mbqc import MBQC
