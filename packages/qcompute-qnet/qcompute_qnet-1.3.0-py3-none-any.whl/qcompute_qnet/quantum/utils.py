...
def complex_log(complex_number: complex) -> complex:
    ...
    if abs(complex_number) <= EPSILON:
        raise ArgumentTypeError(f"Invalid complex number: ({complex_number})!\n"
...
def decompose_to_u_gate(u_mat: numpy.ndarray) -> Tuple[float, float, float]:
    ...
    if not isinstance(u_mat, numpy.ndarray):
        raise ArgumentTypeError(f"Invalid matrix ({u_mat}) with the type: `{type(u_mat)}`!\n"
                                "Only `numpy.ndarray` is supported as the type of the matrix.")
    if u_mat.shape != (2, 2):
        raise ArgumentTypeError(f"Invalid matrix ({u_mat}) with the shape: {u_mat.shape}!\n"
    ...
    if not is_unitary:
        raise ArgumentTypeError(
            f"Invalid matrix ({u_mat}) with the norm: {u_error}! Only unitary matrix is supported.")
...
def plot_results(result_lst: list, legend_lst: list, title=None, xlabel=None, ylabel=None, xticklabels=None) -> None:
    ...
    if not isinstance(result_lst, List):
        raise ArgumentTypeError(f"Invalid dictionary list ({result_lst}) with the type `{type(result_lst)}`!\n"
                                "Only `List` is supported as the dictionary list.")
    if not isinstance(legend_lst, List):
        raise ArgumentTypeError(f"Invalid bar labels ({legend_lst}) with the type `{type(legend_lst)}`!\n"
                                "Only `List` is supported as the bar labels.")
    if len(result_lst) != len(legend_lst):
        raise ArgumentTypeError(f"Invalid dictionary list ({result_lst}) and bar labels ({legend_lst})!\n"
