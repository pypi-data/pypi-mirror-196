"""K-means clustering.
Modified from scikit-learn 1.2.1
https://github.com/scikit-learn/scikit-learn/blob/main/sklearn/cluster/_kmeans.py
https://raw.githubusercontent.com/scikit-learn/scikit-learn/main/sklearn/cluster/_kmeans.py

Only _kmeans_single_elkan has been modified with a callback on iteration end.
So KMeans call a callback only if the algorithm is elkan.
"""
...
def _kmeans_single_elkan(
        ...
        tol=1e-4,
        n_threads=1,
        callback_on_iteration_end=None
):
    ...
    for i in range(max_iter):
        ...
        if np.array_equal(labels, labels_old):
            ...
            if callback_on_iteration_end is not None:
                callback_on_iteration_end(i, True, labels.copy())  # last iteration
            break
        else:
            # No strict convergence, check for tol based convergence.
            if callback_on_iteration_end is not None:
                callback_on_iteration_end(i, False, labels.copy())
    ...
    if not strict_convergence:
        ...
        if callback_on_iteration_end is not None:
            callback_on_iteration_end(i+1, True, labels.copy())
...
class KMeansPartialResult(Bunch):
    def __init__(self, iteration, is_last, labels):
        super().__init__(iteration=iteration, is_last=is_last, labels=labels)
...
class KMeans(_BaseKMeans):
    ...
    def fit(self, X, y=None, sample_weight=None, on_partial_result=None):
        ...
        def callback_on_iteration_end(iteration, is_last, labels):
            if on_partial_result is not None:
                r = KMeansPartialResult(iteration, is_last, labels)
                on_partial_result(r)
        ...
        for i in range(self._n_init):
            ...
            labels, inertia, centers, n_iter_ = kmeans_single(
                ...
                tol=self._tol,
                n_threads=self._n_threads,
                callback_on_iteration_end=callback_on_iteration_end
            )
