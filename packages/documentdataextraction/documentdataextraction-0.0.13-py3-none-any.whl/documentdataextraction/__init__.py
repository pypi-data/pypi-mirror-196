    ...
    def get(self):
        ...
        try:
            ...
            if input_document_type == 'cheque':
                ...
                try:
                    if proc_mtd_value[0] == 'GC':
                        googlecloudaiprocess = GoogleCloudAIDataExtractor()
                        result = googlecloudaiprocess.Data_Process(invoicefile=file_path, templates=templates, input_module=input_reader_module,template_folder=self.template_folder)

                    elif proc_mtd_value[0] == 'AI': 
