...
def normalize_shape_size(input_shape_sizes: List[Union[int,float]], max_shape_size: Union[int,float]):
    input_shape_sizes = np.asarray(input_shape_sizes)
    return ((input_shape_sizes - input_shape_sizes.min()) / (input_shape_sizes.max() - input_shape_sizes.min())) * max_shape_size
...
def visualize_raw_and_adjusted_score_by_position_scatter(crispr_shrinkage_visualization_input: CrisprShrinkageVisualizationInput, max_shape_size = 10):
    for rep_i in crispr_shrinkage_visualization_input.replicate_indices:
        plt.scatter(crispr_shrinkage_visualization_input.explanatory_positions, crispr_shrinkage_visualization_input.explanatory_lfc_rep[rep_i], s=normalize_shape_size(crispr_shrinkage_visualization_input.explanatory_count_rep[rep_i], max_shape_size))
        plt.title("Adjusted LFC")
        plt.show()
        plt.scatter(crispr_shrinkage_visualization_input.explanatory_positions, crispr_shrinkage_visualization_input.explanatory_raw_lfc_rep[rep_i], s=normalize_shape_size(crispr_shrinkage_visualization_input.explanatory_count_rep[rep_i], max_shape_size))
        plt.title("Raw LFC")
        plt.show()
...
def visualize_combined_adjusted_score_by_position_scatter(crispr_shrinkage_visualization_input: CrisprShrinkageVisualizationInput, max_shape_size = 10):
    ...
    for rep_i in crispr_shrinkage_visualization_input.replicate_indices:
        ax.scatter(crispr_shrinkage_visualization_input.explanatory_positions, crispr_shrinkage_visualization_input.explanatory_lfc_rep[rep_i], s=normalize_shape_size(crispr_shrinkage_visualization_input.explanatory_count_rep[rep_i], max_shape_size), alpha=0.3, label="Replicate {}".format(rep_i))
...
def visualize_all_adjusted_score_credible_interval_by_position_scatter(crispr_shrinkage_visualization_input: CrisprShrinkageVisualizationInput, max_shape_size = 10):
    for rep_i in crispr_shrinkage_visualization_input.replicate_indices:
        ...
        ax.scatter(crispr_shrinkage_visualization_input.explanatory_positions,explanatory_lfc_rep_rep_standardized, s=normalize_shape_size(crispr_shrinkage_visualization_input.explanatory_count_rep[rep_i], max_shape_size), color="blue", alpha=0.85, label="Adjusted")
        ax.scatter(crispr_shrinkage_visualization_input.explanatory_positions,explanatory_raw_lfc_rep_standardized, s=normalize_shape_size(crispr_shrinkage_visualization_input.explanatory_count_rep[rep_i], max_shape_size), color="black", alpha=0.85, label="Raw") # Can color based on which of the guides are in the observational set and which are not (specifically which are in the positive, negative.
