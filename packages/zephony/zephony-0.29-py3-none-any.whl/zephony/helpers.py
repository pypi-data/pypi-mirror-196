...
def responsify(data, message=None, http_status=200):
    ...
    if http_status < 400:
        if type(data) == tuple:
            ...
            # Data format is either dict or list
            res = {
                'status': 'success',
                'http_status': http_status,
                'data': data,
...
def send_email(to, subject, mailgun_config, template_string,\
    ...
    if env != 'live':
        to = 'emailtestingforotherthanlive@gmail.com'
        logger.info(f'Environment is not live, sending email to : {to}')
        # mailgun_config = {
        #     'SENDER':'Test',
        #     'URL': 'https://api.mailgun.net/v3/mg.getpreview.io',
        #     # 'API_KEY': 'key-ed7c7844bd3f1a7404469ecc04bf3826',
...
def send_sms(to, body, twilio_config,env=None):
    """
