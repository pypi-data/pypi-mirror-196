...
def create_script():
    ...
    # Search for variables in the commands
    for cmd in cmds.values():
        variables = re.findall(r'\$[a-zA-Z_][a-zA-Z0-9_]*', cmd)
        if variables:
            print(f"Warning: The following variables are used in the command: {', '.join(variables)}")
