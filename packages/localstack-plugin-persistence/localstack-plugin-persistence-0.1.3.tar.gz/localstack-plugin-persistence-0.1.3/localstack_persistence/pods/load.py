...
class InjectPodVisitor(StateVisitor):
	...
	def _(self,state_container):
		...
		C=os.path.relpath(A.path,config.dirs.data)+_A;B=[A for A in self.zip.namelist()if A.startswith(C)]
...
class CloudPodArchive:
	...
	def asset_directories(self):
		'\n        Creates an index of service -> asset directory (path in the zip file)\n        ';D=self.zip.namelist();B={}
		for C in D:
			A=C.lstrip(_A)
			if A.startswith(constants.API_STATES_DIRECTORY):continue
			if not A.endswith(_A):continue
			A=A.rstrip(_A)
			if _A in A:continue
			B[A]=C
		return B