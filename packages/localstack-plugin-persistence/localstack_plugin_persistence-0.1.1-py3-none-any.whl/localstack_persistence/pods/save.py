...
class CreatePodVisitor(StateVisitor):
	...
	def _(self,state_container):
		B=state_container;A=config.dirs.data;A=A.rstrip('/')
		for (E,I,F) in os.walk(B.path):
			C=os.path.relpath(B.path,A).lstrip('/');self.zip.writestr(zipfile.ZipInfo.from_file(A,C),'')
			for D in F:G=os.path.join(E,D);H=os.path.join(C,D);self.zip.write(G,H)