...
from qutil.measurement.spectrometer.daq import settings as daq_settings
...
class _PlotManager:
    ...
    def __init__(self, data: Dict[_keyT, Any], plot_raw: bool = False,
                 ...
                 plot_cumulative_normalized: bool = True, plot_style: _styleT = 'fast',
    ...
    def plot_amplitude(self) -> bool:
        ...
        power spectral density (ASD = sqrt(PSD)). Also applies to the
        cumulative spectrum.
    ...
    def time_plot(self, key, line_type):
        y = self._data[key][f'timetrace_{line_type}'][-1].copy()
        y -= np.mean(y)
    ...
    def setup_figure(self):
        gs = gridspec.GridSpec(1 + self.plot_cumulative + self.plot_timetrace, 1, figure=self.fig,
                               **self.gridspec_kw)
    ...
    def setup_main_axes(self, gs: gridspec.GridSpec):
        if self.axes['main']['processed'] is None:
            self.axes['main']['processed'] = self.fig.add_subplot(gs[0], **self.subplot_kw)
        ...
        self.set_subplotspec('main', gs[0])
        ...
        if self.plot_cumulative:
            if self.axes['cumulative']['processed'] is None:
                self.axes['cumulative']['processed'] = self.fig.add_subplot(
                    gs[1], sharex=self.axes['main']['processed'], **self.subplot_kw
                )
            ...
            self.set_subplotspec('cumulative', gs[1])
        ...
        if self.plot_timetrace:
            ...
            if self.plot_raw:
                if self.axes['time']['raw'] is None:
                    ...
                    self.axes['time']['raw'].set_ylabel('Amplitude ({x})'
                                                        .format(x=self.raw_unit))
    ...
    def get_freq_data(self, key, line_type, dB, reference=False) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if dB and not reference:
            ...
            try:
                ...
                raise RuntimeError(f'dB scale requested but data for key {key} does not have the '
                                   'same shape as reference data with key '
...
class Spectrometer:
    ...
    :func:`~qutil.signal_processing.real_space.welch`.
    ...
    daq_setup : Callable
        ...
        that sets up the data acquisition device for measurement.
        settings is the dictionary that is passed to :meth:`acquire`
        and may be modified by the function if the DAQ only allows
        certain values for instance. The same settings dictionary
        is then passed to the psd_estimator, and so should include
        parameters relevant for that function. See
        :func:`qutil.measurement.spectrometer.daq.qcodes.keysight_344xxA`
        for an example.

        If not given, the instance is read-only and can only be used
        for processing and plotting old data.
    daq_acquire : Callable
        A callable with signature::

            f(**settings) -> Iterable[ndarray]

        that is iterated over for settings['n_avg'] times and that
        executes the measurement, returning a 1d-array of power
        spectral density (PSD) data during each iteration. See
        :func:`qutil.measurement.spectrometer.daq.qcodes.keysight_344xxA`
        for an example.
    ...
    psd_estimator : Callable or kwarg dict
        ...
        :func:`~qutil.signal_processing.real_space.welch` as a PSD
        estimator.
    ...
    procfn : Callable or sequence of Callable
        ...
        :meth:`daq_setup`. If a sequence, the functions are applied
    ...
    plot_cumulative_normalized : bool, default True
    ...
    raw_unit : str
        The unit of the raw, unprocessed data returned by `daq_acquire`.
    processed_unit : str
    ...
    >>> from qutil.measurement.spectrometer.daq import qopt_colored_noise
    >>> def spectrum(f, A=1e-4, exp=1.5, **_):
    ...     return A/f**exp
    >>> savedir = Path(TemporaryDirectory().name)
    >>> setup, acquire = qopt_colored_noise(spectrum)
    >>> spect = Spectrometer(setup, acquire, savepath=savedir)
    ...
    >>> spect.serialize_to_disk(savedir / 'foo')
    >>> spect_loaded = Spectrometer.recall_from_disk(savedir / 'foo',
    ...                                              setup, acquire)
    ...
    @check_literals
    def __init__(self, daq_setup: Optional[Callable] = None,
                 daq_acquire: Optional[Callable] = None,
        ...
        self.daq_acquire = daq_acquire
        self.procfn = chain(*procfn) if np.iterable(procfn) else chain(procfn or Id)
        self.savepath: Path = io.to_global_path(
            savepath or '~/qutil_spectrometer/' + datetime.now().strftime('%Y-%m-%d')
        )
    ...
    def _repr_keys(self) -> str:
        return '\n'.join((str(key) for key in sorted(self.keys())))
    ...
    def _process_data(self, timetrace_raw, **settings) -> Dict[str, Any]:
        ...
        S_processed, f_processed, timetrace_processed = self.psd_estimator(
            self.procfn(timetrace_raw, **settings),
            **settings
        )
        data = dict(timetrace_raw=timetrace_raw,
                    ...
                    settings=daq_settings.DAQSettings(settings))
        return data
    ...
    def take(self, comment: str = '', progress: bool = True, **settings):
        ...
        These are checked for consistency at runtime, since it is for
        example not possible to specify ``f_min`` to be smaller than
        the frequency resolution ``df``. See the ``DAQSettings``
        docstring for examples; the special settings are reproduced
        below.
        ...
        **settings
            ...
            :attr:`fourier_procfn`.
        """
        if not callable(self.daq_setup) or not callable(self.daq_acquire):
            raise ReadonlyError('Cannot take new data since daq_setup or daq_acquire not callable')
        ...
        settings = daq_settings.DAQSettings(self.daq_setup(**settings))
        ...
        iterator = self.daq_acquire(**settings)
            ...
            with filter_warnings('error'):
                try:
                    ...
                    if 'fetched_data' in locals():
                        msg = msg + (f'. {self.daq_acquire} last returned the following data:\n '
                                     f'{fetched_data}')
                    raise RuntimeError(msg) from error

            # Cache data and show new results
        ...
        # Store possible metadata returned by the iterator and save to disk
    ...
    def drop(self, *comment_or_index: _keyT, update_figure: bool = True):
        ...
        *comment_or_index : int | str | (int, str)
            ...
            string comment, or a tuple of both. See :meth:`show_keys`
        ...
        Examples
        --------
        The following are equivalent for a ``Spectrometer`` with keys
        ``[(0, 'a'), (1, 'b')]``::
    ...
    def update_metadata(self, comment_or_index: _keyT, *, delete_old_file: bool = False,
        ...
        *comment_or_index : int | str | (int, str)
        ...
        try:
            ...
            data['filepath'] = backup['filepath'].with_stem(
    ...
    @classmethod
    @mock.patch.multiple('shelve', Unpickler=dill.Unpickler, Pickler=dill.Pickler)
    def recall_from_disk(cls, file: _pathT, daq_setup: Optional[Callable] = None,
                         daq_acquire: Optional[Callable] = None, *,
        ...
        file : str | Path
            The saved file.
        daq_setup, daq_acquire : Callable
            The functions required to set up and execute data
        ...
        spectrometer = cls(daq_setup=daq_setup, daq_acquire=daq_acquire,
                           **cls._make_kwargs_compatible(kwargs))
...
def _ax_unit(plot_amplitude: bool, plot_density: bool, integrated: bool,
             plot_cumulative_normalized: bool, plot_dB: bool, unit: str) -> str:
    s = ' '
    if integrated and plot_cumulative_normalized:
        return s + '(a.u.)'
    s = s + f'({unit}'
    if not plot_amplitude and not plot_dB:
        s = s + '$^2$'
    if integrated:
        if plot_density:
            return s + ')'
    ...
    if plot_density:
        if plot_amplitude:
            return s + r'/$\sqrt{\mathrm{Hz}}$)'
        return s + r'/$\mathrm{Hz}$)'
    return s + ')'


def _ax_label(plot_amplitude: bool, integrated: bool, plot_dB: bool, reference: _keyT) -> str:
    if not plot_dB:
        if not integrated:
            return '$S(f)$'
        return r'$\int_0^f\mathrm{{d}}f^\prime S(f)$'
    if integrated:
        s1 = 'Integrated '
    else:
        s1 = ''
    if plot_amplitude:
        ...
        s2 = 'power '
    return s1 + (s2.capitalize() if not s1 else s2) + f'relative to key {reference}'
