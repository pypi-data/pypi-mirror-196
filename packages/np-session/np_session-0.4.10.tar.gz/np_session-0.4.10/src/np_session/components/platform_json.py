...
class PlatformJson(pydantic.BaseModel):
    ...
    manipulator_coordinates: Dict[str, Dict[str, float | str]] = pydantic.Field(default_factory=dict)
    ...
    def update(self, field, new) -> None:
        ...
        if not new or existing == new:
            return
        logger.info('Updating %s %s: %s -> %s', self.path.name, field, existing, new)
...
def update_from_session(pj: PlatformJson, session) -> None:
    ...
    with pj.write_disabled():
        pj.update('operatorID', session.user.id)
        pj.update('sessionID', session.id)
        pj.update('mouseID', session.mouse.id)
