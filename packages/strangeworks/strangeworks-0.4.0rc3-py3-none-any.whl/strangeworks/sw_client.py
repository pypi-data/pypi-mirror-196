...
class SWClient:
    ...
    def _get_sdk_api(self) -> platform.API:
        if not self._key:
            raise StrangeworksError.authentication_error()
        return platform.API(
            api_key=self._key,
    ...
    def resources(self, slug: Optional[str] = None) -> Optional[List[Resource]]:
        ...
        Optional[List[Resource]]
            ...
            have been created.
        """
        return resource.get(client=self._get_sdk_api(), resource_slug=slug)
    ...
    def workspace_info(self) -> Workspace:
        ...
        return workspace.get(self._get_sdk_api())
    ...
    def get_backend(self, backend_slug: str) -> Backend:
        ...
        Replaces the deprecated BackendsService.
        """
        return get_backend(self._get_sdk_api(), backend_slug)
    ...
    def upload_file(self, file_path: str) -> File:
        ...
        file.
        """
        w = workspace.get(self._get_sdk_api())
        f, signedUrl = file.upload(self._get_sdk_api(), w.slug, file_path)
        try:
    ...
    def set_resource_for_product(
        ...
        self.cfg.set(product_slug=resource_slug)
    ...
    def get_resource_for_product(
        ...
        if len(candidates) > 1:
            raise StrangeworksError(
                message=f"More than one matching resource found for {product_slug}."
                "Please select one."
            )
        if len(candidates) == 0:
            raise StrangeworksError(
