...
PYBIND11_MODULE(_pyikarus, m) {
  ...
  auto lv       = Dune::Python::insertClass<FEreq>(
                ...
                .first;
  lv.def(py::init());
  lv.def("addScalarAffordance", [](FEreq& self, const ScalarAffordances& affordances) { self.addAffordance(affordances); });
  lv.def("addVectorAffordance", [](FEreq& self, const VectorAffordances& affordances) { self.addAffordance(affordances); });
  lv.def("addMatrixAffordance", [](FEreq& self, const MatrixAffordances& affordances) { self.addAffordance(affordances); });
  lv.def(
