...
PYBIND11_MODULE(_pyikarus, m) {
  ...
  enumMA.value("mechanicalPotentialEnergy", MatrixAffordances::geometricstiffness);
  enumMA.value("microMagneticPotentialEnergy", MatrixAffordances::mass);
  enumMA.value("microMagneticPotentialEnergy", MatrixAffordances::materialstiffness);
  enumMA.value("microMagneticPotentialEnergy", MatrixAffordances::stiffness);
  enumMA.value("microMagneticPotentialEnergy", MatrixAffordances::microMagneticHessian);
  enumMA.value("microMagneticPotentialEnergy", MatrixAffordances::stiffnessdiffBucklingVector);
  ...
  auto valueWrapperDouble  = Dune::Python::insertClass<VWd>(
                m, "ValueWrapper", Dune::Python::GenerateTypeName("ValueWrapper<double>"))
                .first;
  ...
  lv.def("insertParameter",
         [](FEreq& self, FEParameter parType, ValueWrapper<double>& parVal) {  self.insertParameter(std::move(parType), parVal.val); },
      py::keep_alive<1,3>(),"FEParameter"_a, "parameterValue"_a.noconvert());

  lv.def("getParameter",
      [](const FEreq& self, FEParameter parType) { return self.getParameter(std::move(parType)); });
}
