...
class YetiApi(object):
    ...
    def entity_search(self, count=50, offset=1, regex=False, **kwargs):
        ...
        json = {
            "filter": kwargs,
            "params": {"page": offset, "range": count, "regex": regex},
        }
    ...
    def add_actor(self, name="", tags=[], description="", **kwargs):
        ...
        """
        return self.__entity_add(
            name=name, entity_type="actor", tags=tags, description=description, **kwargs
        )
    ...
    def add_ttp(self, name="", tags=[], description="", killchain: str = "", **kwargs):
        ...
        Args:
            ...
            tags: A list of tags to add to the entity
            description: A description of the entity
            killchain: The killchain phase of the TTP (Reconnaissance, Weaponisation, Delivery, Exploitation, Installation, C2, Objectives)
            source: The source of the entity
    ...
    def neighbors_observables(self, objectid):
        ...
        return self._make_post("neighbors/tuples/Observable/%s/Observable" % objectid)
    ...
    def entity_to_observables(self, entityid):
        ...
        """
        return self._make_post("neighbors/tuples/Entity/%s/Observable" % entityid)
    ...
    def _entity_to_entities(self, entityid, entity_name):
        ...
        return self._make_post(
            "neighbors/tuples/entity/%s/%s" % (entityid, entity_name)
        )
    ...
    def observable_file_add(self, path, tags=None, context=None):
        ...
        fileinfos = self._make_post("file/addfile", files=files)
    ...
    def observable_bulk_add(self, observables, tags=None, context=None, source="API"):
        ...
        json = {
            "observables": [
                {"tags": tags, "value": o, "source": source, "context": context}
    ...
    def get_analytic_oneshot(self, name_of_oneshot):
        ...
        if all_analytics:
            oneshot = [
                item for item in all_analytics if item["name"] == name_of_oneshot
            ]
            if oneshot:
                return oneshot[0]
    ...
    def analytics_oneshot_run(self, oneshot, observable):
        ...
        one_shot_inst = self._make_post(
            "analytics/oneshot/%s/run" % oneshot["id"], data={"id": observable["id"]}
        )

        status = self.analytics_oneshot_status(one_shot_inst["_id"])
        if status:
            while status["status"] == "running" or status["status"] == "pending":
                status = self.analytics_oneshot_status(one_shot_inst["_id"])
                time.sleep(1)
                continue
            ...
            if status["status"] == "finished":
                ...
                logging.error(
                    "Error Oneshot Processing %s with %s",
                    oneshot["id"],
                    observable["value"],
                )
        return result
    ...
    def analytics_oneshot_list(self):
        ...
        if r:
            ...
            logging.error(
                "Error to list oneshot analytics %s",
    ...
    def _investigation_add_node(self, invest, obs, type_obj="observable"):
        ...
        data = {"links": [], "nodes": [{"$id": {"$oid": obs["id"]}, "$ref": type_obj}]}
    ...
    def investigation_add_link(self, invest, obs_src, obs_dst, label):
        ...
        nodes = []
        nodes.append({"$id": {"$oid": obs_src["id"]}, "$ref": "observable"})
        nodes.append({"$id": {"$oid": obs_dst["id"]}, "$ref": "observable"})
    ...
    def investigation_remove_observable(self, invest, obs_to_delete):
        ...
        node_to_delete = [{"$id": {"$oid": obs_to_delete["id"]}, "$ref": "observable"}]
        links_to_filter = list(
            filter(
                lambda x: x["fromnode"] == "observable-%s" % obs_to_delete["id"]
                or x["tonode"] == "observable-%s" % obs_to_delete["id"],
                invest["links"],
            )
        )
        links_to_delete = []
    ...
    def investigation_remove_link(self, invest, obs_src, obs_dst):
        ...
        for link in links:
            if (
                link["fromnode"] == "observable-%s" % obs_src["id"]
                and link["tonode"] == "observable-%s" % obs_dst["id"]
            ):
                link["from"] = link["fromnode"]
                link["to"] = link["tonode"]
                new_links.append(link)
            if (
                link["fromnode"] == "observable-%s" % obs_dst["id"]
                and link["tonode"] == "observable-%s" % obs_src["id"]
            ):
                link["from"] = link["fromnode"]
                link["to"] = link["tonode"]
                new_links.append(link)
    ...
    def _test_connection(self):
        if self._make_post(
            "observablesearch/"
        ):  # replace this with a more meaningful URL
    ...
    def _make_request(self, url, **kwargs):
        ...
        if method == "POST":
            resp = requests.post(
                url, headers=headers, auth=self.auth, verify=self.verify_ssl, **kwargs
            )
        if method == "GET":
            resp = requests.get(
                url, auth=self.auth, headers=headers, verify=self.verify_ssl
            )
        if method == "DELETE":
        ...
        if method == "DELETE_DATA":
            resp = requests.delete(
                url, auth=self.auth, headers=headers, verify=self.verify_ssl, **kwargs
            )
