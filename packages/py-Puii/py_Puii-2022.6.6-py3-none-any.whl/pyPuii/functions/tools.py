...
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=PUIIUSERBOT/Puii/{}/message={}"
    req_link = chatbot_base.format(
        puii_bot.me.first_name or "puii user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    profile_url = (
        f"https://t.me/{puii_bot.me.username}" if puii_bot.me.username else None
    )
    try:
